/* $Id: ObitTableNX.c 128 2009-09-23 14:48:29Z bill.cotton $   */
...
/**
 * \file ObitTableNX.c
 * ObitTableNX class function definitions.
 *
 * This class is derived from the #ObitTable class.
 */

/** name of the class defined in this file */
static gchar *myClassName = "ObitTableNX";
...
static gchar *myRowClassName = "ObitTableNXRow";
 ...
 */
static ObitTableNXRowClassInfo myRowClassInfo = {FALSE};

/*------------------  Table  ------------------------*/
/**
 * ClassInfo structure ObitTableNXClassInfo.
 * This structure is used by class objects to access class functions.
 */
static ObitTableNXClassInfo myClassInfo = {FALSE};

/*---------------Private function prototypes----------------*/
/** Private: Initialize newly instantiated Row object. */
void  ObitTableNXRowInit  (gpointer in);

/** Private: Deallocate Row members. */
void  ObitTableNXRowClear (gpointer in);

/** Private: Initialize newly instantiated object. */
void  ObitTableNXInit  (gpointer in);

/** Private: Deallocate members. */
void  ObitTableNXClear (gpointer in);

/** Private: update table specific info */
static void ObitTableNXUpdate (ObitTableNX *in, ObitErr *err);

/** Private: copy table keywords to descriptor info list */
static void ObitTableNXDumpKey (ObitTableNX *in, ObitErr *err);

/** Private: Set Class function pointers */
static void ObitTableNXClassInfoDefFn (gpointer inClass);

/** Private: Set Row Class function pointers */
static void ObitTableNXRowClassInfoDefFn (gpointer inClass);
/*----------------------Public functions---------------------------*/
 ...
 */
ObitTableNXRow* newObitTableNXRow (ObitTableNX *table)
{
  ObitTableNXRow* out;
  odouble   *dRow;
  oint      *iRow;
  ...
  if (!myRowClassInfo.initialized) ObitTableNXRowClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTableNXRow));

  /* initialize values */
  out->name = g_strdup("TableNX Row");

  /* set ClassInfo */
  out->ClassInfo = (gpointer)&myRowClassInfo;

  /* initialize other stuff */
  ObitTableNXRowInit((gpointer)out);
 ...
 * \return the new object.
 */
ObitTableNX* newObitTableNX (gchar* name)
{
  ObitTableNX* out;

  /* Class initialization if needed */
  if (!myClassInfo.initialized) ObitTableNXClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTableNX));
 ...
 */
gconstpointer ObitTableNXGetClass (void)
{
  /* Class initialization if needed */
  if (!myClassInfo.initialized) ObitTableNXClassInit();
 ...
 */
ObitTableNX* newObitTableNXValue (gchar* name, ObitData *file, olong *ver,
 	                    ObitIOAccess access,
  		    
		     ObitErr *err)
{
  ObitTableNX* out=NULL;
  ...
  ObitIOCode retCode;
  gchar *tabType = "AIPS NX";
  gchar *routine = "newObitTableNXValue";
 ...
 /* error checks */
  ...
  if (*ver==0) { 
    highVer = ObitTableListGetHigh (list, "AIPS NX");
  ...
  if (*ver>0) { /* has to be specified */
    ...
    if (testTab!=NULL) { /* it exists, use it if is an ObitTableNX */
      if (ObitTableNXIsA(testTab)) { /* it is an ObitTableNX */
	out = ObitTableRef(testTab);
      } else { /* needs conversion */
 	out = ObitTableNXConvert(testTab);
  ...
  for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
  colNo++;
  desc->FieldName[colNo] = g_strdup("START VIS ");
  desc->FieldUnit[colNo] = g_strdup("");
  desc->type[colNo] = OBIT_oint;
  for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
  colNo++;
  desc->FieldName[colNo] = g_strdup("END VIS  ");
  desc->FieldUnit[colNo] = g_strdup("");
  desc->type[colNo] = OBIT_oint;
  for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
  colNo++;
 ...
 return out;
} /* end newObitTableNXValue */

/**
 * Convert an ObitTable to an ObitTableNX.
 ...
 */
ObitTableNX* ObitTableNXConvert (ObitTable* in)
{
  ObitTableNX *out;
 ...
 * \return pointer to the new object.
 */
ObitTableNX* ObitTableNXCopy (ObitTableNX *in, ObitTableNX *out, ObitErr *err)
{
  gchar *routine = "ObitTableNXCopy";
 ...
 */
ObitIOCode ObitTableNXOpen (ObitTableNX *in, ObitIOAccess access, 
			  ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gint32 dim[MAXINFOELEMDIM] = {1,1,1,1,1};
  olong nRowPIO;
  gchar *routine = "ObitTableNXOpen";
 ...
 * \param iNXRow   Row number, -1 -> next
 * \param row      Table Row structure to receive data
 * \param err ObitErr for reporting errors.
 * \return return code, OBIT_IO_OK=> OK
 */
ObitIOCode 
ObitTableNXReadRow  (ObitTableNX *in, olong iNXRow, ObitTableNXRow *row,
		     ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  odouble   *dRow;
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTableNXReadRow";
  ...
  if (in->myStatus == OBIT_Inactive) {
    Obit_log_error(err, OBIT_Error,
		   "AIPS NX Table is inactive for  %s ", in->name);
    return retCode;
 }

  /* read row iNXRow */
  retCode = ObitTableRead ((ObitTable*)in, iNXRow, NULL,  err);
  if (err->error) 
    Obit_traceback_val (err, routine, in->name, retCode);
  ...
  row->StartVis = iRow[in->StartVisOff];
  row->EndVis = iRow[in->EndVisOff];
  row->FreqID = iRow[in->FreqIDOff];
  row->status = iRow[in->myDesc->statusOff];

  return retCode;
} /*  end ObitTableNXReadRow */
 ...
 */
void 
ObitTableNXSetRow  (ObitTableNX *in, ObitTableNXRow *row,
		     ObitErr *err)
{
  odouble   *dRow;
  oint      *iRow;
  ...
  if (in->myStatus == OBIT_Inactive) {
    Obit_log_error(err, OBIT_Error,
		   "NX Table is inactive for  %s ", in->name);
    return;
 }
 ...
 * correct table buffer locations using ObitTableNXSetRow  
 * \param in       Table to read
 * \param iNXRow   Row number, -1 -> next
 ...
 */
ObitIOCode 
ObitTableNXWriteRow  (ObitTableNX *in, olong iNXRow, ObitTableNXRow *row,
		      ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gshort    *siRow;
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTableNXWriteRow";
  ...
  iRow[in->StartVisOff] = row->StartVis;
  iRow[in->EndVisOff] = row->EndVis;
  ...
  /* Write row iNXRow */
  retCode = ObitTableWrite ((ObitTable*)in, iNXRow, NULL,  err);
  if (err->error) 
    Obit_traceback_val (err, routine,in->name, retCode);

  return retCode;
} /*  end ObitTableNXWriteRow */
 ...
 */
ObitIOCode ObitTableNXClose (ObitTableNX *in, ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gchar *routine = "ObitTableNXClose";
 ...
 */
void ObitTableNXRowInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableNXRow *in = inn;
 ...
 *           Actually it should be an ObitTableNXRow* cast to an Obit*.
 */
void ObitTableNXRowClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableNXRow *in = inn;
...
} /* end ObitTableNXRowClear */

/**
 * Initialize global ClassInfo Structure.
 */
void ObitTableNXRowClassInit (void)
{
  ...
  ObitTableNXRowClassInfoDefFn ((gpointer)&myRowClassInfo);
 
  myRowClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTableNXRowClassInit */
...
static void ObitTableNXRowClassInfoDefFn (gpointer inClass)
{
  ObitTableNXRowClassInfo *theClass = (ObitTableNXRowClassInfo*)inClass;
  ...
  theClass->ObitClassInit = (ObitClassInitFP)ObitTableNXRowClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTableNXRowClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTableNXRowGetClass;
  theClass->newObit         = NULL;
  theClass->newObitTableRow = (newObitTableRowFP)newObitTableNXRow;
  theClass->ObitCopy        = NULL;
  theClass->ObitClone       = NULL;
  theClass->ObitClear       = (ObitClearFP)ObitTableNXRowClear;
  theClass->ObitInit        = (ObitInitFP)ObitTableNXRowInit;

} /* end ObitTableNXRowClassDefFn */
 ...
 * \param inn Pointer to the object to initialize.
 */
void ObitTableNXInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableNX *in = inn;
 ...
 * For some reason this wasn't build into the GType class.
 * \param  inn Pointer to the object to deallocate.
 *           Actually it should be an ObitTableNX* cast to an Obit*.
 */
void ObitTableNXClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableNX *in = inn;
...
} /* end ObitTableNXClear */

/**
 * Initialize global ClassInfo Structure.
 */
void ObitTableNXClassInit (void)
{
  ...
  /* Set function pointers */
  ObitTableNXClassInfoDefFn ((gpointer)&myClassInfo);
 
  myClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTableNXClassInit */
...
static void ObitTableNXClassInfoDefFn (gpointer inClass)
{
  ObitTableNXClassInfo *theClass = (ObitTableNXClassInfo*)inClass;
  ...
  /* function pointers defined or overloaded this class */
  theClass->ObitClassInit = (ObitClassInitFP)ObitTableNXClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTableNXClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTableNXGetClass;
  theClass->newObit       = (newObitFP)newObitTableNX;
  theClass->ObitCopy      = (ObitCopyFP)ObitTableNXCopy;
  theClass->ObitClone     = (ObitCloneFP)ObitTableClone;
  theClass->ObitClear     = (ObitClearFP)ObitTableNXClear;
  theClass->ObitInit      = (ObitInitFP)ObitTableNXInit;
  theClass->ObitTableConvert = (ObitTableConvertFP)ObitTableNXConvert;
  theClass->ObitTableOpen    = (ObitTableOpenFP)ObitTableNXOpen;
  theClass->ObitTableClose   = (ObitTableCloseFP)ObitTableNXClose;
  ...
  theClass->ObitTableReadRow = 
    (ObitTableReadRowFP)ObitTableNXReadRow;
  theClass->ObitTableWriteRow = 
    (ObitTableWriteRowFP)ObitTableNXWriteRow;

} /* end ObitTableNXClassDefFn */
...
static void ObitTableNXUpdate (ObitTableNX *in, ObitErr *err)
{
 ...
 /* error checks */
  ...
  if (desc->FieldName) {
    for (i=0; i<desc->nfield; i++) {
	 ...
	 in->SubAOff = desc->offset[i];
      ...
      if (!strncmp (desc->FieldName[i], "START VIS ", 10)) {
	 in->StartVisOff = desc->offset[i];
 	 in->StartVisCol = i;
      }
      if (!strncmp (desc->FieldName[i], "END VIS  ", 9)) {
	 in->EndVisOff = desc->offset[i];
 	 in->EndVisCol = i;
      }
  ...
  Obit_return_if_fail((in->TimeOff > -1), err,
       "ObitTableNXUpdate: Could not find column Time");
  Obit_return_if_fail((in->TimeIOff > -1), err,
       "ObitTableNXUpdate: Could not find column TimeI");
  Obit_return_if_fail((in->SourIDOff > -1), err,
       "ObitTableNXUpdate: Could not find column SourID");
  Obit_return_if_fail((in->SubAOff > -1), err,
       "ObitTableNXUpdate: Could not find column SubA");
  Obit_return_if_fail((in->StartVisOff > -1), err,
       "ObitTableNXUpdate: Could not find column StartVis");
  Obit_return_if_fail((in->EndVisOff > -1), err,
       "ObitTableNXUpdate: Could not find column EndVis");
} /* end ObitTableNXUpdate */
