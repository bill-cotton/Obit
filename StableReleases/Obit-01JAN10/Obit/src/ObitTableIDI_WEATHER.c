...
/**
 * \file ObitTableIDI_WEATHER.c
 * ObitTableIDI_WEATHER class function definitions.
 *
 * This class is derived from the #ObitTable class.
 */

/** name of the class defined in this file */
static gchar *myClassName = "ObitTableIDI_WEATHER";
...
static gchar *myRowClassName = "ObitTableIDI_WEATHERRow";
 ...
 */
static ObitTableIDI_WEATHERRowClassInfo myRowClassInfo = {FALSE};

/*------------------  Table  ------------------------*/
/**
 * ClassInfo structure ObitTableIDI_WEATHERClassInfo.
 * This structure is used by class objects to access class functions.
 */
static ObitTableIDI_WEATHERClassInfo myClassInfo = {FALSE};

/*---------------Private function prototypes----------------*/
/** Private: Initialize newly instantiated Row object. */
void  ObitTableIDI_WEATHERRowInit  (gpointer in);

/** Private: Deallocate Row members. */
void  ObitTableIDI_WEATHERRowClear (gpointer in);

/** Private: Initialize newly instantiated object. */
void  ObitTableIDI_WEATHERInit  (gpointer in);

/** Private: Deallocate members. */
void  ObitTableIDI_WEATHERClear (gpointer in);

/** Private: update table specific info */
static void ObitTableIDI_WEATHERUpdate (ObitTableIDI_WEATHER *in, ObitErr *err);

/** Private: copy table keywords to descriptor info list */
static void ObitTableIDI_WEATHERDumpKey (ObitTableIDI_WEATHER *in, ObitErr *err);

/** Private: Set Class function pointers */
static void ObitTableIDI_WEATHERClassInfoDefFn (gpointer inClass);

/** Private: Set Row Class function pointers */
static void ObitTableIDI_WEATHERRowClassInfoDefFn (gpointer inClass);
/*----------------------Public functions---------------------------*/
...
ObitTableIDI_WEATHERRow* newObitTableIDI_WEATHERRow (ObitTableIDI_WEATHER *table)
{
  ...
  if (!myRowClassInfo.initialized) ObitTableIDI_WEATHERRowClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTableIDI_WEATHERRow));

  /* initialize values */
  out->name = g_strdup("TableIDI_WEATHER Row");

  /* set ClassInfo */
  out->ClassInfo = (gpointer)&myRowClassInfo;

  /* initialize other stuff */
  ObitTableIDI_WEATHERRowInit((gpointer)out);
 ...
 return out;
} /* end newObitTableIDI_WEATHERRow */
...
} /* end ObitTableIDI_WEATHERRowGetClass */
...
{
  ObitTableIDI_WEATHER* out;

  /* Class initialization if needed */
  if (!myClassInfo.initialized) ObitTableIDI_WEATHERClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTableIDI_WEATHER));
  ...
  /* initialize other stuff */
  ObitTableIDI_WEATHERInit((gpointer)out);

 return out;
} /* end newObitTableIDI_WEATHER */
 ...
 */
ObitTableIDI_WEATHER* newObitTableIDI_WEATHERValue (gchar* name, ObitData *file, olong *ver,
 	                    ObitIOAccess access,
  		     oint no_band,
		     ObitErr *err)
{
  ObitTableIDI_WEATHER* out=NULL;
  ...
  ObitIOCode retCode;
  gchar *tabType = "WEATHER";
  gchar *routine = "newObitTableIDI_WEATHERValue";
 ...
 /* error checks */
  ...
  if (*ver==0) { 
    highVer = ObitTableListGetHigh (list, "WEATHER");
  ...
  if (*ver>0) { /* has to be specified */
    ...
    if (testTab!=NULL) { /* it exists, use it if is an ObitTableIDI_WEATHER */
      if (ObitTableIDI_WEATHERIsA(testTab)) { /* it is an ObitTableIDI_WEATHER */
	out = ObitTableRef(testTab);
      } else { /* needs conversion */
 	out = ObitTableIDI_WEATHERConvert(testTab);
  ...
  if (ObitTableIDI_WEATHERIsA(testTab)) { 
    out = ObitTableRef(testTab);
  } else { /* needs conversion */
    out = ObitTableIDI_WEATHERConvert(testTab);
  }
  ...
  out->chan_bw = 1.0;
  out->ref_pixl = 1.0;
  strncpy (out->obscode, "    ", MAXKEYCHARTABLEIDI_WEATHER );
  strncpy (out->RefDate, "YYYY-MM-DD", MAXKEYCHARTABLEIDI_WEATHER );
  ...
  desc->FieldUnit[colNo] = g_strdup("DEG C");
  desc->type[colNo] = OBIT_float;
  for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
  colNo++;
  ...
  desc->FieldUnit[colNo] = g_strdup("DEC");
  desc->type[colNo] = OBIT_float;
  for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
  colNo++;
  desc->FieldName[colNo] = g_strdup("WVR_H2O");
  desc->FieldUnit[colNo] = g_strdup("M**-2");
  desc->type[colNo] = OBIT_float;
  for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
  colNo++;
  desc->FieldName[colNo] = g_strdup("IONOS_ELECTRON");
  desc->FieldUnit[colNo] = g_strdup("M**-2");
  desc->type[colNo] = OBIT_float;
  for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
  colNo++;
 ...
 return out;
} /* end newObitTableIDI_WEATHERValue */

/**
 * Convert an ObitTable to an ObitTableIDI_WEATHER.
...
ObitTableIDI_WEATHER* ObitTableIDI_WEATHERConvert (ObitTable* in)
{
  ...
  /* create basic object */
  out = newObitTableIDI_WEATHER(in->name);
 ...
 out->myHost  = in->myHost;
  ...
  return out;
} /* end ObitTableIDI_WEATHERConvert */
 ...
 * \return pointer to the new object.
 */
ObitTableIDI_WEATHER* ObitTableIDI_WEATHERCopy (ObitTableIDI_WEATHER *in, ObitTableIDI_WEATHER *out, ObitErr *err)
{
  gchar *routine = "ObitTableIDI_WEATHERCopy";
 ...
 /* error checks */
  ...
  out = (ObitTableIDI_WEATHER*)ObitTableCopy ((ObitTable*)in, (ObitTable*)out, err);
  if (err->error) /* add traceback,return */
  ...
  strncpy (out->obscode, in->obscode, MAXKEYCHARTABLEIDI_WEATHER );
  strncpy (out->ArrName, in->ArrName, MAXKEYCHARTABLEIDI_WEATHER );
  strncpy (out->RefDate, in->RefDate, MAXKEYCHARTABLEIDI_WEATHER );
  /* Update class specific info */
  ObitTableIDI_WEATHERUpdate (out, err);
    
  return out;
} /* end ObitTableIDI_WEATHERCopy */
			  ...
			  ObitErr *err)
{
  ...
  olong nRowPIO;
  gchar *routine = "ObitTableIDI_WEATHEROpen";
  ...
  g_assert (ObitIsA(in, &myClassInfo));
   ...
   /* Update class specific info */
   ObitTableIDI_WEATHERUpdate (in, err);
   
   return retCode;
} /* end ObitTableIDI_WEATHEROpen */
 ...
 * \param iIDI_WEATHERRow   Row number, -1 -> next
		     ...
		     ObitErr *err)
{
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTableIDI_WEATHERReadRow";
  ...
  if (in->myStatus == OBIT_Inactive) {
    Obit_log_error(err, OBIT_Error,
		   "IDI_WEATHER Table is inactive for  %s ", in->name);
    return retCode;
 }

  /* read row iIDI_WEATHERRow */
  retCode = ObitTableRead ((ObitTable*)in, iIDI_WEATHERRow, NULL,  err);
  if (err->error) 
    Obit_traceback_val (err, routine, in->name, retCode);
  ...
  row->wind_velocity = fRow[in->wind_velocityOff];
  row->wind_direction = fRow[in->wind_directionOff];
  row->wvr_h2o = fRow[in->wvr_h2oOff];
  row->ionos_electron = fRow[in->ionos_electronOff];
  row->status = iRow[in->myDesc->statusOff];

  return retCode;
} /*  end ObitTableIDI_WEATHERReadRow */
...
} /*  end ObitTableIDI_WEATHERSetRow */
 ...
 * correct table buffer locations using ObitTableIDI_WEATHERSetRow  
 * \param in       Table to read
 * \param iIDI_WEATHERRow   Row number, -1 -> next
		      ...
		      ObitErr *err)
{
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTableIDI_WEATHERWriteRow";
  ...
  fRow[in->wind_velocityOff] = row->wind_velocity;
  fRow[in->wind_directionOff] = row->wind_direction;
  fRow[in->wvr_h2oOff] = row->wvr_h2o;
  fRow[in->ionos_electronOff] = row->ionos_electron;
  ...
  /* Write row iIDI_WEATHERRow */
  retCode = ObitTableWrite ((ObitTable*)in, iIDI_WEATHERRow, NULL,  err);
  if (err->error) 
    Obit_traceback_val (err, routine,in->name, retCode);

  return retCode;
} /*  end ObitTableIDI_WEATHERWriteRow */
...
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gchar *routine = "ObitTableIDI_WEATHERClose";
  ...
  if (in->myDesc->access != OBIT_IO_ReadOnly) 
    ObitTableIDI_WEATHERDumpKey (in, err);
  if (err->error) 
    Obit_traceback_val (err, routine, in->name, retCode);
  ...
  return retCode;
} /* end ObitTableIDI_WEATHERClose */

/*---------------Private functions--------------------------*/
/*----------------  TableIDI_WEATHER Row  ----------------------*/
/**
 * Creates empty member objects, initialize reference count.
 * Parent classes portions are (recursively) initialized first
 * \param inn Pointer to the object to initialize.
 */
void ObitTableIDI_WEATHERRowInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableIDI_WEATHERRow *in = inn;
...
} /* end ObitTableIDI_WEATHERRowInit */
 ...
 *           Actually it should be an ObitTableIDI_WEATHERRow* cast to an Obit*.
 */
void ObitTableIDI_WEATHERRowClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableIDI_WEATHERRow *in = inn;
...
} /* end ObitTableIDI_WEATHERRowClear */
...
void ObitTableIDI_WEATHERRowClassInit (void)
{
  ...
  ObitTableIDI_WEATHERRowClassInfoDefFn ((gpointer)&myRowClassInfo);
 
  myRowClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTableIDI_WEATHERRowClassInit */
...
static void ObitTableIDI_WEATHERRowClassInfoDefFn (gpointer inClass)
{
  ObitTableIDI_WEATHERRowClassInfo *theClass = (ObitTableIDI_WEATHERRowClassInfo*)inClass;
  ...
  theClass->ObitClassInit = (ObitClassInitFP)ObitTableIDI_WEATHERRowClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTableIDI_WEATHERRowClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTableIDI_WEATHERRowGetClass;
  theClass->newObit         = NULL;
  theClass->newObitTableRow = (newObitTableRowFP)newObitTableIDI_WEATHERRow;
  theClass->ObitCopy        = NULL;
  theClass->ObitClone       = NULL;
  theClass->ObitClear       = (ObitClearFP)ObitTableIDI_WEATHERRowClear;
  theClass->ObitInit        = (ObitInitFP)ObitTableIDI_WEATHERRowInit;

} /* end ObitTableIDI_WEATHERRowClassDefFn */
 ...
 * \param inn Pointer to the object to initialize.
 */
void ObitTableIDI_WEATHERInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableIDI_WEATHER *in = inn;
 ...
 * For some reason this wasn't build into the GType class.
 * \param  inn Pointer to the object to deallocate.
 *           Actually it should be an ObitTableIDI_WEATHER* cast to an Obit*.
 */
void ObitTableIDI_WEATHERClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableIDI_WEATHER *in = inn;
...
void ObitTableIDI_WEATHERClassInit (void)
{
  ...
  /* Set function pointers */
  ObitTableIDI_WEATHERClassInfoDefFn ((gpointer)&myClassInfo);
 
  myClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTableIDI_WEATHERClassInit */
...
static void ObitTableIDI_WEATHERClassInfoDefFn (gpointer inClass)
{
  ObitTableIDI_WEATHERClassInfo *theClass = (ObitTableIDI_WEATHERClassInfo*)inClass;
  ...
  /* function pointers defined or overloaded this class */
  theClass->ObitClassInit = (ObitClassInitFP)ObitTableIDI_WEATHERClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTableIDI_WEATHERClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTableIDI_WEATHERGetClass;
  theClass->newObit       = (newObitFP)newObitTableIDI_WEATHER;
  theClass->ObitCopy      = (ObitCopyFP)ObitTableIDI_WEATHERCopy;
  theClass->ObitClone     = (ObitCloneFP)ObitTableClone;
  theClass->ObitClear     = (ObitClearFP)ObitTableIDI_WEATHERClear;
  theClass->ObitInit      = (ObitInitFP)ObitTableIDI_WEATHERInit;
  theClass->ObitTableConvert = (ObitTableConvertFP)ObitTableIDI_WEATHERConvert;
  theClass->ObitTableOpen    = (ObitTableOpenFP)ObitTableIDI_WEATHEROpen;
  theClass->ObitTableClose   = (ObitTableCloseFP)ObitTableIDI_WEATHERClose;
  ...
  theClass->ObitTableReadRow = 
    (ObitTableReadRowFP)ObitTableIDI_WEATHERReadRow;
  theClass->ObitTableWriteRow = 
    (ObitTableWriteRowFP)ObitTableIDI_WEATHERWriteRow;

} /* end ObitTableIDI_WEATHERClassDefFn */
...
static void ObitTableIDI_WEATHERUpdate (ObitTableIDI_WEATHER *in, ObitErr *err)
{
 ...
 /* error checks */
  ...
  ObitInfoListGetTest(in->myDesc->info, "REF_PIXL", &type, dim, 
   ...
   /* obscode */
  strncpy (in->obscode, "    ", MAXKEYCHARTABLEIDI_WEATHER); 
  ...
  if (!ObitInfoListGet(in->myDesc->info, "ARRNAM", &type, dim, 
   ...
   /* RefDate */
  strncpy (in->RefDate, "YYYY-MM-DD", MAXKEYCHARTABLEIDI_WEATHER); 
  ...
  in->dewpointOff = -1;
  in->dewpointCol = -1;
  in->wind_velocityOff = -1;
  in->wind_velocityCol = -1;
  in->wind_directionOff = -1;
  in->wind_directionCol = -1;
  in->wvr_h2oOff = -1;
  in->wvr_h2oCol = -1;
  in->ionos_electronOff = -1;
  in->ionos_electronCol = -1;
  ...
  if (desc->FieldName) {
    for (i=0; i<desc->nfield; i++) {
	 ...
	 in->wind_velocityOff = desc->offset[i];
 	 in->wind_velocityCol = i;
      }
      if (!strncmp (desc->FieldName[i], "WIND_DIRECTION", 14)) {
	 in->wind_directionOff = desc->offset[i];
 	 in->wind_directionCol = i;
      }
      if (!strncmp (desc->FieldName[i], "WVR_H2O", 7)) {
	 in->wvr_h2oOff = desc->offset[i];
 	 in->wvr_h2oCol = i;
      }
      if (!strncmp (desc->FieldName[i], "IONOS_ELECTRON", 14)) {
	 in->ionos_electronOff = desc->offset[i];
 	 in->ionos_electronCol = i;
      }
     }
  }
  ...
  Obit_return_if_fail((in->TimeOff > -1), err,
       "ObitTableIDI_WEATHERUpdate: Could not find column Time");
  Obit_return_if_fail((in->TimeIOff > -1), err,
       "ObitTableIDI_WEATHERUpdate: Could not find column TimeI");
  Obit_return_if_fail((in->antNoOff > -1), err,
       "ObitTableIDI_WEATHERUpdate: Could not find column antNo");
  Obit_return_if_fail((in->temperatureOff > -1), err,
       "ObitTableIDI_WEATHERUpdate: Could not find column temperature");
  Obit_return_if_fail((in->pressureOff > -1), err,
       "ObitTableIDI_WEATHERUpdate: Could not find column pressure");
  Obit_return_if_fail((in->dewpointOff > -1), err,
       "ObitTableIDI_WEATHERUpdate: Could not find column dewpoint");
  Obit_return_if_fail((in->wind_velocityOff > -1), err,
       "ObitTableIDI_WEATHERUpdate: Could not find column wind_velocity");
  Obit_return_if_fail((in->wind_directionOff > -1), err,
       "ObitTableIDI_WEATHERUpdate: Could not find column wind_direction");
  Obit_return_if_fail((in->wvr_h2oOff > -1), err,
       "ObitTableIDI_WEATHERUpdate: Could not find column wvr_h2o");
} /* end ObitTableIDI_WEATHERUpdate */
...
static void ObitTableIDI_WEATHERDumpKey (ObitTableIDI_WEATHER *in, ObitErr *err)
{
 ...
 /* error checks */
  ...
  /* obscode */
  type  = OBIT_string;
  dim[0] = MAXKEYCHARTABLEIDI_WEATHER;
...
} /* end ObitTableIDI_WEATHERDumpKey */
