/* $Id: ObitTableVZ.c 128 2009-09-23 14:48:29Z bill.cotton $   */
...
/**
 * \file ObitTableVZ.c
 * ObitTableVZ class function definitions.
 *
 * This class is derived from the #ObitTable class.
 */

/** name of the class defined in this file */
static gchar *myClassName = "ObitTableVZ";
...
static gchar *myRowClassName = "ObitTableVZRow";
 ...
 */
static ObitTableVZRowClassInfo myRowClassInfo = {FALSE};

/*------------------  Table  ------------------------*/
/**
 * ClassInfo structure ObitTableVZClassInfo.
 * This structure is used by class objects to access class functions.
 */
static ObitTableVZClassInfo myClassInfo = {FALSE};

/*---------------Private function prototypes----------------*/
/** Private: Initialize newly instantiated Row object. */
void  ObitTableVZRowInit  (gpointer in);

/** Private: Deallocate Row members. */
void  ObitTableVZRowClear (gpointer in);

/** Private: Initialize newly instantiated object. */
void  ObitTableVZInit  (gpointer in);

/** Private: Deallocate members. */
void  ObitTableVZClear (gpointer in);

/** Private: update table specific info */
static void ObitTableVZUpdate (ObitTableVZ *in, ObitErr *err);

/** Private: copy table keywords to descriptor info list */
static void ObitTableVZDumpKey (ObitTableVZ *in, ObitErr *err);

/** Private: Set Class function pointers */
static void ObitTableVZClassInfoDefFn (gpointer inClass);

/** Private: Set Row Class function pointers */
static void ObitTableVZRowClassInfoDefFn (gpointer inClass);
/*----------------------Public functions---------------------------*/
 ...
 */
ObitTableVZRow* newObitTableVZRow (ObitTableVZ *table)
{
  ObitTableVZRow* out;
  odouble   *dRow;
  oint      *iRow;
  ...
  if (!myRowClassInfo.initialized) ObitTableVZRowClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTableVZRow));

  /* initialize values */
  out->name = g_strdup("TableVZ Row");

  /* set ClassInfo */
  out->ClassInfo = (gpointer)&myRowClassInfo;

  /* initialize other stuff */
  ObitTableVZRowInit((gpointer)out);
 ...
 * \return the new object.
 */
ObitTableVZ* newObitTableVZ (gchar* name)
{
  ObitTableVZ* out;

  /* Class initialization if needed */
  if (!myClassInfo.initialized) ObitTableVZClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTableVZ));
 ...
 */
gconstpointer ObitTableVZGetClass (void)
{
  /* Class initialization if needed */
  if (!myClassInfo.initialized) ObitTableVZClassInit();
 ...
 */
ObitTableVZ* newObitTableVZValue (gchar* name, ObitData *file, olong *ver,
 	                    ObitIOAccess access,
  		    
		     ObitErr *err)
{
  ObitTableVZ* out=NULL;
  ...
  ObitIOCode retCode;
  gchar *tabType = "AIPS VZ";
  gchar *routine = "newObitTableVZValue";
 ...
 /* error checks */
  ...
  if (*ver==0) { 
    highVer = ObitTableListGetHigh (list, "AIPS VZ");
  ...
  if (*ver>0) { /* has to be specified */
    ...
    if (testTab!=NULL) { /* it exists, use it if is an ObitTableVZ */
      if (ObitTableVZIsA(testTab)) { /* it is an ObitTableVZ */
	out = ObitTableRef(testTab);
      } else { /* needs conversion */
 	out = ObitTableVZConvert(testTab);
 ...
 return out;
} /* end newObitTableVZValue */

/**
 * Convert an ObitTable to an ObitTableVZ.
 ...
 */
ObitTableVZ* ObitTableVZConvert (ObitTable* in)
{
  ObitTableVZ *out;
 ...
 * \return pointer to the new object.
 */
ObitTableVZ* ObitTableVZCopy (ObitTableVZ *in, ObitTableVZ *out, ObitErr *err)
{
  gchar *routine = "ObitTableVZCopy";
 ...
 /* error checks */
  ...
  out = (ObitTableVZ*)ObitTableCopy ((ObitTable*)in, (ObitTable*)out, err);
  if (err->error) /* add traceback,return */
 ...
 */
ObitIOCode ObitTableVZOpen (ObitTableVZ *in, ObitIOAccess access, 
			  ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gint32 dim[MAXINFOELEMDIM] = {1,1,1,1,1};
  olong nRowPIO;
  gchar *routine = "ObitTableVZOpen";
 ...
 * \param iVZRow   Row number, -1 -> next
 * \param row      Table Row structure to receive data
 * \param err ObitErr for reporting errors.
 * \return return code, OBIT_IO_OK=> OK
 */
ObitIOCode 
ObitTableVZReadRow  (ObitTableVZ *in, olong iVZRow, ObitTableVZRow *row,
		     ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  odouble   *dRow;
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTableVZReadRow";
  ...
  if (in->myStatus == OBIT_Inactive) {
    Obit_log_error(err, OBIT_Error,
		   "AIPS VZ Table is inactive for  %s ", in->name);
    return retCode;
 }

  /* read row iVZRow */
  retCode = ObitTableRead ((ObitTable*)in, iVZRow, NULL,  err);
  if (err->error) 
    Obit_traceback_val (err, routine, in->name, retCode);
  ...
  row->Quality = iRow[in->QualityOff];
  row->status = iRow[in->myDesc->statusOff];

  return retCode;
} /*  end ObitTableVZReadRow */
 ...
 */
void 
ObitTableVZSetRow  (ObitTableVZ *in, ObitTableVZRow *row,
		     ObitErr *err)
{
  odouble   *dRow;
  oint      *iRow;
  ...
  if (in->myStatus == OBIT_Inactive) {
    Obit_log_error(err, OBIT_Error,
		   "VZ Table is inactive for  %s ", in->name);
    return;
 }
 ...
 * correct table buffer locations using ObitTableVZSetRow  
 * \param in       Table to read
 * \param iVZRow   Row number, -1 -> next
 ...
 */
ObitIOCode 
ObitTableVZWriteRow  (ObitTableVZ *in, olong iVZRow, ObitTableVZRow *row,
		      ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gshort    *siRow;
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTableVZWriteRow";
  ...
  iRow[in->QualityOff] = row->Quality;
  ...
  /* Write row iVZRow */
  retCode = ObitTableWrite ((ObitTable*)in, iVZRow, NULL,  err);
  if (err->error) 
    Obit_traceback_val (err, routine,in->name, retCode);

  return retCode;
} /*  end ObitTableVZWriteRow */
 ...
 */
ObitIOCode ObitTableVZClose (ObitTableVZ *in, ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gchar *routine = "ObitTableVZClose";
 ...
 */
void ObitTableVZRowInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableVZRow *in = inn;
 ...
 *           Actually it should be an ObitTableVZRow* cast to an Obit*.
 */
void ObitTableVZRowClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableVZRow *in = inn;
...
} /* end ObitTableVZRowClear */

/**
 * Initialize global ClassInfo Structure.
 */
void ObitTableVZRowClassInit (void)
{
  ...
  ObitTableVZRowClassInfoDefFn ((gpointer)&myRowClassInfo);
 
  myRowClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTableVZRowClassInit */
...
static void ObitTableVZRowClassInfoDefFn (gpointer inClass)
{
  ObitTableVZRowClassInfo *theClass = (ObitTableVZRowClassInfo*)inClass;
  ...
  theClass->ObitClassInit = (ObitClassInitFP)ObitTableVZRowClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTableVZRowClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTableVZRowGetClass;
  theClass->newObit         = NULL;
  theClass->newObitTableRow = (newObitTableRowFP)newObitTableVZRow;
  theClass->ObitCopy        = NULL;
  theClass->ObitClone       = NULL;
  theClass->ObitClear       = (ObitClearFP)ObitTableVZRowClear;
  theClass->ObitInit        = (ObitInitFP)ObitTableVZRowInit;

} /* end ObitTableVZRowClassDefFn */
 ...
 * \param inn Pointer to the object to initialize.
 */
void ObitTableVZInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableVZ *in = inn;
 ...
 * For some reason this wasn't build into the GType class.
 * \param  inn Pointer to the object to deallocate.
 *           Actually it should be an ObitTableVZ* cast to an Obit*.
 */
void ObitTableVZClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableVZ *in = inn;
...
} /* end ObitTableVZClear */

/**
 * Initialize global ClassInfo Structure.
 */
void ObitTableVZClassInit (void)
{
  ...
  /* Set function pointers */
  ObitTableVZClassInfoDefFn ((gpointer)&myClassInfo);
 
  myClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTableVZClassInit */
...
static void ObitTableVZClassInfoDefFn (gpointer inClass)
{
  ObitTableVZClassInfo *theClass = (ObitTableVZClassInfo*)inClass;
  ...
  /* function pointers defined or overloaded this class */
  theClass->ObitClassInit = (ObitClassInitFP)ObitTableVZClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTableVZClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTableVZGetClass;
  theClass->newObit       = (newObitFP)newObitTableVZ;
  theClass->ObitCopy      = (ObitCopyFP)ObitTableVZCopy;
  theClass->ObitClone     = (ObitCloneFP)ObitTableClone;
  theClass->ObitClear     = (ObitClearFP)ObitTableVZClear;
  theClass->ObitInit      = (ObitInitFP)ObitTableVZInit;
  theClass->ObitTableConvert = (ObitTableConvertFP)ObitTableVZConvert;
  theClass->ObitTableOpen    = (ObitTableOpenFP)ObitTableVZOpen;
  theClass->ObitTableClose   = (ObitTableCloseFP)ObitTableVZClose;
  ...
  theClass->ObitTableReadRow = 
    (ObitTableReadRowFP)ObitTableVZReadRow;
  theClass->ObitTableWriteRow = 
    (ObitTableWriteRowFP)ObitTableVZWriteRow;

} /* end ObitTableVZClassDefFn */
...
static void ObitTableVZUpdate (ObitTableVZ *in, ObitErr *err)
{
 ...
 /* error checks */
  ...
  if (desc->FieldName) {
    for (i=0; i<desc->nfield; i++) {
	 ...
	 in->PeakIntOff = desc->offset[i];
      ...
      if (!strncmp (desc->FieldName[i], "QUAL  ", 6)) {
  ...
  Obit_return_if_fail((in->Ra2000Off > -1), err,
       "ObitTableVZUpdate: Could not find column Ra2000");
  Obit_return_if_fail((in->Dec2000Off > -1), err,
       "ObitTableVZUpdate: Could not find column Dec2000");
  Obit_return_if_fail((in->PeakIntOff > -1), err,
       "ObitTableVZUpdate: Could not find column PeakInt");
} /* end ObitTableVZUpdate */
