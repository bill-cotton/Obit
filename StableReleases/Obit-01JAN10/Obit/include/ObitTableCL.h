/* $Id: ObitTableCL.h 123 2009-09-04 11:26:14Z bill.cotton $   */
...
/*--------------------------------------------------------------------*/
#ifndef OBITTABLECL_H 
#define OBITTABLECL_H 
 ...
 *
 * \section ObitTableCLSpecification Specifying desired data transfer parameters
 ...
 * The convenience Macro #ObitTableCLSetFITS simplifies specifying the 
 * desired data.
 ...
 *
 * subsection ObitTableCLAIPS AIPS files
 ...
 *
 * \section ObitTableCLaccess Creators and Destructors
 * An ObitTableCL can be created using newObitTableCLValue which attaches the 
 * table to an ObitData for the object.  
 * If the output ObitTableCL has previously been specified, including file information,
 * then ObitTableCLCopy will copy the disk resident as well as the memory 
 * resident information.
 *
 * A copy of a pointer to an ObitTableCL should always be made using the
 * ObitTableCLRef function which updates the reference count in the object.
 * Then whenever freeing an ObitTableCL or changing a pointer, the function
 * ObitTableCLUnref will decrement the reference count and destroy the object
 * when the reference count hits 0.
 *
 * \section ObitTableCLUsage I/O
 * Visibility data is available after an input object is "Opened"
 * and "Read".
 * I/O optionally uses a buffer attached to the ObitTableCL or some external
 * location.
 * To Write an ObitTableCL, create it, open it, and write.
 * The object should be closed to ensure all data is flushed to disk.
 * Deletion of an ObitTableCL after its final unreferencing will automatically
 * close it.
 */
...
/** ObitTableCL Class structure. */
typedef struct {
#include "ObitTableCLDef.h"   /* this class definition */
} ObitTableCL;

/** ObitTableCLRow Class structure. */
typedef struct {
#include "ObitTableCLRowDef.h"   /* this class row definition */
} ObitTableCLRow;

/*----------------- Macroes ---------------------------*/
/** 
 * Macro to unreference (and possibly destroy) an ObitTableCL
 * returns an ObitTableCL*.
...
/** 
 * Macro to unreference (and possibly destroy) an ObitTableCLRow
 * returns an ObitTableCLRow*.
 * in = object to unreference
 */
...
ObitTableCL* 
newObitTableCLValue (gchar* name, ObitData *file, olong *ver,
  		     ObitIOAccess access,
...
ObitTableCL* ObitTableCLCopy  (ObitTableCL *in, ObitTableCL *out, 
			   ObitErr *err);

/** Public: Copy (shallow) constructor. */
ObitTableCL* ObitTableCLClone (ObitTableCL *in, ObitTableCL *out);
 ...
 */
typedef struct  {
#include "ObitTableCLClassDef.h"
} ObitTableCLClassInfo; 

/**
 * ClassInfo Structure For TableCLRow.
 * Contains class name, a pointer to any parent class
 * (NULL if none) and function pointers.
 */
typedef struct  {
#include "ObitTableCLRowClassDef.h"
} ObitTableCLRowClassInfo; 
#endif /* OBITTABLECL_H */ 
