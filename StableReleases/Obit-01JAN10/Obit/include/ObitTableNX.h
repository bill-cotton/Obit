/* $Id: ObitTableNX.h 123 2009-09-04 11:26:14Z bill.cotton $   */
...
/*--------------------------------------------------------------------*/
#ifndef OBITTABLENX_H 
#define OBITTABLENX_H 
 ...
 *
 * \section ObitTableNXSpecification Specifying desired data transfer parameters
 ...
 * The convenience Macro #ObitTableNXSetFITS simplifies specifying the 
 * desired data.
 ...
 *
 * subsection ObitTableNXAIPS AIPS files
 ...
 *
 * \section ObitTableNXaccess Creators and Destructors
 * An ObitTableNX can be created using newObitTableNXValue which attaches the 
 * table to an ObitData for the object.  
 * If the output ObitTableNX has previously been specified, including file information,
 * then ObitTableNXCopy will copy the disk resident as well as the memory 
 * resident information.
 *
 * A copy of a pointer to an ObitTableNX should always be made using the
 * ObitTableNXRef function which updates the reference count in the object.
 * Then whenever freeing an ObitTableNX or changing a pointer, the function
 * ObitTableNXUnref will decrement the reference count and destroy the object
 * when the reference count hits 0.
 *
 * \section ObitTableNXUsage I/O
 * Visibility data is available after an input object is "Opened"
 * and "Read".
 * I/O optionally uses a buffer attached to the ObitTableNX or some external
 * location.
 * To Write an ObitTableNX, create it, open it, and write.
 * The object should be closed to ensure all data is flushed to disk.
 * Deletion of an ObitTableNX after its final unreferencing will automatically
 * close it.
 */
...
/** Number of characters for Table keyword */
 #define MAXKEYCHARTABLENX 24

/** ObitTableNX Class structure. */
typedef struct {
#include "ObitTableNXDef.h"   /* this class definition */
} ObitTableNX;

/** ObitTableNXRow Class structure. */
typedef struct {
#include "ObitTableNXRowDef.h"   /* this class row definition */
} ObitTableNXRow;

/*----------------- Macroes ---------------------------*/
/** 
 * Macro to unreference (and possibly destroy) an ObitTableNX
 * returns an ObitTableNX*.
 * in = object to unreference
 */
#define ObitTableNXUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableNX.
 * returns an ObitTableNX*.
 * in = object to reference
 */
#define ObitTableNXRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableNXIsA(in) ObitIsA (in, ObitTableNXGetClass())

/** 
 * Macro to unreference (and possibly destroy) an ObitTableNXRow
 * returns an ObitTableNXRow*.
 * in = object to unreference
 */
#define ObitTableNXRowUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableNXRow.
 * returns an ObitTableNXRow*.
 * in = object to reference
 */
#define ObitTableNXRowRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableNXRowIsA(in) ObitIsA (in, ObitTableNXRowGetClass())
...
ObitTableNX* 
newObitTableNXValue (gchar* name, ObitData *file, olong *ver,
  		     ObitIOAccess access,
...
ObitTableNX* ObitTableNXCopy  (ObitTableNX *in, ObitTableNX *out, 
			   ObitErr *err);

/** Public: Copy (shallow) constructor. */
ObitTableNX* ObitTableNXClone (ObitTableNX *in, ObitTableNX *out);

/** Public: Convert an ObitTable to an ObitTableNX */
 ...
 */
typedef struct  {
#include "ObitTableNXClassDef.h"
} ObitTableNXClassInfo; 

/**
 * ClassInfo Structure For TableNXRow.
 * Contains class name, a pointer to any parent class
 * (NULL if none) and function pointers.
 */
typedef struct  {
#include "ObitTableNXRowClassDef.h"
} ObitTableNXRowClassInfo; 
#endif /* OBITTABLENX_H */ 
