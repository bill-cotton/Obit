/* $Id: ObitTableFG.h 123 2009-09-04 11:26:14Z bill.cotton $   */
...
/*--------------------------------------------------------------------*/
#ifndef OBITTABLEFG_H 
#define OBITTABLEFG_H 
 ...
 *
 * \section ObitTableFGSpecification Specifying desired data transfer parameters
 ...
 * The convenience Macro #ObitTableFGSetFITS simplifies specifying the 
 * desired data.
 ...
 *
 * subsection ObitTableFGAIPS AIPS files
 ...
 *
 * \section ObitTableFGaccess Creators and Destructors
 * An ObitTableFG can be created using newObitTableFGValue which attaches the 
 * table to an ObitData for the object.  
 * If the output ObitTableFG has previously been specified, including file information,
 * then ObitTableFGCopy will copy the disk resident as well as the memory 
 * resident information.
 *
 * A copy of a pointer to an ObitTableFG should always be made using the
 * ObitTableFGRef function which updates the reference count in the object.
 * Then whenever freeing an ObitTableFG or changing a pointer, the function
 * ObitTableFGUnref will decrement the reference count and destroy the object
 * when the reference count hits 0.
 *
 * \section ObitTableFGUsage I/O
 * Visibility data is available after an input object is "Opened"
 * and "Read".
 * I/O optionally uses a buffer attached to the ObitTableFG or some external
 * location.
 * To Write an ObitTableFG, create it, open it, and write.
 * The object should be closed to ensure all data is flushed to disk.
 * Deletion of an ObitTableFG after its final unreferencing will automatically
 * close it.
 */
...
/** Number of characters for Table keyword */
 #define MAXKEYCHARTABLEFG 24

/** ObitTableFG Class structure. */
typedef struct {
#include "ObitTableFGDef.h"   /* this class definition */
} ObitTableFG;

/** ObitTableFGRow Class structure. */
typedef struct {
#include "ObitTableFGRowDef.h"   /* this class row definition */
} ObitTableFGRow;

/*----------------- Macroes ---------------------------*/
/** 
 * Macro to unreference (and possibly destroy) an ObitTableFG
 * returns an ObitTableFG*.
 * in = object to unreference
 */
#define ObitTableFGUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableFG.
 * returns an ObitTableFG*.
 * in = object to reference
 */
#define ObitTableFGRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableFGIsA(in) ObitIsA (in, ObitTableFGGetClass())

/** 
 * Macro to unreference (and possibly destroy) an ObitTableFGRow
 * returns an ObitTableFGRow*.
 * in = object to unreference
 */
#define ObitTableFGRowUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableFGRow.
 * returns an ObitTableFGRow*.
 * in = object to reference
 */
#define ObitTableFGRowRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableFGRowIsA(in) ObitIsA (in, ObitTableFGRowGetClass())
...
ObitTableFG* 
newObitTableFGValue (gchar* name, ObitData *file, olong *ver,
  		     ObitIOAccess access,
...
ObitTableFG* ObitTableFGCopy  (ObitTableFG *in, ObitTableFG *out, 
			   ObitErr *err);

/** Public: Copy (shallow) constructor. */
ObitTableFG* ObitTableFGClone (ObitTableFG *in, ObitTableFG *out);

/** Public: Convert an ObitTable to an ObitTableFG */
 ...
 */
typedef struct  {
#include "ObitTableFGClassDef.h"
} ObitTableFGClassInfo; 

/**
 * ClassInfo Structure For TableFGRow.
 * Contains class name, a pointer to any parent class
 * (NULL if none) and function pointers.
 */
typedef struct  {
#include "ObitTableFGRowClassDef.h"
} ObitTableFGRowClassInfo; 
#endif /* OBITTABLEFG_H */ 
