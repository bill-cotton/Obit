/* $Id: ObitTableFQ.h 123 2009-09-04 11:26:14Z bill.cotton $   */
...
/*--------------------------------------------------------------------*/
#ifndef OBITTABLEFQ_H 
#define OBITTABLEFQ_H 
 ...
 *
 * \section ObitTableFQSpecification Specifying desired data transfer parameters
 ...
 * The convenience Macro #ObitTableFQSetFITS simplifies specifying the 
 * desired data.
 ...
 *
 * subsection ObitTableFQAIPS AIPS files
 ...
 *
 * \section ObitTableFQaccess Creators and Destructors
 * An ObitTableFQ can be created using newObitTableFQValue which attaches the 
 * table to an ObitData for the object.  
 * If the output ObitTableFQ has previously been specified, including file information,
 * then ObitTableFQCopy will copy the disk resident as well as the memory 
 * resident information.
 *
 * A copy of a pointer to an ObitTableFQ should always be made using the
 * ObitTableFQRef function which updates the reference count in the object.
 * Then whenever freeing an ObitTableFQ or changing a pointer, the function
 * ObitTableFQUnref will decrement the reference count and destroy the object
 * when the reference count hits 0.
 *
 * \section ObitTableFQUsage I/O
 * Visibility data is available after an input object is "Opened"
 * and "Read".
 * I/O optionally uses a buffer attached to the ObitTableFQ or some external
 * location.
 * To Write an ObitTableFQ, create it, open it, and write.
 * The object should be closed to ensure all data is flushed to disk.
 * Deletion of an ObitTableFQ after its final unreferencing will automatically
 * close it.
 */
...
/** Number of characters for Table keyword */
 #define MAXKEYCHARTABLEFQ 24

/** ObitTableFQ Class structure. */
typedef struct {
#include "ObitTableFQDef.h"   /* this class definition */
} ObitTableFQ;

/** ObitTableFQRow Class structure. */
typedef struct {
#include "ObitTableFQRowDef.h"   /* this class row definition */
} ObitTableFQRow;

/*----------------- Macroes ---------------------------*/
/** 
 * Macro to unreference (and possibly destroy) an ObitTableFQ
 * returns an ObitTableFQ*.
 * in = object to unreference
 */
#define ObitTableFQUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableFQ.
 * returns an ObitTableFQ*.
 * in = object to reference
 */
#define ObitTableFQRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableFQIsA(in) ObitIsA (in, ObitTableFQGetClass())

/** 
 * Macro to unreference (and possibly destroy) an ObitTableFQRow
 * returns an ObitTableFQRow*.
 * in = object to unreference
 */
#define ObitTableFQRowUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableFQRow.
 * returns an ObitTableFQRow*.
 * in = object to reference
 */
#define ObitTableFQRowRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableFQRowIsA(in) ObitIsA (in, ObitTableFQRowGetClass())
...
ObitTableFQ* 
newObitTableFQValue (gchar* name, ObitData *file, olong *ver,
  		     ObitIOAccess access,
                     oint numIF,
		     ObitErr *err);
...
ObitTableFQ* ObitTableFQCopy  (ObitTableFQ *in, ObitTableFQ *out, 
			   ObitErr *err);

/** Public: Copy (shallow) constructor. */
ObitTableFQ* ObitTableFQClone (ObitTableFQ *in, ObitTableFQ *out);

/** Public: Convert an ObitTable to an ObitTableFQ */
 ...
 */
typedef struct  {
#include "ObitTableFQClassDef.h"
} ObitTableFQClassInfo; 

/**
 * ClassInfo Structure For TableFQRow.
 * Contains class name, a pointer to any parent class
 * (NULL if none) and function pointers.
 */
typedef struct  {
#include "ObitTableFQRowClassDef.h"
} ObitTableFQRowClassInfo; 
#endif /* OBITTABLEFQ_H */ 
