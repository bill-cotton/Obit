/* $Id: ObitTableVL.h 123 2009-09-04 11:26:14Z bill.cotton $   */
...
/*--------------------------------------------------------------------*/
#ifndef OBITTABLEVL_H 
#define OBITTABLEVL_H 
 ...
 *
 * \section ObitTableVLSpecification Specifying desired data transfer parameters
 ...
 * The convenience Macro #ObitTableVLSetFITS simplifies specifying the 
 * desired data.
 ...
 *
 * subsection ObitTableVLAIPS AIPS files
 ...
 *
 * \section ObitTableVLaccess Creators and Destructors
 * An ObitTableVL can be created using newObitTableVLValue which attaches the 
 * table to an ObitData for the object.  
 * If the output ObitTableVL has previously been specified, including file information,
 * then ObitTableVLCopy will copy the disk resident as well as the memory 
 * resident information.
 *
 * A copy of a pointer to an ObitTableVL should always be made using the
 * ObitTableVLRef function which updates the reference count in the object.
 * Then whenever freeing an ObitTableVL or changing a pointer, the function
 * ObitTableVLUnref will decrement the reference count and destroy the object
 * when the reference count hits 0.
 *
 * \section ObitTableVLUsage I/O
 * Visibility data is available after an input object is "Opened"
 * and "Read".
 * I/O optionally uses a buffer attached to the ObitTableVL or some external
 * location.
 * To Write an ObitTableVL, create it, open it, and write.
 * The object should be closed to ensure all data is flushed to disk.
 * Deletion of an ObitTableVL after its final unreferencing will automatically
 * close it.
 */
...
/** Number of characters for Table keyword */
 #define MAXKEYCHARTABLEVL 24

/** ObitTableVL Class structure. */
typedef struct {
#include "ObitTableVLDef.h"   /* this class definition */
} ObitTableVL;

/** ObitTableVLRow Class structure. */
typedef struct {
#include "ObitTableVLRowDef.h"   /* this class row definition */
} ObitTableVLRow;

/*----------------- Macroes ---------------------------*/
/** 
 * Macro to unreference (and possibly destroy) an ObitTableVL
 * returns an ObitTableVL*.
 * in = object to unreference
 */
#define ObitTableVLUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableVL.
 * returns an ObitTableVL*.
 * in = object to reference
 */
#define ObitTableVLRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableVLIsA(in) ObitIsA (in, ObitTableVLGetClass())

/** 
 * Macro to unreference (and possibly destroy) an ObitTableVLRow
 * returns an ObitTableVLRow*.
 * in = object to unreference
 */
#define ObitTableVLRowUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableVLRow.
 * returns an ObitTableVLRow*.
 * in = object to reference
 */
#define ObitTableVLRowRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableVLRowIsA(in) ObitIsA (in, ObitTableVLRowGetClass())
...
ObitTableVL* 
newObitTableVLValue (gchar* name, ObitData *file, olong *ver,
  		     ObitIOAccess access,
...
ObitTableVL* ObitTableVLCopy  (ObitTableVL *in, ObitTableVL *out, 
			   ObitErr *err);

/** Public: Copy (shallow) constructor. */
ObitTableVL* ObitTableVLClone (ObitTableVL *in, ObitTableVL *out);

/** Public: Convert an ObitTable to an ObitTableVL */
 ...
 */
typedef struct  {
#include "ObitTableVLClassDef.h"
} ObitTableVLClassInfo; 

/**
 * ClassInfo Structure For TableVLRow.
 * Contains class name, a pointer to any parent class
 * (NULL if none) and function pointers.
 */
typedef struct  {
#include "ObitTableVLRowClassDef.h"
} ObitTableVLRowClassInfo; 
#endif /* OBITTABLEVL_H */ 
