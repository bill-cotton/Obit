/* $Id: ObitTableHistory.h 123 2009-09-04 11:26:14Z bill.cotton $   */
...
/**
 * \file ObitTableHistory.h
 * ObitTableHistory class definition.
 *
 ...
 * This class contains tabular data and allows access.
 * "AIPS History" table
 * An ObitTableHistory is the front end to a persistent disk resident structure.
 ...
 *
 * \section ObitTableHistorySpecification Specifying desired data transfer parameters
 ...
 * The convenience Macro #ObitTableHistorySetFITS simplifies specifying the 
 * desired data.
 ...
 *
 * subsection ObitTableHistoryAIPS AIPS files
 ...
 *
 * \section ObitTableHistoryaccess Creators and Destructors
 * An ObitTableHistory can be created using newObitTableHistoryValue which attaches the 
 * table to an ObitData for the object.  
 * If the output ObitTableHistory has previously been specified, including file information,
 * then ObitTableHistoryCopy will copy the disk resident as well as the memory 
 * resident information.
 *
 * A copy of a pointer to an ObitTableHistory should always be made using the
 * ObitTableHistoryRef function which updates the reference count in the object.
 * Then whenever freeing an ObitTableHistory or changing a pointer, the function
 * ObitTableHistoryUnref will decrement the reference count and destroy the object
 * when the reference count hits 0.
 *
 * \section ObitTableHistoryUsage I/O
 * Visibility data is available after an input object is "Opened"
 * and "Read".
 * I/O optionally uses a buffer attached to the ObitTableHistory or some external
 * location.
 * To Write an ObitTableHistory, create it, open it, and write.
 * The object should be closed to ensure all data is flushed to disk.
 * Deletion of an ObitTableHistory after its final unreferencing will automatically
 * close it.
 */
...
/** Number of characters for Table keyword */
 #define MAXKEYCHARTABLEHistory 24

/** ObitTableHistory Class structure. */
typedef struct {
#include "ObitTableHistoryDef.h"   /* this class definition */
} ObitTableHistory;

/** ObitTableHistoryRow Class structure. */
typedef struct {
#include "ObitTableHistoryRowDef.h"   /* this class row definition */
} ObitTableHistoryRow;

/*----------------- Macroes ---------------------------*/
/** 
 * Macro to unreference (and possibly destroy) an ObitTableHistory
 * returns an ObitTableHistory*.
 * in = object to unreference
 */
#define ObitTableHistoryUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableHistory.
 * returns an ObitTableHistory*.
 * in = object to reference
 */
#define ObitTableHistoryRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableHistoryIsA(in) ObitIsA (in, ObitTableHistoryGetClass())

/** 
 * Macro to unreference (and possibly destroy) an ObitTableHistoryRow
 * returns an ObitTableHistoryRow*.
 * in = object to unreference
 */
#define ObitTableHistoryRowUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableHistoryRow.
 * returns an ObitTableHistoryRow*.
 * in = object to reference
 */
#define ObitTableHistoryRowRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableHistoryRowIsA(in) ObitIsA (in, ObitTableHistoryRowGetClass())
...
ObitTableHistory* 
newObitTableHistoryValue (gchar* name, ObitData *file, olong *ver,
  		     ObitIOAccess access,
...
ObitTableHistory* ObitTableHistoryCopy  (ObitTableHistory *in, ObitTableHistory *out, 
			   ObitErr *err);

/** Public: Copy (shallow) constructor. */
ObitTableHistory* ObitTableHistoryClone (ObitTableHistory *in, ObitTableHistory *out);

/** Public: Convert an ObitTable to an ObitTableHistory */
 ...
 */
typedef struct  {
#include "ObitTableHistoryClassDef.h"
} ObitTableHistoryClassInfo; 

/**
 * ClassInfo Structure For TableHistoryRow.
 * Contains class name, a pointer to any parent class
 * (NULL if none) and function pointers.
 */
typedef struct  {
#include "ObitTableHistoryRowClassDef.h"
} ObitTableHistoryRowClassInfo; 
