/* $Id: ObitTableBP.h 123 2009-09-04 11:26:14Z bill.cotton $   */
...
/*--------------------------------------------------------------------*/
#ifndef OBITTABLEBP_H 
#define OBITTABLEBP_H 
 ...
 *
 * \section ObitTableBPSpecification Specifying desired data transfer parameters
 ...
 * The convenience Macro #ObitTableBPSetFITS simplifies specifying the 
 * desired data.
 ...
 *
 * subsection ObitTableBPAIPS AIPS files
 ...
 *
 * \section ObitTableBPaccess Creators and Destructors
 * An ObitTableBP can be created using newObitTableBPValue which attaches the 
 * table to an ObitData for the object.  
 * If the output ObitTableBP has previously been specified, including file information,
 * then ObitTableBPCopy will copy the disk resident as well as the memory 
 * resident information.
 *
 * A copy of a pointer to an ObitTableBP should always be made using the
 * ObitTableBPRef function which updates the reference count in the object.
 * Then whenever freeing an ObitTableBP or changing a pointer, the function
 * ObitTableBPUnref will decrement the reference count and destroy the object
 * when the reference count hits 0.
 *
 * \section ObitTableBPUsage I/O
 * Visibility data is available after an input object is "Opened"
 * and "Read".
 * I/O optionally uses a buffer attached to the ObitTableBP or some external
 * location.
 * To Write an ObitTableBP, create it, open it, and write.
 * The object should be closed to ensure all data is flushed to disk.
 * Deletion of an ObitTableBP after its final unreferencing will automatically
 * close it.
 */
...
/** Number of characters for Table keyword */
 #define MAXKEYCHARTABLEBP 24

/** ObitTableBP Class structure. */
typedef struct {
#include "ObitTableBPDef.h"   /* this class definition */
} ObitTableBP;

/** ObitTableBPRow Class structure. */
typedef struct {
#include "ObitTableBPRowDef.h"   /* this class row definition */
} ObitTableBPRow;

/*----------------- Macroes ---------------------------*/
/** 
 * Macro to unreference (and possibly destroy) an ObitTableBP
 * returns an ObitTableBP*.
 * in = object to unreference
 */
#define ObitTableBPUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableBP.
 * returns an ObitTableBP*.
 * in = object to reference
 */
#define ObitTableBPRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableBPIsA(in) ObitIsA (in, ObitTableBPGetClass())

/** 
 * Macro to unreference (and possibly destroy) an ObitTableBPRow
 * returns an ObitTableBPRow*.
 * in = object to unreference
 */
#define ObitTableBPRowUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableBPRow.
 * returns an ObitTableBPRow*.
 * in = object to reference
 */
#define ObitTableBPRowRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableBPRowIsA(in) ObitIsA (in, ObitTableBPRowGetClass())
...
ObitTableBP* 
newObitTableBPValue (gchar* name, ObitData *file, olong *ver,
  		     ObitIOAccess access,
...
ObitTableBP* ObitTableBPCopy  (ObitTableBP *in, ObitTableBP *out, 
			   ObitErr *err);

/** Public: Copy (shallow) constructor. */
ObitTableBP* ObitTableBPClone (ObitTableBP *in, ObitTableBP *out);

/** Public: Convert an ObitTable to an ObitTableBP */
 ...
 */
typedef struct  {
#include "ObitTableBPClassDef.h"
} ObitTableBPClassInfo; 

/**
 * ClassInfo Structure For TableBPRow.
 * Contains class name, a pointer to any parent class
 * (NULL if none) and function pointers.
 */
typedef struct  {
#include "ObitTableBPRowClassDef.h"
} ObitTableBPRowClassInfo; 
#endif /* OBITTABLEBP_H */ 
