/* $Id: ObitTableAN.h 123 2009-09-04 11:26:14Z bill.cotton $   */
...
/*--------------------------------------------------------------------*/
#ifndef OBITTABLEAN_H 
#define OBITTABLEAN_H 
 ...
 * This class contains tabular data and allows access.
 * "AIPS AN" table
 * An ObitTableAN is the front end to a persistent disk resident structure.
 ...
 *
 * \section ObitTableANSpecification Specifying desired data transfer parameters
 ...
 * The convenience Macro #ObitTableANSetFITS simplifies specifying the 
 * desired data.
 ...
 *
 * subsection ObitTableANAIPS AIPS files
 ...
 *
 * \section ObitTableANaccess Creators and Destructors
 * An ObitTableAN can be created using newObitTableANValue which attaches the 
 * table to an ObitData for the object.  
 * If the output ObitTableAN has previously been specified, including file information,
 * then ObitTableANCopy will copy the disk resident as well as the memory 
 * resident information.
 *
 * A copy of a pointer to an ObitTableAN should always be made using the
 * ObitTableANRef function which updates the reference count in the object.
 * Then whenever freeing an ObitTableAN or changing a pointer, the function
 * ObitTableANUnref will decrement the reference count and destroy the object
 * when the reference count hits 0.
 *
 * \section ObitTableANUsage I/O
 * Visibility data is available after an input object is "Opened"
 * and "Read".
 * I/O optionally uses a buffer attached to the ObitTableAN or some external
 * location.
 * To Write an ObitTableAN, create it, open it, and write.
 * The object should be closed to ensure all data is flushed to disk.
 * Deletion of an ObitTableAN after its final unreferencing will automatically
 * close it.
 */
...
/** Number of characters for Table keyword */
 #define MAXKEYCHARTABLEAN 24

/** ObitTableAN Class structure. */
typedef struct {
#include "ObitTableANDef.h"   /* this class definition */
} ObitTableAN;

/** ObitTableANRow Class structure. */
typedef struct {
#include "ObitTableANRowDef.h"   /* this class row definition */
} ObitTableANRow;

/*----------------- Macroes ---------------------------*/
/** 
 * Macro to unreference (and possibly destroy) an ObitTableAN
 * returns an ObitTableAN*.
 * in = object to unreference
 */
#define ObitTableANUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableAN.
 * returns an ObitTableAN*.
 * in = object to reference
 */
#define ObitTableANRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableANIsA(in) ObitIsA (in, ObitTableANGetClass())

/** 
 * Macro to unreference (and possibly destroy) an ObitTableANRow
 * returns an ObitTableANRow*.
 * in = object to unreference
 */
#define ObitTableANRowUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableANRow.
 * returns an ObitTableANRow*.
 * in = object to reference
 */
#define ObitTableANRowRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableANRowIsA(in) ObitIsA (in, ObitTableANRowGetClass())
...
ObitTableAN* 
newObitTableANValue (gchar* name, ObitData *file, olong *ver,
  		     ObitIOAccess access,
...
ObitTableAN* ObitTableANCopy  (ObitTableAN *in, ObitTableAN *out, 
			   ObitErr *err);

/** Public: Copy (shallow) constructor. */
ObitTableAN* ObitTableANClone (ObitTableAN *in, ObitTableAN *out);

/** Public: Convert an ObitTable to an ObitTableAN */
 ...
 */
typedef struct  {
#include "ObitTableANClassDef.h"
} ObitTableANClassInfo; 

/**
 * ClassInfo Structure For TableANRow.
 * Contains class name, a pointer to any parent class
 * (NULL if none) and function pointers.
 */
typedef struct  {
#include "ObitTableANRowClassDef.h"
} ObitTableANRowClassInfo; 
#endif /* OBITTABLEAN_H */ 
