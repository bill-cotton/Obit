/* $Id: ObitTableSU.h 123 2009-09-04 11:26:14Z bill.cotton $   */
...
/*--------------------------------------------------------------------*/
#ifndef OBITTABLESU_H 
#define OBITTABLESU_H 
 ...
 *
 * \section ObitTableSUSpecification Specifying desired data transfer parameters
 ...
 * The convenience Macro #ObitTableSUSetFITS simplifies specifying the 
 * desired data.
 ...
 *
 * subsection ObitTableSUAIPS AIPS files
 ...
 *
 * \section ObitTableSUaccess Creators and Destructors
 * An ObitTableSU can be created using newObitTableSUValue which attaches the 
 * table to an ObitData for the object.  
 * If the output ObitTableSU has previously been specified, including file information,
 * then ObitTableSUCopy will copy the disk resident as well as the memory 
 * resident information.
 *
 * A copy of a pointer to an ObitTableSU should always be made using the
 * ObitTableSURef function which updates the reference count in the object.
 * Then whenever freeing an ObitTableSU or changing a pointer, the function
 * ObitTableSUUnref will decrement the reference count and destroy the object
 * when the reference count hits 0.
 *
 * \section ObitTableSUUsage I/O
 * Visibility data is available after an input object is "Opened"
 * and "Read".
 * I/O optionally uses a buffer attached to the ObitTableSU or some external
 * location.
 * To Write an ObitTableSU, create it, open it, and write.
 * The object should be closed to ensure all data is flushed to disk.
 * Deletion of an ObitTableSU after its final unreferencing will automatically
 * close it.
 */
...
/** Number of characters for Table keyword */
 #define MAXKEYCHARTABLESU 24

/** ObitTableSU Class structure. */
typedef struct {
#include "ObitTableSUDef.h"   /* this class definition */
} ObitTableSU;

/** ObitTableSURow Class structure. */
typedef struct {
#include "ObitTableSURowDef.h"   /* this class row definition */
} ObitTableSURow;

/*----------------- Macroes ---------------------------*/
/** 
 * Macro to unreference (and possibly destroy) an ObitTableSU
 * returns an ObitTableSU*.
 * in = object to unreference
 */
#define ObitTableSUUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableSU.
 * returns an ObitTableSU*.
 * in = object to reference
 */
#define ObitTableSURef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableSUIsA(in) ObitIsA (in, ObitTableSUGetClass())

/** 
 * Macro to unreference (and possibly destroy) an ObitTableSURow
 * returns an ObitTableSURow*.
 * in = object to unreference
 */
#define ObitTableSURowUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableSURow.
 * returns an ObitTableSURow*.
 * in = object to reference
 */
#define ObitTableSURowRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableSURowIsA(in) ObitIsA (in, ObitTableSURowGetClass())
...
ObitTableSU* 
newObitTableSUValue (gchar* name, ObitData *file, olong *ver,
  		     ObitIOAccess access,
                     oint numIF,
		     ObitErr *err);
...
ObitTableSU* ObitTableSUCopy  (ObitTableSU *in, ObitTableSU *out, 
			   ObitErr *err);

/** Public: Copy (shallow) constructor. */
ObitTableSU* ObitTableSUClone (ObitTableSU *in, ObitTableSU *out);

/** Public: Convert an ObitTable to an ObitTableSU */
 ...
 */
typedef struct  {
#include "ObitTableSUClassDef.h"
} ObitTableSUClassInfo; 

/**
 * ClassInfo Structure For TableSURow.
 * Contains class name, a pointer to any parent class
 * (NULL if none) and function pointers.
 */
typedef struct  {
#include "ObitTableSURowClassDef.h"
} ObitTableSURowClassInfo; 
#endif /* OBITTABLESU_H */ 
