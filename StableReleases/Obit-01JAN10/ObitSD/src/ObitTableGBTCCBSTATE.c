/* $Id: ObitTableGBTCCBSTATE.c 69 2009-01-21 16:00:01Z bill.cotton $  */
...
/**
 * \file ObitTableGBTCCBSTATE.c
 * ObitTableGBTCCBSTATE class function definitions.
 * This class is derived from the ObitTable class.
 */

/** name of the class defined in this file */
static gchar *myClassName = "ObitTableGBTCCBSTATE";
...
static gchar *myRowClassName = "ObitTableGBTCCBSTATERow";
 ...
 */
static ObitTableGBTCCBSTATERowClassInfo myRowClassInfo = {FALSE};

/*------------------  Table  ------------------------*/
/**
 * ClassInfo structure ObitTableGBTCCBSTATEClassInfo.
 * This structure is used by class objects to access class functions.
 */
static ObitTableGBTCCBSTATEClassInfo myClassInfo = {FALSE};


/*---------------Private function prototypes----------------*/
/** Private: Initialize newly instantiated Row object. */
void  ObitTableGBTCCBSTATERowInit  (gpointer in);

/** Private: Deallocate Row members. */
void  ObitTableGBTCCBSTATERowClear (gpointer in);

/** Private: Initialize newly instantiated object. */
void  ObitTableGBTCCBSTATEInit  (gpointer in);

/** Private: Deallocate members. */
void  ObitTableGBTCCBSTATEClear (gpointer in);

/** Private: update table specific info */
static void ObitTableGBTCCBSTATEUpdate (ObitTableGBTCCBSTATE *in, ObitErr *err);

/** Private: copy table keywords to descriptor info list */
static void ObitTableGBTCCBSTATEDumpKey (ObitTableGBTCCBSTATE *in, ObitErr *err);

/** Private: Set Class function pointers */
static void ObitTableGBTCCBSTATEClassInfoDefFn (gpointer inClass);

/** Private: Set Row Class function pointers */
static void ObitTableGBTCCBSTATERowClassInfoDefFn (gpointer inClass);
/*----------------------Public functions---------------------------*/
 ...
 */
ObitTableGBTCCBSTATERow* newObitTableGBTCCBSTATERow (ObitTableGBTCCBSTATE *table)
{
  ObitTableGBTCCBSTATERow* out;
  odouble   *dRow;
  oint      *iRow;
  gshort    *siRow;
  ofloat    *fRow;
  ...
  if (!myRowClassInfo.initialized) ObitTableGBTCCBSTATERowClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTableGBTCCBSTATERow));

  /* initialize values */
  out->name = g_strdup("TableObitTableGBTCCBSTATE Row");

  /* set ClassInfo */
  out->ClassInfo = (gpointer)&myRowClassInfo;

  /* initialize other stuff */
  ObitTableGBTCCBSTATERowInit((gpointer)out);
 ...
 */
gconstpointer ObitTableGBTCCBSTATERowGetClass (void)
{
  /* Class initialization if needed */
  if (!myRowClassInfo.initialized) ObitTableGBTCCBSTATERowClassInit();
  return (gconstpointer)&myRowClassInfo;
} /* end ObitTableGBTCCBSTATERowGetClass */
 ...
 * \return the new object.
 */
ObitTableGBTCCBSTATE* newObitTableGBTCCBSTATE (gchar* name)
{
  ObitTableGBTCCBSTATE* out;

  /* Class initialization if needed */
  if (!myClassInfo.initialized) ObitTableGBTCCBSTATEClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTableGBTCCBSTATE));
  ...
  /* initialize other stuff */
  ObitTableGBTCCBSTATEInit((gpointer)out);

 return out;
} /* end newObitTableGBTCCBSTATE */

/**
 * Returns ClassInfo pointer for the class.
 * \return pointer to the class structure.
 */
gconstpointer ObitTableGBTCCBSTATEGetClass (void)
{
  /* Class initialization if needed */
  if (!myClassInfo.initialized) ObitTableGBTCCBSTATEClassInit();
  return (gconstpointer)&myClassInfo;
} /* end ObitTableGBTCCBSTATEGetClass */
 ...
 */
ObitTableGBTCCBSTATE* newObitTableGBTCCBSTATEValue (gchar* name, ObitData *file, olong *ver,
 				  ObitIOAccess access,
  		    
		     ObitErr *err)
{
  ObitTableGBTCCBSTATE* out=NULL;
  ...
  ObitIOCode retCode;
  gchar *tabType = "STATE";
  gchar *routine = "newObitTableGBTCCBSTATEValue";
 ...
 /* error checks */
  ...
  if (*ver>0) { /* has to be specified */
    ...
    if (testTab!=NULL) { /* it exists, use it if is an ObitTableGBTCCBSTATE */
      if (ObitTableGBTCCBSTATEIsA(testTab)) { /* it is an ObitTableGBTCCBSTATE */
	out = ObitTableRef(testTab);
      } else { /* needs conversion */
 	out = ObitTableGBTCCBSTATEConvert(testTab);
  ...
  if (ObitTableGBTCCBSTATEIsA(testTab)) { 
    out = ObitTableRef(testTab);
  } else { /* needs conversion */
    out = ObitTableGBTCCBSTATEConvert(testTab);
  }
  ...
  /* Define Columns */
  desc->FieldName[colNo] = g_strdup("PHIA");
  desc->FieldUnit[colNo] = g_strdup("state");
  desc->type[colNo] = OBIT_oint;
  for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
  colNo++;
  desc->FieldName[colNo] = g_strdup("PHIB");
  desc->FieldUnit[colNo] = g_strdup("state");
  desc->type[colNo] = OBIT_oint;
  for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
  colNo++;
 ...
 return out;
} /* end newObitTableGBTCCBSTATEValue */

/**
 * Convert an ObitTable to an ObitTableGBTCCBSTATE.
 ...
 */
ObitTableGBTCCBSTATE* ObitTableGBTCCBSTATEConvert (ObitTable* in)
{
  ObitTableGBTCCBSTATE *out;
  ...
  /* create basic object */
  out = newObitTableGBTCCBSTATE(in->name);
 ...
 out->myHost  = in->myHost;
  ...
  return out;
} /* end ObitTableGBTCCBSTATEConvert */
 ...
 * \return pointer to the new object.
 */
ObitTableGBTCCBSTATE* ObitTableGBTCCBSTATECopy (ObitTableGBTCCBSTATE *in, ObitTableGBTCCBSTATE *out, ObitErr *err)
{
  gchar *routine = "ObitTableGBTCCBSTATECopy";
 ...
 /* error checks */
  ...
  out = (ObitTableGBTCCBSTATE*)ObitTableCopy ((ObitTable*)in, (ObitTable*)out, err);
  if (err->error) /* add traceback,return */
    Obit_traceback_val (err, routine,in->name, out);

  /* Copy this class  info */
  out->nphases = in->nphases;
  /* Update class specific info */
  ObitTableGBTCCBSTATEUpdate (out, err);
    
  return out;
} /* end ObitTableGBTCCBSTATECopy */
 ...
 */
ObitIOCode ObitTableGBTCCBSTATEOpen (ObitTableGBTCCBSTATE *in, ObitIOAccess access, 
			  ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gint32 dim[MAXINFOELEMDIM] = {1,1,1,1,1};
  olong nRowPIO;
  gchar *routine = "ObitTableGBTCCBSTATEOpen";
  ...
  g_assert (ObitIsA(in, &myClassInfo));
   ...
   /* Update class specific info */
   ObitTableGBTCCBSTATEUpdate (in, err);
   
   return retCode;
} /* end ObitTableGBTCCBSTATEOpen */
 ...
 */
ObitIOCode 
ObitTableGBTCCBSTATEReadRow  (ObitTableGBTCCBSTATE *in, olong iSTATERow, ObitTableGBTCCBSTATERow *row,
		     ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  odouble   *dRow;
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTableGBTCCBSTATEReadRow";
  ...
  if (in->myStatus == OBIT_Inactive) {
    Obit_log_error(err, OBIT_Error,
		   "ObitTableGBTCCBSTATE Table is inactive for  %s ", in->name);
    return retCode;
 }
  ...
  row->phia = iRow[in->phiaOff];
  row->phib = iRow[in->phibOff];
  row->status = iRow[in->myDesc->statusOff];

  return retCode;
} /*  end ObitTableGBTCCBSTATEReadRow */
 ...
 */
void 
ObitTableGBTCCBSTATESetRow  (ObitTableGBTCCBSTATE *in, ObitTableGBTCCBSTATERow *row,
		     ObitErr *err)
{
  odouble   *dRow;
  oint      *iRow;
...
} /*  end ObitTableGBTCCBSTATESetRow */
 ...
 * correct table buffer locations using ObitTableGBTCCBSTATESetRow  
 * \param in       Table to read
 * \param iSTATERow   Row number, -1 -> next
 ...
 */
ObitIOCode 
ObitTableGBTCCBSTATEWriteRow  (ObitTableGBTCCBSTATE *in, olong iSTATERow, ObitTableGBTCCBSTATERow *row,
		      ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gshort    *siRow;
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTableGBTCCBSTATEWriteRow";
  ...
  iRow[in->phiaOff] = row->phia;
  iRow[in->phibOff] = row->phib;
  ...
  return retCode;
} /*  end ObitTableGBTCCBSTATEWriteRow */
 ...
 */
ObitIOCode ObitTableGBTCCBSTATEClose (ObitTableGBTCCBSTATE *in, ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gchar *routine = "ObitTableGBTCCBSTATEClose";
  ...
  if (in->myDesc->access != OBIT_IO_ReadOnly) 
    ObitTableGBTCCBSTATEDumpKey (in, err);
  if (err->error) 
    Obit_traceback_val (err, routine, in->name, retCode);
  ...
  return retCode;
} /* end ObitTableGBTCCBSTATEClose */

/*---------------Private functions--------------------------*/
/*----------------  ObitTableGBTCCBSTATE Row  ----------------------*/
/**
 * Creates empty member objects, initialize reference count.
 * Parent classes portions are (recursively) initialized first
 * \param inn Pointer to the object to initialize.
 */
void ObitTableGBTCCBSTATERowInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableGBTCCBSTATERow *in = inn;
...
} /* end ObitTableGBTCCBSTATERowInit */
 ...
 *           Actually it should be an ObitTableGBTCCBSTATERow* cast to an Obit*.
 */
void ObitTableGBTCCBSTATERowClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableGBTCCBSTATERow *in = inn;
...
} /* end ObitTableGBTCCBSTATERowClear */

/**
 * Initialize global ClassInfo Structure.
 */
void ObitTableGBTCCBSTATERowClassInit (void)
{
  ...
  ObitTableGBTCCBSTATERowClassInfoDefFn ((gpointer)&myRowClassInfo);
 
  myRowClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTableGBTCCBSTATERowClassInit */
...
static void ObitTableGBTCCBSTATERowClassInfoDefFn (gpointer inClass)
{
  ObitTableGBTCCBSTATERowClassInfo *theClass = (ObitTableGBTCCBSTATERowClassInfo*)inClass;
  ...
  theClass->ObitClassInit = (ObitClassInitFP)ObitTableGBTCCBSTATERowClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTableGBTCCBSTATERowClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTableGBTCCBSTATERowGetClass;
  theClass->newObit         = NULL;
  theClass->newObitTableRow = (newObitTableRowFP)newObitTableGBTCCBSTATERow;
  theClass->ObitCopy        = NULL;
  theClass->ObitClone       = NULL;
  theClass->ObitClear       = (ObitClearFP)ObitTableGBTCCBSTATERowClear;
  theClass->ObitInit        = (ObitInitFP)ObitTableGBTCCBSTATERowInit;

} /* end ObitTableGBTCCBSTATERowClassDefFn */

/*------------------  ObitTableGBTCCBSTATE  ------------------------*/
 ...
 * \param inn Pointer to the object to initialize.
 */
void ObitTableGBTCCBSTATEInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableGBTCCBSTATE *in = inn;
 ...
 * For some reason this wasn't build into the GType class.
 * \param  inn Pointer to the object to deallocate.
 *           Actually it should be an ObitTableGBTCCBSTATE* cast to an Obit*.
 */
void ObitTableGBTCCBSTATEClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableGBTCCBSTATE *in = inn;
...
} /* end ObitTableGBTCCBSTATEClear */

/**
 * Initialize global ClassInfo Structure.
 */
void ObitTableGBTCCBSTATEClassInit (void)
{
  ...
  /* Set function pointers */
  ObitTableGBTCCBSTATEClassInfoDefFn ((gpointer)&myClassInfo);
 
  myClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTableGBTCCBSTATEClassInit */
...
static void ObitTableGBTCCBSTATEClassInfoDefFn (gpointer inClass)
{
  ObitTableGBTCCBSTATEClassInfo *theClass = (ObitTableGBTCCBSTATEClassInfo*)inClass;
  ...
  /* function pointers defined or overloaded this class */
  theClass->ObitClassInit = (ObitClassInitFP)ObitTableGBTCCBSTATEClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTableGBTCCBSTATEClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTableGBTCCBSTATEGetClass;
  theClass->newObit       = (newObitFP)newObitTableGBTCCBSTATE;
  theClass->ObitCopy      = (ObitCopyFP)ObitTableGBTCCBSTATECopy;
  theClass->ObitClone     = (ObitCloneFP)ObitTableClone;
  theClass->ObitClear     = (ObitClearFP)ObitTableGBTCCBSTATEClear;
  theClass->ObitInit      = (ObitInitFP)ObitTableGBTCCBSTATEInit;
  theClass->ObitTableConvert = (ObitTableConvertFP)ObitTableGBTCCBSTATEConvert;
  theClass->ObitTableOpen    = (ObitTableOpenFP)ObitTableGBTCCBSTATEOpen;
  theClass->ObitTableClose   = (ObitTableCloseFP)ObitTableGBTCCBSTATEClose;
  ...
  theClass->ObitTableReadRow = 
    (ObitTableReadRowFP)ObitTableGBTCCBSTATEReadRow;
  theClass->ObitTableWriteRow = 
    (ObitTableWriteRowFP)ObitTableGBTCCBSTATEWriteRow;

} /* end ObitTableGBTCCBSTATEClassDefFn */
...
static void ObitTableGBTCCBSTATEUpdate (ObitTableGBTCCBSTATE *in, ObitErr *err)
{
 ...
 /* error checks */
  ...
  /* Get Keywords */
   /* nphases */
  if (!ObitInfoListGet(in->myDesc->info, "NPHASES", &type, (gint32*)&dim, 
		       (gpointer)&in->nphases, err)) return;
  ...
  if (desc->FieldName) {
    for (i=0; i<desc->nfield; i++) {
      if (!strncmp (desc->FieldName[i], "PHIA", 4)) {
	 in->phiaOff = desc->offset[i];
 	 in->phiaCol = i;
      }
      if (!strncmp (desc->FieldName[i], "PHIB", 4)) {
	 in->phibOff = desc->offset[i];
 	 in->phibCol = i;
      }
     }
  }

  Obit_return_if_fail((in->phiaOff > -1), err,
       "ObitTableSTATEUpdate: Could not find column phia");
} /* end ObitTableGBTCCBSTATEUpdate */
...
static void ObitTableGBTCCBSTATEDumpKey (ObitTableGBTCCBSTATE *in, ObitErr *err)
{
 ...
 /* error checks */
  ...
  type  = OBIT_oint;
  dim[0] = 1;
  ObitInfoListAlwaysPut(info, "NPHASES", type, (gint32*)&dim, 
		  (gpointer)&in->nphases);
   
} /* end ObitTableGBTCCBSTATEDumpKey */
