/* $Id: ObitTableOTFModel.c 69 2009-01-21 16:00:01Z bill.cotton $  */
...
/**
 * \file ObitTableOTFModel.c
 * ObitTableOTFModel class function definitions.
 * This class is derived from the ObitTable class.
 */

/** name of the class defined in this file */
static gchar *myClassName = "ObitTableOTFModel";
...
static gchar *myRowClassName = "ObitTableOTFModelRow";
 ...
 */
static ObitTableOTFModelRowClassInfo myRowClassInfo = {FALSE};

/*------------------  Table  ------------------------*/
/**
 * ClassInfo structure ObitTableOTFModelClassInfo.
 * This structure is used by class objects to access class functions.
 */
static ObitTableOTFModelClassInfo myClassInfo = {FALSE};


/*---------------Private function prototypes----------------*/
/** Private: Initialize newly instantiated Row object. */
void  ObitTableOTFModelRowInit  (gpointer in);

/** Private: Deallocate Row members. */
void  ObitTableOTFModelRowClear (gpointer in);

/** Private: Initialize newly instantiated object. */
void  ObitTableOTFModelInit  (gpointer in);

/** Private: Deallocate members. */
void  ObitTableOTFModelClear (gpointer in);

/** Private: update table specific info */
static void ObitTableOTFModelUpdate (ObitTableOTFModel *in, ObitErr *err);

/** Private: copy table keywords to descriptor info list */
static void ObitTableOTFModelDumpKey (ObitTableOTFModel *in, ObitErr *err);

/** Private: Set Class function pointers */
static void ObitTableOTFModelClassInfoDefFn (gpointer inClass);

/** Private: Set Row Class function pointers */
static void ObitTableOTFModelRowClassInfoDefFn (gpointer inClass);
/*----------------------Public functions---------------------------*/
...
ObitTableOTFModelRow* newObitTableOTFModelRow (ObitTableOTFModel *table)
{
  ...
  if (!myRowClassInfo.initialized) ObitTableOTFModelRowClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTableOTFModelRow));

  /* initialize values */
  out->name = g_strdup("TableObitTableOTFModel Row");

  /* set ClassInfo */
  out->ClassInfo = (gpointer)&myRowClassInfo;

  /* initialize other stuff */
  ObitTableOTFModelRowInit((gpointer)out);
...
} /* end ObitTableOTFModelRowGetClass */
...
{
  ObitTableOTFModel* out;

  /* Class initialization if needed */
  if (!myClassInfo.initialized) ObitTableOTFModelClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTableOTFModel));
  ...
  /* initialize other stuff */
  ObitTableOTFModelInit((gpointer)out);

 return out;
} /* end newObitTableOTFModel */
 ...
 * \param numParm The number of IFs
 * \param err Error stack, returns if not empty.
 * \return the new object, NULL on failure.
 */
ObitTableOTFModel* newObitTableOTFModelValue (gchar* name, ObitData *file, olong *ver,
 				  ObitIOAccess access,
  		     oint numParm,
		     ObitErr *err)
{
  ObitTableOTFModel* out=NULL;
  ...
  ObitIOCode retCode;
  gchar *tabType = "OTFModel";
  gchar *routine = "newObitTableOTFModelValue";
 ...
 /* error checks */
  ...
  if (*ver>0) { /* has to be specified */
    ...
    if (testTab!=NULL) { /* it exists, use it if is an ObitTableOTFModel */
      if (ObitTableOTFModelIsA(testTab)) { /* it is an ObitTableOTFModel */
	out = ObitTableRef(testTab);
      } else { /* needs conversion */
 	out = ObitTableOTFModelConvert(testTab);
  ...
  if (access==OBIT_IO_ReadOnly) { 
    highVer = ObitTableListGetHigh (list, "OTFModel");
  ...
  if (ObitTableOTFModelIsA(testTab)) { 
    out = ObitTableRef(testTab);
  } else { /* needs conversion */
    out = ObitTableOTFModelConvert(testTab);
  }
  ...
  /* Set values */
  out->numParm = MAX (0, numParm);

  /* initialize descriptor */
  desc = out->myDesc;
  /* How many columns actually in table? */
  ncol = 5 + out->numParm*0 ;
 ...
 return out;
} /* end newObitTableOTFModelValue */

/**
 * Convert an ObitTable to an ObitTableOTFModel.
...
ObitTableOTFModel* ObitTableOTFModelConvert (ObitTable* in)
{
  ...
  /* create basic object */
  out = newObitTableOTFModel(in->name);
 ...
 * \return pointer to the new object.
 */
ObitTableOTFModel* ObitTableOTFModelCopy (ObitTableOTFModel *in, ObitTableOTFModel *out, ObitErr *err)
{
  gchar *routine = "ObitTableOTFModelCopy";
 ...
 /* error checks */
  ...
  out = (ObitTableOTFModel*)ObitTableCopy ((ObitTable*)in, (ObitTable*)out, err);
  if (err->error) /* add traceback,return */
    Obit_traceback_val (err, routine,in->name, out);

  /* Copy this class  info */
  out->numParm = in->numParm;
  /* Update class specific info */
  ObitTableOTFModelUpdate (out, err);
    
  return out;
} /* end ObitTableOTFModelCopy */
			  ...
			  ObitErr *err)
{
  ...
  olong nRowPIO;
  gchar *routine = "ObitTableOTFModelOpen";
  ...
  g_assert (ObitIsA(in, &myClassInfo));
   ...
   /* Update class specific info */
   ObitTableOTFModelUpdate (in, err);
   
   return retCode;
} /* end ObitTableOTFModelOpen */
 ...
 * \param iOTFModelRow   Row number, -1 -> next
		     ...
		     ObitErr *err)
{
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTableOTFModelReadRow";
  ...
  if (in->myStatus == OBIT_Inactive) {
    Obit_log_error(err, OBIT_Error,
		   "ObitTableOTFModel Table is inactive for  %s ", in->name);
    return retCode;
 }

  /* read row iOTFModelRow */
  retCode = ObitTableRead ((ObitTable*)in, iOTFModelRow, NULL,  err);
  if (err->error) 
    Obit_traceback_val (err, routine, in->name, retCode);
		     ...
		     ObitErr *err)
{
  ...
  if (in->myStatus == OBIT_Inactive) {
    Obit_log_error(err, OBIT_Error,
		   "OTFModel Table is inactive for  %s ", in->name);
    return;
 }
 ...
 * correct table buffer locations using ObitTableOTFModelSetRow  
 * \param in       Table to read
 * \param iOTFModelRow   Row number, -1 -> next
		      ...
		      ObitErr *err)
{
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTableOTFModelWriteRow";
  ...
  /* Write row iOTFModelRow */
  retCode = ObitTableWrite ((ObitTable*)in, iOTFModelRow, NULL,  err);
  if (err->error) 
    Obit_traceback_val (err, routine,in->name, retCode);

  return retCode;
} /*  end ObitTableOTFModelWriteRow */
...
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gchar *routine = "ObitTableOTFModelClose";
  ...
  if (in->myDesc->access != OBIT_IO_ReadOnly) 
    ObitTableOTFModelDumpKey (in, err);
  if (err->error) 
    Obit_traceback_val (err, routine, in->name, retCode);
  ...
  return retCode;
} /* end ObitTableOTFModelClose */

/*---------------Private functions--------------------------*/
/*----------------  ObitTableOTFModel Row  ----------------------*/
/**
 * Creates empty member objects, initialize reference count.
 * Parent classes portions are (recursively) initialized first
 * \param inn Pointer to the object to initialize.
 */
void ObitTableOTFModelRowInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableOTFModelRow *in = inn;
 ...
 *           Actually it should be an ObitTableOTFModelRow* cast to an Obit*.
 */
void ObitTableOTFModelRowClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableOTFModelRow *in = inn;
...
void ObitTableOTFModelRowClassInit (void)
{
  ...
  ObitTableOTFModelRowClassInfoDefFn ((gpointer)&myRowClassInfo);
 
  myRowClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTableOTFModelRowClassInit */
...
static void ObitTableOTFModelRowClassInfoDefFn (gpointer inClass)
{
  ObitTableOTFModelRowClassInfo *theClass = (ObitTableOTFModelRowClassInfo*)inClass;
  ...
  theClass->ObitClassInit = (ObitClassInitFP)ObitTableOTFModelRowClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTableOTFModelRowClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTableOTFModelRowGetClass;
  theClass->newObit         = NULL;
  theClass->newObitTableRow = (newObitTableRowFP)newObitTableOTFModelRow;
  theClass->ObitCopy        = NULL;
  theClass->ObitClone       = NULL;
  theClass->ObitClear       = (ObitClearFP)ObitTableOTFModelRowClear;
  theClass->ObitInit        = (ObitInitFP)ObitTableOTFModelRowInit;

} /* end ObitTableOTFModelRowClassDefFn */
 ...
 * \param inn Pointer to the object to initialize.
 */
void ObitTableOTFModelInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableOTFModel *in = inn;
 ...
 * For some reason this wasn't build into the GType class.
 * \param  inn Pointer to the object to deallocate.
 *           Actually it should be an ObitTableOTFModel* cast to an Obit*.
 */
void ObitTableOTFModelClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableOTFModel *in = inn;
...
void ObitTableOTFModelClassInit (void)
{
  ...
  /* Set function pointers */
  ObitTableOTFModelClassInfoDefFn ((gpointer)&myClassInfo);
 
  myClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTableOTFModelClassInit */
...
static void ObitTableOTFModelClassInfoDefFn (gpointer inClass)
{
  ObitTableOTFModelClassInfo *theClass = (ObitTableOTFModelClassInfo*)inClass;
  ...
  /* function pointers defined or overloaded this class */
  theClass->ObitClassInit = (ObitClassInitFP)ObitTableOTFModelClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTableOTFModelClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTableOTFModelGetClass;
  theClass->newObit       = (newObitFP)newObitTableOTFModel;
  theClass->ObitCopy      = (ObitCopyFP)ObitTableOTFModelCopy;
  theClass->ObitClone     = (ObitCloneFP)ObitTableClone;
  theClass->ObitClear     = (ObitClearFP)ObitTableOTFModelClear;
  theClass->ObitInit      = (ObitInitFP)ObitTableOTFModelInit;
  theClass->ObitTableConvert = (ObitTableConvertFP)ObitTableOTFModelConvert;
  theClass->ObitTableOpen    = (ObitTableOpenFP)ObitTableOTFModelOpen;
  theClass->ObitTableClose   = (ObitTableCloseFP)ObitTableOTFModelClose;
  ...
  theClass->ObitTableReadRow = 
    (ObitTableReadRowFP)ObitTableOTFModelReadRow;
  theClass->ObitTableWriteRow = 
    (ObitTableWriteRowFP)ObitTableOTFModelWriteRow;

} /* end ObitTableOTFModelClassDefFn */
...
static void ObitTableOTFModelUpdate (ObitTableOTFModel *in, ObitErr *err)
{
 ...
 /* error checks */
  ...
  /* Get Keywords */
   /* numParm */
  in->numParm = 0; 
  ObitInfoListGetTest(in->myDesc->info, "NO_PARM", &type, (gint32*)&dim, 
		       (gpointer)&in->numParm);
  ...
  if (desc->FieldName) {
    for (i=0; i<desc->nfield; i++) {
      if (!strncmp (desc->FieldName[i], "X       ", 8)) {
	 in->XOff = desc->offset[i];
 	 in->XCol = i;
      }
      if (!strncmp (desc->FieldName[i], "Y       ", 8)) {
	 in->YOff = desc->offset[i];
 	 in->YCol = i;
      }
	 ...
	 in->FluxOff = desc->offset[i];
      ...
      if (!strncmp (desc->FieldName[i], "TYPE    ", 8)) {
	 in->TypeOff = desc->offset[i];
 	 in->TypeCol = i;
      }
  ...
  Obit_return_if_fail((in->XOff > -1), err,
       "ObitTableOTFModelUpdate: Could not find column X");
  Obit_return_if_fail((in->YOff > -1), err,
       "ObitTableOTFModelUpdate: Could not find column Y");
  Obit_return_if_fail((in->FluxOff > -1), err,
       "ObitTableOTFModelUpdate: Could not find column Flux");
  Obit_return_if_fail((in->TypeOff > -1), err,
       "ObitTableOTFModelUpdate: Could not find column Type");
} /* end ObitTableOTFModelUpdate */
...
static void ObitTableOTFModelDumpKey (ObitTableOTFModel *in, ObitErr *err)
{
 ...
 /* error checks */
  ...
  type  = OBIT_oint;
  dim[0] = 1;
  ObitInfoListAlwaysPut(info, "NO_PARM", type, (gint32*)&dim, 
		  (gpointer)&in->numParm);
   
} /* end ObitTableOTFModelDumpKey */
