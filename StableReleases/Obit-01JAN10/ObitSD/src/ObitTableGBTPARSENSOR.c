/* $Id: ObitTableGBTPARSENSOR.c 69 2009-01-21 16:00:01Z bill.cotton $  */
...
/**
 * \file ObitTableGBTPARSENSOR.c
 * ObitTableGBTPARSENSOR class function definitions.
 * This class is derived from the ObitTable class.
 */

/** name of the class defined in this file */
static gchar *myClassName = "ObitTableGBTPARSENSOR";
...
static gchar *myRowClassName = "ObitTableGBTPARSENSORRow";
 ...
 */
static ObitTableGBTPARSENSORRowClassInfo myRowClassInfo = {FALSE};

/*------------------  Table  ------------------------*/
/**
 * ClassInfo structure ObitTableGBTPARSENSORClassInfo.
 * This structure is used by class objects to access class functions.
 */
static ObitTableGBTPARSENSORClassInfo myClassInfo = {FALSE};


/*---------------Private function prototypes----------------*/
/** Private: Initialize newly instantiated Row object. */
void  ObitTableGBTPARSENSORRowInit  (gpointer in);

/** Private: Deallocate Row members. */
void  ObitTableGBTPARSENSORRowClear (gpointer in);

/** Private: Initialize newly instantiated object. */
void  ObitTableGBTPARSENSORInit  (gpointer in);

/** Private: Deallocate members. */
void  ObitTableGBTPARSENSORClear (gpointer in);

/** Private: update table specific info */
static void ObitTableGBTPARSENSORUpdate (ObitTableGBTPARSENSOR *in, ObitErr *err);

/** Private: copy table keywords to descriptor info list */
static void ObitTableGBTPARSENSORDumpKey (ObitTableGBTPARSENSOR *in, ObitErr *err);

/** Private: Set Class function pointers */
static void ObitTableGBTPARSENSORClassInfoDefFn (gpointer inClass);

/** Private: Set Row Class function pointers */
static void ObitTableGBTPARSENSORRowClassInfoDefFn (gpointer inClass);
/*----------------------Public functions---------------------------*/
...
ObitTableGBTPARSENSORRow* newObitTableGBTPARSENSORRow (ObitTableGBTPARSENSOR *table)
{
  ...
  if (!myRowClassInfo.initialized) ObitTableGBTPARSENSORRowClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTableGBTPARSENSORRow));

  /* initialize values */
  out->name = g_strdup("TableObitTableGBTPARSENSOR Row");

  /* set ClassInfo */
  out->ClassInfo = (gpointer)&myRowClassInfo;

  /* initialize other stuff */
  ObitTableGBTPARSENSORRowInit((gpointer)out);
...
} /* end ObitTableGBTPARSENSORRowGetClass */
...
{
  ObitTableGBTPARSENSOR* out;

  /* Class initialization if needed */
  if (!myClassInfo.initialized) ObitTableGBTPARSENSORClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTableGBTPARSENSOR));
  ...
  /* initialize other stuff */
  ObitTableGBTPARSENSORInit((gpointer)out);

 return out;
} /* end newObitTableGBTPARSENSOR */
...
} /* end ObitTableGBTPARSENSORGetClass */
 ...
 */
ObitTableGBTPARSENSOR* newObitTableGBTPARSENSORValue (gchar* name, ObitData *file, olong *ver,
 				  ObitIOAccess access,
  		    
		     ObitErr *err)
{
  ObitTableGBTPARSENSOR* out=NULL;
  ...
  ObitIOCode retCode;
  gchar *tabType = "Sensor";
  gchar *routine = "newObitTableGBTPARSENSORValue";
 ...
 /* error checks */
  ...
  if (*ver>0) { /* has to be specified */
    ...
    if (testTab!=NULL) { /* it exists, use it if is an ObitTableGBTPARSENSOR */
      if (ObitTableGBTPARSENSORIsA(testTab)) { /* it is an ObitTableGBTPARSENSOR */
	out = ObitTableRef(testTab);
      } else { /* needs conversion */
 	out = ObitTableGBTPARSENSORConvert(testTab);
  ...
  if (access==OBIT_IO_ReadOnly) { 
    highVer = ObitTableListGetHigh (list, "Sensor");
  ...
  if (ObitTableGBTPARSENSORIsA(testTab)) { 
    out = ObitTableRef(testTab);
  } else { /* needs conversion */
    out = ObitTableGBTPARSENSORConvert(testTab);
  }
 ...
 return out;
} /* end newObitTableGBTPARSENSORValue */

/**
 * Convert an ObitTable to an ObitTableGBTPARSENSOR.
...
ObitTableGBTPARSENSOR* ObitTableGBTPARSENSORConvert (ObitTable* in)
{
  ...
  /* create basic object */
  out = newObitTableGBTPARSENSOR(in->name);
 ...
 out->myHost  = in->myHost;
  ...
  return out;
} /* end ObitTableGBTPARSENSORConvert */
 ...
 * \return pointer to the new object.
 */
ObitTableGBTPARSENSOR* ObitTableGBTPARSENSORCopy (ObitTableGBTPARSENSOR *in, ObitTableGBTPARSENSOR *out, ObitErr *err)
{
  gchar *routine = "ObitTableGBTPARSENSORCopy";
 ...
 /* error checks */
  ...
  out = (ObitTableGBTPARSENSOR*)ObitTableCopy ((ObitTable*)in, (ObitTable*)out, err);
  if (err->error) /* add traceback,return */
    Obit_traceback_val (err, routine,in->name, out);

  /* Copy this class  info */
  /* Update class specific info */
  ObitTableGBTPARSENSORUpdate (out, err);
    
  return out;
} /* end ObitTableGBTPARSENSORCopy */
			  ...
			  ObitErr *err)
{
  ...
  olong nRowPIO;
  gchar *routine = "ObitTableGBTPARSENSOROpen";
  ...
  g_assert (ObitIsA(in, &myClassInfo));
   ...
   /* Update class specific info */
   ObitTableGBTPARSENSORUpdate (in, err);
   
   return retCode;
} /* end ObitTableGBTPARSENSOROpen */
 ...
 * \param iSensorRow   Row number, -1 -> next
		     ...
		     ObitErr *err)
{
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTableGBTPARSENSORReadRow";
  ...
  if (in->myStatus == OBIT_Inactive) {
    Obit_log_error(err, OBIT_Error,
		   "ObitTableGBTPARSENSOR Table is inactive for  %s ", in->name);
    return retCode;
 }

  /* read row iSensorRow */
  retCode = ObitTableRead ((ObitTable*)in, iSensorRow, NULL,  err);
  if (err->error) 
    Obit_traceback_val (err, routine, in->name, retCode);
  ...
  row->row = iRow[in->rowOff];
  row->col = iRow[in->colOff];
  row->status = iRow[in->myDesc->statusOff];

  return retCode;
} /*  end ObitTableGBTPARSENSORReadRow */
		     ...
		     ObitErr *err)
{
  ...
  if (in->myStatus == OBIT_Inactive) {
    Obit_log_error(err, OBIT_Error,
		   "Sensor Table is inactive for  %s ", in->name);
    return;
 }
...
} /*  end ObitTableGBTPARSENSORSetRow */
 ...
 * correct table buffer locations using ObitTableGBTPARSENSORSetRow  
 * \param in       Table to read
 * \param iSensorRow   Row number, -1 -> next
		      ...
		      ObitErr *err)
{
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTableGBTPARSENSORWriteRow";
  ...
  iRow[in->rowOff] = row->row;
  iRow[in->colOff] = row->col;
  ...
  /* Write row iSensorRow */
  retCode = ObitTableWrite ((ObitTable*)in, iSensorRow, NULL,  err);
  if (err->error) 
    Obit_traceback_val (err, routine,in->name, retCode);

  return retCode;
} /*  end ObitTableGBTPARSENSORWriteRow */
...
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gchar *routine = "ObitTableGBTPARSENSORClose";
  ...
  if (in->myDesc->access != OBIT_IO_ReadOnly) 
    ObitTableGBTPARSENSORDumpKey (in, err);
  if (err->error) 
    Obit_traceback_val (err, routine, in->name, retCode);
  ...
  return retCode;
} /* end ObitTableGBTPARSENSORClose */

/*---------------Private functions--------------------------*/
/*----------------  ObitTableGBTPARSENSOR Row  ----------------------*/
/**
 * Creates empty member objects, initialize reference count.
 * Parent classes portions are (recursively) initialized first
 * \param inn Pointer to the object to initialize.
 */
void ObitTableGBTPARSENSORRowInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableGBTPARSENSORRow *in = inn;
...
} /* end ObitTableGBTPARSENSORRowInit */
 ...
 *           Actually it should be an ObitTableGBTPARSENSORRow* cast to an Obit*.
 */
void ObitTableGBTPARSENSORRowClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableGBTPARSENSORRow *in = inn;
...
} /* end ObitTableGBTPARSENSORRowClear */
...
void ObitTableGBTPARSENSORRowClassInit (void)
{
  ...
  ObitTableGBTPARSENSORRowClassInfoDefFn ((gpointer)&myRowClassInfo);
 
  myRowClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTableGBTPARSENSORRowClassInit */
...
static void ObitTableGBTPARSENSORRowClassInfoDefFn (gpointer inClass)
{
  ObitTableGBTPARSENSORRowClassInfo *theClass = (ObitTableGBTPARSENSORRowClassInfo*)inClass;
  ...
  theClass->ObitClassInit = (ObitClassInitFP)ObitTableGBTPARSENSORRowClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTableGBTPARSENSORRowClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTableGBTPARSENSORRowGetClass;
  theClass->newObit         = NULL;
  theClass->newObitTableRow = (newObitTableRowFP)newObitTableGBTPARSENSORRow;
  theClass->ObitCopy        = NULL;
  theClass->ObitClone       = NULL;
  theClass->ObitClear       = (ObitClearFP)ObitTableGBTPARSENSORRowClear;
  theClass->ObitInit        = (ObitInitFP)ObitTableGBTPARSENSORRowInit;

} /* end ObitTableGBTPARSENSORRowClassDefFn */
 ...
 * \param inn Pointer to the object to initialize.
 */
void ObitTableGBTPARSENSORInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableGBTPARSENSOR *in = inn;
 ...
 * For some reason this wasn't build into the GType class.
 * \param  inn Pointer to the object to deallocate.
 *           Actually it should be an ObitTableGBTPARSENSOR* cast to an Obit*.
 */
void ObitTableGBTPARSENSORClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableGBTPARSENSOR *in = inn;
...
} /* end ObitTableGBTPARSENSORClear */
...
void ObitTableGBTPARSENSORClassInit (void)
{
  ...
  /* Set function pointers */
  ObitTableGBTPARSENSORClassInfoDefFn ((gpointer)&myClassInfo);
 
  myClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTableGBTPARSENSORClassInit */
...
static void ObitTableGBTPARSENSORClassInfoDefFn (gpointer inClass)
{
  ObitTableGBTPARSENSORClassInfo *theClass = (ObitTableGBTPARSENSORClassInfo*)inClass;
  ...
  /* function pointers defined or overloaded this class */
  theClass->ObitClassInit = (ObitClassInitFP)ObitTableGBTPARSENSORClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTableGBTPARSENSORClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTableGBTPARSENSORGetClass;
  theClass->newObit       = (newObitFP)newObitTableGBTPARSENSOR;
  theClass->ObitCopy      = (ObitCopyFP)ObitTableGBTPARSENSORCopy;
  theClass->ObitClone     = (ObitCloneFP)ObitTableClone;
  theClass->ObitClear     = (ObitClearFP)ObitTableGBTPARSENSORClear;
  theClass->ObitInit      = (ObitInitFP)ObitTableGBTPARSENSORInit;
  theClass->ObitTableConvert = (ObitTableConvertFP)ObitTableGBTPARSENSORConvert;
  theClass->ObitTableOpen    = (ObitTableOpenFP)ObitTableGBTPARSENSOROpen;
  theClass->ObitTableClose   = (ObitTableCloseFP)ObitTableGBTPARSENSORClose;
  ...
  theClass->ObitTableReadRow = 
    (ObitTableReadRowFP)ObitTableGBTPARSENSORReadRow;
  theClass->ObitTableWriteRow = 
    (ObitTableWriteRowFP)ObitTableGBTPARSENSORWriteRow;

} /* end ObitTableGBTPARSENSORClassDefFn */
 ...
 /* error checks */
  ...
  if (desc->FieldName) {
    for (i=0; i<desc->nfield; i++) {
      if (!strncmp (desc->FieldName[i], "Row", 3)) {
	 in->rowOff = desc->offset[i];
 	 in->rowCol = i;
      }
      if (!strncmp (desc->FieldName[i], "Col", 3)) {
	 in->colOff = desc->offset[i];
 	 in->colCol = i;
      }
     }
  }

  Obit_return_if_fail((in->rowOff > -1), err,
       "ObitTableSensorUpdate: Could not find column row");
} /* end ObitTableGBTPARSENSORUpdate */
...
} /* end ObitTableGBTPARSENSORDumpKey */
