/* $Id: ObitTableGBTPARDATA.h 69 2009-01-21 16:00:01Z bill.cotton $  */
...
/*--------------------------------------------------------------------*/
#ifndef ObitTableGBTPARDATA_H 
#define ObitTableGBTPARDATA_H 
...
/**
 * \file ObitTableGBTPARDATA.h
 * ObitTableGBTPARDATA class definition.
 *
 ...
 * This class contains tabular data and allows access.
 * ObitTableGBTPARDATA" 
 * An ObitTableGBTPARDATA is the front end to a persistent disk resident structure.
 * This class is derived from the ObitTable class.
 *
 ...
 *
 * \section ObitTableGBTPARDATASpecification Specifying desired data transfer parameters
 ...
 * The convenience Macro #ObitTableGBTPARDATASetFITS simplifies specifying the 
 * desired data.
 ...
 *
 *
 * \section ObitTableGBTPARDATAaccess Creators and Destructors
 * An ObitTableGBTPARDATA can be created using newObitTableGBTPARDATAValue which attaches the 
 * table to an ObitData for the object.  
 * If the output ObitTableGBTPARDATA has previously been specified, including file information,
 * then ObitTableGBTPARDATACopy will copy the disk resident as well as the memory 
 * resident information.
 *
 * A copy of a pointer to an ObitTableGBTPARDATA should always be made using the
 * ObitTableGBTPARDATARef function which updates the reference count in the object.
 * Then whenever freeing an ObitTableGBTPARDATA or changing a pointer, the function
 * ObitTableGBTPARDATAUnref will decrement the reference count and destroy the object
 * when the reference count hits 0.
 *
 * \section ObitTableGBTPARDATAUsage I/O
 * Visibility data is available after an input object is "Opened"
 * and "Read".
 * I/O optionally uses a buffer attached to the ObitTableGBTPARDATA or some external
 * location.
 * To Write an ObitTableGBTPARDATA, create it, open it, and write.
 * The object should be closed to ensure all data is flushed to disk.
 * Deletion of an ObitTableGBTPARDATA after its final unreferencing will automatically
 * close it.
 */
...
typedef struct {
#include "ObitTableGBTPARDATADef.h"   /* this class definition */
} ObitTableGBTPARDATA;

/** ObitTableGBTPARDATARow Class structure. */
typedef struct {
#include "ObitTableGBTPARDATARowDef.h"   /* this class row definition */
} ObitTableGBTPARDATARow;

/*----------------- Macroes ---------------------------*/
/** 
 * Macro to unreference (and possibly destroy) an ObitTableGBTPARDATA
 * returns an ObitTableGBTPARDATA*.
 * in = object to unreference
 */
#define ObitTableGBTPARDATAUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableGBTPARDATA.
 * returns an ObitTableGBTPARDATA*.
 * in = object to reference
 */
#define ObitTableGBTPARDATARef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableGBTPARDATAIsA(in) ObitIsA (in, ObitTableGBTPARDATAGetClass())

/** 
 * Macro to unreference (and possibly destroy) an ObitTableGBTPARDATARow
 * returns an ObitTableGBTPARDATARow*.
 * in = object to unreference
 */
#define ObitTableGBTPARDATARowUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableGBTPARDATARow.
 * returns an ObitTableGBTPARDATARow*.
 * in = object to reference
 */
#define ObitTableGBTPARDATARowRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableGBTPARDATARowIsA(in) ObitIsA (in, ObitTableGBTPARDATARowGetClass())
...
ObitTableGBTPARDATA* 
newObitTableGBTPARDATAValue (gchar* name, ObitData *file, olong *ver,
  		     ObitIOAccess access,
...
ObitTableGBTPARDATA* ObitTableGBTPARDATACopy  (ObitTableGBTPARDATA *in, ObitTableGBTPARDATA *out, 
			   ObitErr *err);

/** Public: Copy (shallow) constructor. */
ObitTableGBTPARDATA* ObitTableGBTPARDATAClone (ObitTableGBTPARDATA *in, ObitTableGBTPARDATA *out);
 ...
 */
typedef struct  {
#include "ObitTableGBTPARDATAClassDef.h"
} ObitTableGBTPARDATAClassInfo; 

/**
 * ClassInfo Structure For TableObitTableGBTPARDATARow.
 * Contains class name, a pointer to any parent class
 * (NULL if none) and function pointers.
 */
typedef struct  {
#include "ObitTableGBTPARDATARowClassDef.h"
} ObitTableGBTPARDATARowClassInfo; 
#endif /* ObitTableGBTPARDATA_H */ 
