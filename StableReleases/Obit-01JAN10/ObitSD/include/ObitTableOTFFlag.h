/* $Id: ObitTableOTFFlag.h 69 2009-01-21 16:00:01Z bill.cotton $  */
...
/*--------------------------------------------------------------------*/
#ifndef ObitTableOTFFlag_H 
#define ObitTableOTFFlag_H 
...
/**
 * \file ObitTableOTFFlag.h
 * ObitTableOTFFlag class definition.
 *
 ...
 *
 * \section ObitTableOTFFlagSpecification Specifying desired data transfer parameters
 ...
 * The convenience Macro #ObitTableOTFFlagSetFITS simplifies specifying the 
 * desired data.
 ...
 *
 *
 * \section ObitTableOTFFlagaccess Creators and Destructors
 * An ObitTableOTFFlag can be created using newObitTableOTFFlagValue which attaches the 
 * table to an ObitData for the object.  
 * If the output ObitTableOTFFlag has previously been specified, including file information,
 * then ObitTableOTFFlagCopy will copy the disk resident as well as the memory 
 * resident information.
 *
 * A copy of a pointer to an ObitTableOTFFlag should always be made using the
 * ObitTableOTFFlagRef function which updates the reference count in the object.
 * Then whenever freeing an ObitTableOTFFlag or changing a pointer, the function
 * ObitTableOTFFlagUnref will decrement the reference count and destroy the object
 * when the reference count hits 0.
 *
 * \section ObitTableOTFFlagUsage I/O
 * Visibility data is available after an input object is "Opened"
 * and "Read".
 * I/O optionally uses a buffer attached to the ObitTableOTFFlag or some external
 * location.
 * To Write an ObitTableOTFFlag, create it, open it, and write.
 * The object should be closed to ensure all data is flushed to disk.
 * Deletion of an ObitTableOTFFlag after its final unreferencing will automatically
 * close it.
 */
...
/** Number of characters for Table keyword */
 #define MAXKEYCHARTABLEOTFFlag 24

/** ObitTableOTFFlag Class structure. */
typedef struct {
#include "ObitTableOTFFlagDef.h"   /* this class definition */
} ObitTableOTFFlag;

/** ObitTableOTFFlagRow Class structure. */
typedef struct {
#include "ObitTableOTFFlagRowDef.h"   /* this class row definition */
} ObitTableOTFFlagRow;

/*----------------- Macroes ---------------------------*/
/** 
 * Macro to unreference (and possibly destroy) an ObitTableOTFFlag
 * returns an ObitTableOTFFlag*.
 * in = object to unreference
 */
#define ObitTableOTFFlagUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableOTFFlag.
 * returns an ObitTableOTFFlag*.
 * in = object to reference
 */
#define ObitTableOTFFlagRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableOTFFlagIsA(in) ObitIsA (in, ObitTableOTFFlagGetClass())

/** 
 * Macro to unreference (and possibly destroy) an ObitTableOTFFlagRow
 * returns an ObitTableOTFFlagRow*.
 * in = object to unreference
 */
#define ObitTableOTFFlagRowUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitTableOTFFlagRow.
 * returns an ObitTableOTFFlagRow*.
 * in = object to reference
 */
#define ObitTableOTFFlagRowRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitTableOTFFlagRowIsA(in) ObitIsA (in, ObitTableOTFFlagRowGetClass())
...
ObitTableOTFFlag* 
newObitTableOTFFlagValue (gchar* name, ObitData *file, olong *ver,
  		     ObitIOAccess access,
...
ObitTableOTFFlag* ObitTableOTFFlagCopy  (ObitTableOTFFlag *in, ObitTableOTFFlag *out, 
			   ObitErr *err);

/** Public: Copy (shallow) constructor. */
ObitTableOTFFlag* ObitTableOTFFlagClone (ObitTableOTFFlag *in, ObitTableOTFFlag *out);

/** Public: Convert an ObitTable to an ObitTableOTFFlag */
 ...
 */
typedef struct  {
#include "ObitTableOTFFlagClassDef.h"
} ObitTableOTFFlagClassInfo; 

/**
 * ClassInfo Structure For TableObitTableOTFFlagRow.
 * Contains class name, a pointer to any parent class
 * (NULL if none) and function pointers.
 */
typedef struct  {
#include "ObitTableOTFFlagRowClassDef.h"
} ObitTableOTFFlagRowClassInfo; 
#endif /* ObitTableOTFFlag_H */ 
