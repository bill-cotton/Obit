/* To do:
  ...
  2) add iloc? type array values to the OTFDesc structure
     and be sure they are always updated.
  3) Add selection by scan(?) or target(s) (timerange),
  6) Generic Obit read/Write FITS keywords - replace explicit cfitsio
  7) History
*/
/* $Id: DCR2OTF.c 2 2008-06-10 15:32:27Z bill.cotton $                            */
...
/* internal prototypes */
/* Get inputs */
ObitInfoList* DCR2OTFin (int argc, char **argv, ObitErr *err);
...
/*----------------------------------------------------------------------- */
{
  ...
  ierr = 0;
  myInput = DCR2OTFin (argc, argv, err);
  if (err->error) ierr = 1;
  ...
  timeCorr = 0.0;
  ObitInfoListGet(myInput, "timeCorr", &type, dim, &timeCorr, err);
  /* debug fprintf (stderr,"timeCorr = %lf\n",timeCorr); */

  /* Initialize Obit */
  FITSdir[1] =  DataRoot;  /* Input FITS data directory */
  mySystem = ObitSystemStartup ("DCR2OTF", 1, 0, 0, NULL, 2, FITSdir, (oint)TRUE, (oint)FALSE, err);
  if (err->error) ierr = 1;
  ObitErrLog(err); /* show any error messages on err */
  if (ierr!=0) return ierr;
  ...
  /* Make sure Name blank filled to 16 char */
  for (i=strlen(Name); i<16; i++) Name[i] = ' '; Name[i]=0;
...
/*----------------------------------------------------------------------- */
{
  olong ax;
  gint32 dim[MAXINFOELEMDIM] = {1,1,1,1,1};
  gchar *input_file="DCR2OTF.in", *arg;
...
/*----------------------------------------------------------------------- */
{
    fprintf(stderr, "Usage: DCR2OTF -input file [-scan date/time]\n");
    fprintf(stderr, "Convert an GBT DCR file format to Obit/OTF\n");
    fprintf(stderr, "Arguments:\n");
    fprintf(stderr, "  -input input parameter file, def DCR2OTF.in\n");
...
ObitInfoList* defaultInputs(gchar *scan_name, ObitErr *err)
{
  ...
  dim[0] = 1;
  dtemp = 0.0;
  ObitInfoListPut (out, "timeCorr", OBIT_double, dim, &dtemp, err);

  /* Don't think the rest are used: */
...
/*----------------------------------------------------------------------- */
{
  ...
  /* get full file name */
  sprintf (FullFile,"%sDCR/%s.fits", DataRoot, infile);
