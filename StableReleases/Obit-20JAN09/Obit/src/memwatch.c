...
char *mwStrndup( const char* str, int num, const char* file, int line ) {
    size_t len;
    char *newstring;
    ...
    if( str == NULL ) {
        mwIncErr();
        mwWrite( "strndup: <%ld> %s(%d), strdnup(NULL) called\n",
            mwCounter, file, line );
        FLUSH();
		MW_MUTEX_UNLOCK();
        return NULL;
...
/* Note: uses variable length argument list */
char *mwStrconcat(const char* file, int line, ... ) {
  size_t len, ll, lx;
  char *newstring, *str, **cargs;
  va_list args;
...
void* mwFreeNULL( void* p, const char* file, int line ) {
  mwFree(p,file,line);
  return NULL;
}
