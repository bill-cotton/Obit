...
/**
 * \file ObitTableGBTVEGASSTATE.c
 * ObitTableGBTVEGASSTATE class function definitions.
 * This class is derived from the ObitTable class.
 */

/** name of the class defined in this file */
static gchar *myClassName = "ObitTableGBTVEGASSTATE";
...
static gchar *myRowClassName = "ObitTableGBTVEGASSTATERow";
 ...
 */
static ObitTableGBTVEGASSTATERowClassInfo myRowClassInfo = {FALSE};

/*------------------  Table  ------------------------*/
/**
 * ClassInfo structure ObitTableGBTVEGASSTATEClassInfo.
 * This structure is used by class objects to access class functions.
 */
static ObitTableGBTVEGASSTATEClassInfo myClassInfo = {FALSE};


/*---------------Private function prototypes----------------*/
/** Private: Initialize newly instantiated Row object. */
void  ObitTableGBTVEGASSTATERowInit  (gpointer in);

/** Private: Deallocate Row members. */
void  ObitTableGBTVEGASSTATERowClear (gpointer in);

/** Private: Initialize newly instantiated object. */
void  ObitTableGBTVEGASSTATEInit  (gpointer in);

/** Private: Deallocate members. */
void  ObitTableGBTVEGASSTATEClear (gpointer in);

/** Private: update table specific info */
static void ObitTableGBTVEGASSTATEUpdate (ObitTableGBTVEGASSTATE *in, ObitErr *err);

/** Private: copy table keywords to descriptor info list */
static void ObitTableGBTVEGASSTATEDumpKey (ObitTableGBTVEGASSTATE *in, ObitErr *err);

/** Private: Set Class function pointers */
static void ObitTableGBTVEGASSTATEClassInfoDefFn (gpointer inClass);

/** Private: Set Row Class function pointers */
static void ObitTableGBTVEGASSTATERowClassInfoDefFn (gpointer inClass);
/*----------------------Public functions---------------------------*/
...
ObitTableGBTVEGASSTATERow* newObitTableGBTVEGASSTATERow (ObitTableGBTVEGASSTATE *table)
{
  ...
  if (!myRowClassInfo.initialized) ObitTableGBTVEGASSTATERowClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTableGBTVEGASSTATERow));

  /* initialize values */
  out->name = g_strdup("TableObitTableGBTVEGASSTATE Row");

  /* set ClassInfo */
  out->ClassInfo = (gpointer)&myRowClassInfo;

  /* initialize other stuff */
  ObitTableGBTVEGASSTATERowInit((gpointer)out);
...
} /* end ObitTableGBTVEGASSTATERowGetClass */
...
{
  ObitTableGBTVEGASSTATE* out;

  /* Class initialization if needed */
  if (!myClassInfo.initialized) ObitTableGBTVEGASSTATEClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTableGBTVEGASSTATE));
  ...
  /* initialize other stuff */
  ObitTableGBTVEGASSTATEInit((gpointer)out);

 return out;
} /* end newObitTableGBTVEGASSTATE */
...
} /* end ObitTableGBTVEGASSTATEGetClass */
 ...
 */
ObitTableGBTVEGASSTATE* newObitTableGBTVEGASSTATEValue (gchar* name, ObitData *file, olong *ver,
 				  ObitIOAccess access,
  		    
		     ObitErr *err)
{
  ObitTableGBTVEGASSTATE* out=NULL;
  ...
  ObitIOCode retCode;
  gchar *tabType = "STATE";
  gchar *routine = "newObitTableGBTVEGASSTATEValue";
 ...
 /* error checks */
  ...
  if (*ver>0) { /* has to be specified */
    ...
    if (testTab!=NULL) { /* it exists, use it if is an ObitTableGBTVEGASSTATE */
      if (ObitTableGBTVEGASSTATEIsA(testTab)) { /* it is an ObitTableGBTVEGASSTATE */
	out = ObitTableRef(testTab);
      } else { /* needs conversion */
 	out = ObitTableGBTVEGASSTATEConvert(testTab);
  ...
  if (ObitTableGBTVEGASSTATEIsA(testTab)) { 
    out = ObitTableRef(testTab);
  } else { /* needs conversion */
    out = ObitTableGBTVEGASSTATEConvert(testTab);
  }
  ...
  colNo++;
  desc->FieldName[colNo] = g_strdup("PHSESTRT");
  desc->FieldUnit[colNo] = g_strdup("");
  desc->type[colNo] = OBIT_double;
  for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
  colNo++;
  desc->FieldName[colNo] = g_strdup("SIGREF  ");
  desc->FieldUnit[colNo] = g_strdup("T/F");
 ...
 return out;
} /* end newObitTableGBTVEGASSTATEValue */

/**
 * Convert an ObitTable to an ObitTableGBTVEGASSTATE.
...
ObitTableGBTVEGASSTATE* ObitTableGBTVEGASSTATEConvert (ObitTable* in)
{
  ...
  /* create basic object */
  out = newObitTableGBTVEGASSTATE(in->name);
 ...
 out->myHost  = in->myHost;
  ...
  return out;
} /* end ObitTableGBTVEGASSTATEConvert */
 ...
 * \return pointer to the new object.
 */
ObitTableGBTVEGASSTATE* ObitTableGBTVEGASSTATECopy (ObitTableGBTVEGASSTATE *in, ObitTableGBTVEGASSTATE *out, ObitErr *err)
{
  gchar *routine = "ObitTableGBTVEGASSTATECopy";
 ...
 /* error checks */
  ...
  out = (ObitTableGBTVEGASSTATE*)ObitTableCopy ((ObitTable*)in, (ObitTable*)out, err);
  if (err->error) /* add traceback,return */
    Obit_traceback_val (err, routine,in->name, out);

  /* Copy this class  info */
  out->numphase = in->numphase;
  out->swperiod = in->swperiod;
  strncpy (out->master, in->master, MAXKEYCHARTABLESTATE );
  /* Update class specific info */
  ObitTableGBTVEGASSTATEUpdate (out, err);
    
  return out;
} /* end ObitTableGBTVEGASSTATECopy */
			  ...
			  ObitErr *err)
{
  ...
  olong nRowPIO;
  gchar *routine = "ObitTableGBTVEGASSTATEOpen";
  ...
  g_assert (ObitIsA(in, &myClassInfo));
   ...
   /* Update class specific info */
   ObitTableGBTVEGASSTATEUpdate (in, err);
   
   return retCode;
} /* end ObitTableGBTVEGASSTATEOpen */
		     ...
		     ObitErr *err)
{
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTableGBTVEGASSTATEReadRow";
  ...
  if (in->myStatus == OBIT_Inactive) {
    Obit_log_error(err, OBIT_Error,
		   "ObitTableGBTVEGASSTATE Table is inactive for  %s ", in->name);
    return retCode;
 }
  ...
  row->phsestrt = dRow[in->phsestrtOff];
  row->sigref = iRow[in->sigrefOff];
  row->cal = iRow[in->calOff];
  row->status = iRow[in->myDesc->statusOff];

  return retCode;
} /*  end ObitTableGBTVEGASSTATEReadRow */
...
} /*  end ObitTableGBTVEGASSTATESetRow */
 ...
 * correct table buffer locations using ObitTableGBTVEGASSTATESetRow  
 * \param in       Table to read
 * \param iSTATERow   Row number, -1 -> next
		      ...
		      ObitErr *err)
{
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTableGBTVEGASSTATEWriteRow";
  ...
  dRow[in->phsestrtOff] = row->phsestrt;
  iRow[in->sigrefOff] = row->sigref;
  iRow[in->calOff] = row->cal;
  ...
  return retCode;
} /*  end ObitTableGBTVEGASSTATEWriteRow */
...
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gchar *routine = "ObitTableGBTVEGASSTATEClose";
  ...
  if (in->myDesc->access != OBIT_IO_ReadOnly) 
    ObitTableGBTVEGASSTATEDumpKey (in, err);
  if (err->error) 
    Obit_traceback_val (err, routine, in->name, retCode);
  ...
  return retCode;
} /* end ObitTableGBTVEGASSTATEClose */

/*---------------Private functions--------------------------*/
/*----------------  ObitTableGBTVEGASSTATE Row  ----------------------*/
/**
 * Creates empty member objects, initialize reference count.
 * Parent classes portions are (recursively) initialized first
 * \param inn Pointer to the object to initialize.
 */
void ObitTableGBTVEGASSTATERowInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableGBTVEGASSTATERow *in = inn;
...
} /* end ObitTableGBTVEGASSTATERowInit */
 ...
 *           Actually it should be an ObitTableGBTVEGASSTATERow* cast to an Obit*.
 */
void ObitTableGBTVEGASSTATERowClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableGBTVEGASSTATERow *in = inn;
...
} /* end ObitTableGBTVEGASSTATERowClear */
...
void ObitTableGBTVEGASSTATERowClassInit (void)
{
  ...
  ObitTableGBTVEGASSTATERowClassInfoDefFn ((gpointer)&myRowClassInfo);
 
  myRowClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTableGBTVEGASSTATERowClassInit */
...
static void ObitTableGBTVEGASSTATERowClassInfoDefFn (gpointer inClass)
{
  ObitTableGBTVEGASSTATERowClassInfo *theClass = (ObitTableGBTVEGASSTATERowClassInfo*)inClass;
  ...
  theClass->ObitClassInit = (ObitClassInitFP)ObitTableGBTVEGASSTATERowClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTableGBTVEGASSTATERowClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTableGBTVEGASSTATERowGetClass;
  theClass->newObit         = NULL;
  theClass->newObitTableRow = (newObitTableRowFP)newObitTableGBTVEGASSTATERow;
  theClass->ObitCopy        = NULL;
  theClass->ObitClone       = NULL;
  theClass->ObitClear       = (ObitClearFP)ObitTableGBTVEGASSTATERowClear;
  theClass->ObitInit        = (ObitInitFP)ObitTableGBTVEGASSTATERowInit;

} /* end ObitTableGBTVEGASSTATERowClassDefFn */
 ...
 * \param inn Pointer to the object to initialize.
 */
void ObitTableGBTVEGASSTATEInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableGBTVEGASSTATE *in = inn;
...
} /* end ObitTableGBTVEGASSTATEInit */
 ...
 * For some reason this wasn't build into the GType class.
 * \param  inn Pointer to the object to deallocate.
 *           Actually it should be an ObitTableGBTVEGASSTATE* cast to an Obit*.
 */
void ObitTableGBTVEGASSTATEClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableGBTVEGASSTATE *in = inn;
...
} /* end ObitTableGBTVEGASSTATEClear */
...
void ObitTableGBTVEGASSTATEClassInit (void)
{
  ...
  /* Set function pointers */
  ObitTableGBTVEGASSTATEClassInfoDefFn ((gpointer)&myClassInfo);
 
  myClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTableGBTVEGASSTATEClassInit */
...
static void ObitTableGBTVEGASSTATEClassInfoDefFn (gpointer inClass)
{
  ObitTableGBTVEGASSTATEClassInfo *theClass = (ObitTableGBTVEGASSTATEClassInfo*)inClass;
  ...
  /* function pointers defined or overloaded this class */
  theClass->ObitClassInit = (ObitClassInitFP)ObitTableGBTVEGASSTATEClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTableGBTVEGASSTATEClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTableGBTVEGASSTATEGetClass;
  theClass->newObit       = (newObitFP)newObitTableGBTVEGASSTATE;
  theClass->ObitCopy      = (ObitCopyFP)ObitTableGBTVEGASSTATECopy;
  theClass->ObitClone     = (ObitCloneFP)ObitTableClone;
  theClass->ObitClear     = (ObitClearFP)ObitTableGBTVEGASSTATEClear;
  theClass->ObitInit      = (ObitInitFP)ObitTableGBTVEGASSTATEInit;
  theClass->ObitTableConvert = (ObitTableConvertFP)ObitTableGBTVEGASSTATEConvert;
  theClass->ObitTableOpen    = (ObitTableOpenFP)ObitTableGBTVEGASSTATEOpen;
  theClass->ObitTableClose   = (ObitTableCloseFP)ObitTableGBTVEGASSTATEClose;
  ...
  theClass->ObitTableReadRow = 
    (ObitTableReadRowFP)ObitTableGBTVEGASSTATEReadRow;
  theClass->ObitTableWriteRow = 
    (ObitTableWriteRowFP)ObitTableGBTVEGASSTATEWriteRow;

} /* end ObitTableGBTVEGASSTATEClassDefFn */
...
static void ObitTableGBTVEGASSTATEUpdate (ObitTableGBTVEGASSTATE *in, ObitErr *err)
{
 ...
 /* error checks */
  ...
  /* Get Keywords */
   /* numphase */
  if (!ObitInfoListGet(in->myDesc->info, "NUMPHASE", &type, (gint32*)&dim, 
		       (gpointer)&in->numphase, err)) return;
   /* swperiod */
  if (!ObitInfoListGet(in->myDesc->info, "SWPERIOD", &type, (gint32*)&dim, 
		       (gpointer)&InfoReal, err)) return;
  if (type==OBIT_double) in->swperiod = InfoReal.dbl;
  if (type==OBIT_float)  in->swperiod = InfoReal.flt;
   /* master */
  if (!ObitInfoListGet(in->myDesc->info, "MASTER", &type, (gint32*)&dim, 
		       (gpointer)&in->master, err)) return;
  ...
  if (desc->FieldName) {
    for (i=0; i<desc->nfield; i++) {
	 ...
	 in->blanktimOff = desc->offset[i];
      ...
      if (!strncmp (desc->FieldName[i], "PHSESTRT", 8)) {
	 in->phsestrtOff = desc->offset[i];
 	 in->phsestrtCol = i;
      }
      if (!strncmp (desc->FieldName[i], "SIGREF  ", 8)) {
	 in->sigrefOff = desc->offset[i];
 	 in->sigrefCol = i;
      }
      if (!strncmp (desc->FieldName[i], "CAL ", 4)) {
  ...
  Obit_return_if_fail((in->phsestrtOff > -1), err,
       "ObitTableSTATEUpdate: Could not find column phsestrt");
  Obit_return_if_fail((in->sigrefOff > -1), err,
       "ObitTableSTATEUpdate: Could not find column sigref");
} /* end ObitTableGBTVEGASSTATEUpdate */
...
static void ObitTableGBTVEGASSTATEDumpKey (ObitTableGBTVEGASSTATE *in, ObitErr *err)
{
 ...
 /* error checks */
  ...
  type  = OBIT_oint;
  dim[0] = 1;
  ObitInfoListAlwaysPut(info, "NUMPHASE", type, (gint32*)&dim, 
		  (gpointer)&in->numphase);
  /* swperiod */
  type  = OBIT_float;
  dim[0] = 1;
  ObitInfoListAlwaysPut(info, "SWPERIOD", type, (gint32*)&dim, 
		  (gpointer)&in->swperiod);
  /* master */
  type  = OBIT_string;
...
} /* end ObitTableGBTVEGASSTATEDumpKey */
