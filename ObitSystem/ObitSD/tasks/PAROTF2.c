...
/* Get elevation for a given time */
...
void GetData (ObitOTF *outData, gchar *infile, ObitInfoList *myInput, 
	      ofloat avgTime, ofloat offTime, olong scanNo, 
	      gboolean doQuadD, ObitErr *err);
/* Get target id */
...
void ReadDatum (ObitTableGBTPARDATA2* PARtable, olong ndetect, gboolean *bad, 
...
/* Convert Quadrant detector measurements to offsets */
void ProcessQDData (ObitErr *err);
/* Get Quadrant detector pointing offsetfor a given time */
void GetPointOffset (odouble time, ofloat *dAz, ofloat *dEl);

/* Program globals */
gchar *pgmName = "PAROTF2";      /* Program name */
gchar *infile  = "PAROTF2.inp";  /* File with program inputs */
gchar *outfile = "PAROTF2.out";  /* File to contain program outputs */
...
odouble *AntDec;  /* Array of Antenna Dec J2000 values */
...
/*----------------------------------------------------------------------- */
{
  ...
  /* Want Quadrant detector pointing corrections? */
  doQuadD = FALSE;
  ObitInfoListGetTest(myInput, "doQuadD",  &type, dim, &doQuadD);

  if (doQuadD) {
    Obit_log_error(err, OBIT_InfoErr, "Applying Quadrant detector corrections");
    ObitErrLog(err);
    ...
    if (err->error) 
      Obit_log_error(err, OBIT_Error, 
		     "Error reading Quadrant detector file for scan %s", inscan);

    /* Convert measurements to offsets */
    ProcessQDData (err);
    if (err->error) 
      Obit_log_error(err, OBIT_Error, 
		     "Error processing Quadrant detector data for scan %s", inscan);
  } /* end Quadrant detector pointing corrections */
  ...
  /* convert data  */
  GetData (outData, inscan, myInput, avgTime, offTime, scanNo, doQuadD, err);
  if (err->error) ierr = 1;  ObitErrLog(err);  if (ierr!=0) goto exit;
...
/*----------------------------------------------------------------------- */
{
  ...
  gint32 dim[MAXINFOELEMDIM] = {1,1,1,1,1};
  ObitInfoType type;
  gchar *input_file="PAROTF2.in", *arg;
  gboolean init=FALSE;
  oint itemp;
...
/*----------------------------------------------------------------------- */
{
    fprintf(stderr, "Usage: PAROTF2 -input file [-Scan date/time]\n");
    fprintf(stderr, "Convert an 2009+ GBT PAR file format to Obit/OTF\n");
    fprintf(stderr, "Arguments:\n");
    fprintf(stderr, "  -input input parameter file, def PAROTF2.in\n");
...
/*      doQuadD  Apply Quadrant detector corrections                      */
/*   Output:                                                              */
/*       err       Obit return error stack                                */
/*----------------------------------------------------------------------- */
{
  ...
  ofloat *ATime=NULL, **AData=NULL, *ACal=NULL, refDay, corDay, parAng;
  ofloat *data, ra, dec, dAz=0.0, dEl=0.0, dRa=0.0, dDec=0.0;
  ...
  for (irow=0; irow<ntime; irow++) {
    ...
    /* Drop data without good pointing value */
    if (dec<-85.0) continue;

    /* Parallactic angle (deg) */
    data[desc->ilocrot] = ObitOTFArrayGeomParAng(geom, data[desc->iloct], ra, dec);
    parAng = DG2RAD*data[desc->ilocrot]; /* in radians */

    /* Quadrant Detector corrections */
    if (doQuadD) {
     GetPointOffset (dmjd+offTime+refDay, &dAz, &dEl);
 
     /* position offset - convert az, el offset to RA, dec 
	azOff is actually in Xel */
     if ((dAz!=fblank) && (dEl!=fblank)) {
       dRa  = dAz*cos (parAng) + dEl*sin (parAng);
       dDec = dEl*cos (parAng) - dAz*sin (parAng);
       
       /* apply correction */
       ra  += dRa;
       dec += dDec;
     }
    } /* end Quadrant Detector corrections */
    ...
    /* DEBUG - save dAz, dEl, dRA, dDec as 68,69,70,71 */
    data[desc->ilocdata+68*incdatawt]   = dAz;
    data[desc->ilocdata+68*incdatawt+1] = -1.0; /* Initial Weight */
    data[desc->ilocdata+69*incdatawt]   = dEl;
    data[desc->ilocdata+69*incdatawt+1] = -1.0; /* Initial Weight */
    data[desc->ilocdata+70*incdatawt]   = dRa;
    data[desc->ilocdata+70*incdatawt+1] = -1.0; /* Initial Weight */
    data[desc->ilocdata+71*incdatawt]   = dDec;
    data[desc->ilocdata+71*incdatawt+1] = -1.0; /* Initial Weight */
...
/*----------------------------------------------------------------------- */
{
  ...
  /* Create table structure */
  PARtable = newObitTableGBTPARDATA2("Data");
  if (err->error) return;
  ...
  /* Open */
  ObitTableGBTPARDATA2Open (PARtable, OBIT_IO_ReadOnly, err);
  ...
  *AData = g_malloc0((*ndetect)*sizeof(ofloat**));
  ...
  for (id=0; id<(*ndetect); id++) {
    ...
    /* Save first good ACal ATime */
    if (!bad[id] && (saveACal==NULL)) {
      saveACal = *ACal;
      *ACal = NULL;
      saveATime = *ATime;
      *ATime = NULL;
  ...
  *ATime = saveATime;
  ...
  /* Close */
  ObitTableGBTPARDATA2Close (PARtable, err);
  if (err->error) Obit_traceback_msg (err, routine, PARtable->name);
  
  /* Cleanup */
  PARtable = ObitTableGBTPARDATA2Unref(PARtable);
...
/*----------------------------------------------------------------------- */
{
  ...
  *cal  = g_malloc0((*ntime)*sizeof(ofloat));
  ...
  /* Create Row structure */
  row = newObitTableGBTPARDATA2Row (PARtable);

  /* Loop over table */
  for (irow=1; irow<=PARtable->myDesc->nrow; irow++) {

    /* Read row */
    retCode = ObitTableGBTPARDATA2ReadRow (PARtable, irow, row, err);
    if (err->error) Obit_traceback_msg (err, routine, PARtable->name);

    /* Save selected data */	
    (*time)[irow-1] = row->DMJD;
    ...
    if (row->DigInput[1]) {
      tErr = (row->DMJD - (olong)row->DMJD);
      tErr *= 86400.0; /* to seconds */
      tErr -= (olong)tErr;
  ...
  /* Cleanup */
  row   = ObitTableGBTPARDATA2RowUnref(row);
...
/*----------------------------------------------------------------------- */
{
  ofloat *el=NULL, *work=NULL, median, med1, med3, med4, med5;
  ...
  work  = g_malloc0(nQDetTime*sizeof(ofloat));
  ...
  /*  convert from QD units to pointing error
      qd_xz=ESM_D##C_err (use QDetdEl, QDetdAz )*/
  for (i=0; i<n; i++) {
     /* WPP   */
    ...
    QDetdAz[i] = +(ofloat)(ESM_D[0][0] * C_err[0][i] + ESM_D[1][0] * C_err[1][i]);  */
     /* XPP 
    QDetdEl[i] = +(ofloat)(ESM_D[0][0] * C_err[0][i] + ESM_D[1][0] * C_err[1][i]);
    QDetdAz[i] = -(ofloat)(ESM_D[0][1] * C_err[0][i] + ESM_D[1][1] * C_err[1][i]);  */
     /* XMP
    QDetdEl[i] = -(ofloat)(ESM_D[0][0] * C_err[0][i] + ESM_D[1][0] * C_err[1][i]);
    QDetdAz[i] = -(ofloat)(ESM_D[0][1] * C_err[0][i] + ESM_D[1][1] * C_err[1][i]);  */
     /* XMM 
  ...
  for (i=0; i<n; i++) work[i] = QDetdAz[i];
  median = medianValue(work,1,n);
  for (i=0; i<n; i++) 
    if (QDetdAz[i]!=fblank) QDetdAz[i] -= median;
  for (i=0; i<n; i++) work[i] = QDetdEl[i];
  median = medianValue(work,1,n);
  for (i=0; i<n; i++) 
  ...
  /* Data quality checks
     fog warning */
  for (i=0; i<n; i++) work[i] = QDetV1[i];
  med1 = medianValue(work,1,n);
  for (i=0; i<n; i++) work[i] = QDetV3[i];
  med3 = medianValue(work,1,n);
  for (i=0; i<n; i++) work[i] = QDetV4[i];
  med4 = medianValue(work,1,n);
  for (i=0; i<n; i++) work[i] = QDetV5[i];
  med5 = medianValue(work,1,n);
  ...
  for (i=0; i<n; i++) work[i] = QDetdAz[i];
  med1 = medianValue(work,1,n/4);
  for (i=0; i<n; i++) work[i] = QDetdAz[i];
  med3 = medianValue(&work[n-n/4],1,n/4);
  if (fabs(med1-med3) > (10.0)) 
    Obit_log_error(err, OBIT_InfoWarn, 
...
/*      time   The desired MJD                                            */
/*   Output:                                                              */
/*      dAz    Azimuth offset in degrees  (actually Xel)                  */
/*      dEl    Elevation offset in degrees                                */
/*             dAz,dEl fblank if invalid                                  */
/*----------------------------------------------------------------------- */
{
  olong i, best;
  odouble test, delta;
  ...
  *dEl  = 0.0;
  ...
  for (i=0; i<nQDetTime; i++) { /* loop over array */
    test = fabs (QDetDMJD[i]-time);
      if ((delta> test) && (QDetdAz[i]!=fblank) && (QDetdEl[i]!=fblank)) {
	delta = test;
	best = i;
      } else if ((QDetdAz[i]!=fblank) && (QDetdEl[i]!=fblank)) { 
	/* must be getting further, stop */
	break;
      }
  }
  ...
  if ((best==0) || (best==(nQDetTime-1))) {
    ...
    /* previous good? */
    if ((QDetdAz[best-1]==fblank) || (QDetdEl[best-1]==fblank))  w1 = 0.0;
    w2 = 1.0 - w1;
  ...
  /* to degrees */
  if ((*dAz)!=fblank) *dAz /= 3600.0;
  if ((*dEl)!=fblank)*dEl /= 3600.0;
} /* end GetPointOffset */
