/* $Id: ObitUVImagerWB.h 128 2009-09-23 14:48:29Z bill.cotton $     */
/*--------------------------------------------------------------------*/
/*;  Copyright (C) 2010                                               */
...
/*--------------------------------------------------------------------*/
#ifndef OBITUVIMAGERWB_H 
#define OBITUVIMAGERWB_H 
...
/**
 * \file ObitUVImagerWB.h
 * ObitUVImagerWB Class for imaging UV data.
 *
 * This class is derived from the #ObitUVImagerWB class.
 *
 ...
 * single plane.
 * 
 * \section ObitUVImagerWBaccess Creators and Destructors
 * An ObitUVImagerWB will usually be created using ObitUVImagerWBCreate which allows 
 * specifying a name for the object as well as other information.
 *
 * A copy of a pointer to an ObitUVImagerWB should always be made using the
 * #ObitUVImagerWBRef function which updates the reference count in the object.
 * Then whenever freeing an ObitUVImagerWB or changing a pointer, the function
 * #ObitUVImagerWBUnref will decrement the reference count and destroy the object
 * when the reference count hits 0.
 * There is no explicit destructor.
 *
 * \section ObitUVImagerWBparameters Control Parameters
...
/*--------------Class definitions-------------------------------------*/
/** ObitUVImagerWB Class structure. */
typedef struct {
#include "ObitUVImagerWBDef.h"   /* this class definition */
} ObitUVImagerWB;

/*----------------- Macroes ---------------------------*/
/** 
 * Macro to unreference (and possibly destroy) an ObitUVImagerWB
 * returns a ObitUVImagerWB*.
 * in = object to unreference
 */
#define ObitUVImagerWBUnref(in) ObitUnref (in)

/** 
 * Macro to reference (update reference count) an ObitUVImagerWB.
 * returns a ObitUVImagerWB*.
 * in = object to reference
 */
#define ObitUVImagerWBRef(in) ObitRef (in)
 ...
 * in = object to reference
 */
#define ObitUVImagerWBIsA(in) ObitIsA (in, ObitUVImagerWBGetClass())
...
/** Public: Create UVImagerWB object from description in an ObitInfoList */
void ObitUVImagerWBFromInfo (ObitUVImager *out, gchar *prefix, 
...
void ObitUVImagerWBClone (ObitUVImager *inn, ObitUVImager *outn, ObitErr *err);

/** Public: Create/initialize ObitUVImagerWB structures */
ObitUVImagerWB* ObitUVImagerWBCreate (gchar* name, olong order, ObitUV *uvdata, 
				      ObitErr *err);

/** Public: Create/initialize ObitUVImagerWB structures given mosaic */
 ...
 */
typedef struct  {
#include "ObitUVImagerWBClassDef.h"
} ObitUVImagerWBClassInfo; 

#endif /* OBITFUVIMAGERWB_H */ 
