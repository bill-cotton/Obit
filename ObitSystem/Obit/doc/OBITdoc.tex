% $Id: OBITdoc.tex,v 1.31 2008/01/22 12:33:36 bcotton Exp $ 
%
...
difficulty.
In addition, most classes have the hooks to provide locking and
synchronism mechanisms but at present, these are stubbed.
cfitsio, which is used for I/O to FITS files, is not thread--safe which
puts some limitations on parallel usage.
...
At present, the output is to stderr, but more formal logging, such as
AIPS logging, is easily implemented.
...
It is important that all software linked togetheer is all compiled
...
This heirarchy is currently (Jan. 2005) only partially implemented but
 ...
 =============================
              ...
              ObitDconCleanImage  ObitDconCleanVis
               (APCLN like)          (MX like)
                           ...
                           -------------------------
                           |                       |
                 ObitDconCleanVisSDI        ObitDconCleanVisMR
                 (SDI-Greisen)               (multi resolution)
...
images. Only support AIPS OVERLAP=3, DO3D=1 type.
\begin{verbatim}
Major Members:
   ObitUVImager
...
\end{verbatim}
\item ObitDConCleanVisSDI\\
Steer-Dwedney-Itio-Greisen CLEAN.
NYI
...
without them.
In particular, these are pgplot, cfitsio, GSL, and FFTW.
...
If configure does not find the FFTW library, FFTs are done using the GSL library
if that was found.
...
major axis size (deg), minor axis size (deg), position angle (deg) and
model type (0 = point,1 = Gaussian on sky,2 = Convolved Gaussian,
...
{[OPTIONAL] Component parameters;}
  parms[3] = type:\hfill\break
    0 = point, no parameters\hfill\break
    1 = Sky Gaussian, [0:3]=maj, min, PA\hfill\break
    2 = Convolved Gaussian, [0:3]=maj axis, min axis, PA (all deg)\hfill\break
    3 = Uniform Sphere [0] = radius (deg)\hfill\break
] 
\end{columns}
%
% Table modification history
...
\tablecol[{"DISP\_\#NO\_POL "}{"SECONDS "}{E}{(npoly,numBand)}{Disp}
{Dispersion}
] 
\tablecol[{"DDISP\_\#NO\_POL "}{"SEC/SEC  "}{E}{(npoly)}{DRate}
{Dispersion rate}
] 
\end{columns}
%
...
\tablecol[{"STATE\_\#NO\_POL "}{"PERCENT "}{E}{(8)}{State}
{State counts(?)}
] 
\tablecol[{"PC\_FREQ\_\#NO\_POL "}{"HZ      "}{D}{(numTones)}{PCFreq}
{Frequencies of the phase tones}
] 
\tablecol[{"PC\_REAL\_\#NO\_POL "}{"        "}{D}{(numTones)}{PCReal}
{Real part of tone phase}
] 
\tablecol[{"PC\_IMAG\_\#NO\_POL "}{"        "}{D}{(numTones)}{PCImag}
{Imaginary part of tone phase}
] 
\tablecol[{"PC\_RATE\_\#NO\_POL "}{"SEC/SEC "}{D}{(numTones)}{PCRate}
{Tone rate phase}
] 
\end{columns}
%
...
\clearpage
%
%%%%%%%%%%%%%%% ObitTableIDI_IARRAY_GEOMETRY Class %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
%
...
\tablecol[{"DERXYZ  "}{"METERS " }{D}{(3)}{derXYZ}
{look this up}
] 
\tablecol[{"ORBPARM "}{"       " }{D}{(numOrb)}{OrbParm}
{Orbital parameters.}
...
{This class contains tabular data and allows access.
This table is part of the IDI uv data format.
"IDI\_FREQUENCY" contains frequency related information for ``IF'' in uv
data.
An ``IF'' is a construct that allows sets of arbitrarily spaced frequencies.
...
\tablecol[{"ALPHA   "}{"JY     " }{E}{(no\_band)}{alpha}
{Spectral index per IF }
] 
\tablecol[{"FREQOFF "}{"HZ     " }{D}{(no\_band)}{FreqOff}
{Frequency offset (Hz) from IF nominal per IF}
] 
\tablecol[{"RAEPO   "}{"DEGREES " }{D}{(1)}{RAMean}
{Right ascension at mean EPOCH (actually equinox) }
...
\tablecol[{"WEIGHT"}{"       " }{E}{(maxis2,maxis3,maxis4,maxis5)}{Weight}
