...
\tablecol[{"TCAL\#NPOL"}{"KELVIN  " }{E}{(nIF)}{TCal}
...
Both FITS and AIPS cataloged data are supported.
This class is derived from the ObitTable class. 
For polynomials in zenith angle (TYPE\_n=2), only NTERM\_n, GAIN\_n and SENS\_n
are used.
Other types are defined but seem to have no implementations.}
]
\tableover{
In memory tables are stored in a fashion similar to how they are 
stored on disk - in large blocks in memory rather than structures.
...
{The number of tabulated values per row.}
]
\tablekey[{"TABREV"}{J}{revision}{3}{}
{Revision number of the table definition.}
]
...
{TYPE\_\#NO\_POL 1=tabulated, 2=polynomial in ZA, 3=spherical harmonics}
] 
\tablecol[{"NTERM\_\#NO\_POL"}{"        " }{J}{(numBand)}{NTerm}
{NTERM\_\#NO\_POL number of terms or entries actually given}
] 
\tablecol[{"X\_TYP\_\#NO\_POL"}{"        " }{J}{(numBand)}{XTyp}
{X\_TYP\_\#NO\_POL  Gain curve X-absicssa code type:
0 =$>$ none, 1 =$>$ elevation (degrees), 2 =$>$ zenith angle (degrees), 
3 =$>$ hour angle (degrees), 4=$>$ declination (degrees),  5 =$>$ 90-declination (degrees)}
] 
\tablecol[{"Y\_TYP\_\#NO\_POL"}{"        " }{J}{(numBand)}{YTyp}
{Y\_TYP\_\#NO\_POL Gain curve Y-absicssa code type}
] 
\tablecol[{"X\_VAL\_\#NO\_POL"}{"        " }{E}{(numBand)}{XVal}
{X\_TYP\_\#NO\_POL Value of X abscissa}
] 
\tablecol[{"Y\_VAL\_\#NO\_POL"}{"        " }{E}{(numTabs,numBand)}{YVal}
{Y\_TYP\_\#NO\_POL Values of Y abscissa}
] 
\tablecol[{"GAIN\_\#NO\_POL"}{"        " }{E}{(numTabs,numBand)}{gain}
{GAIN\_\#NO\_POL Corresponding values of relative gain (if tabulated);
or coefficients of approximating function. }
] 
\tablecol[{"SENS\_\#NO\_POL"}{"K/JY    " }{E}{(numBand)}{sens}
{X\_TYP\_\#NO\_POL Point source sensitivity}
] 
\end{columns}
%
% Table modification history
...
% 
%
\ClassName[{ObitTableOT}]
ObitTableOT Class
\tabletitle{Over the Top antenna pointing table for uv data}
% table name
\tablename{OT}
\tableintro[
{This class contains tabular data and allows access.
"AIPS OT" contains a flag indicating if the antenna elevation was over
$90^\circ$ deg, i.e. ``over the top''.
An ObitTableOT is the front end to a persistent disk resident structure.
Both FITS and AIPS cataloged data are supported.
This class is derived from the ObitTable class. }
]
\tableover{
...
\tablecol[{"ANTENNA    "}{"        " }{J}{(1)}{Antenna}
{Antenna number}
]
\tablecol[{"OVER TOP"}{"        " }{L}{(1)}{OverTop}
{True if antenna is observing at elevations over $90^\circ$ = ``Over
the top''}
] 
\end{columns}
%
% Table modification history
\begin{history}
\modhistory[{W. D. Cotton}{09/14/2006}{Revision 1: Copied from AIPS}]
\end{history}
%
\clearpage
%%%%%%%%%%%%%%% ObitTablePC Class %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\tablecol[{"STATE \#NO\_POL "}{"PERCENT "}{E}{(4,numBand)}{State}
{State counts(?)}
] 
\tablecol[{"PC\_FREQ \#NO\_POL "}{"HZ      "}{D}{(numTones,numBand)}{PCFreq}
{Frequencies of the phase tones}
] 
\tablecol[{"PC\_REAL \#NO\_POL "}{"        "}{E}{(numTones,numBand)}{PCReal}
{Real part of tone phase}
] 
\tablecol[{"PC\_IMAG \#NO\_POL "}{"        "}{E}{(numTones,numBand)}{PCImag}
{Imaginary part of tone phase}
] 
\tablecol[{"PC\_RATE \#NO\_POL "}{"SEC/SEC "}{E}{(numTones,numBand)}{PCRate}
{Tone rate phase}
] 
\end{columns}
%
