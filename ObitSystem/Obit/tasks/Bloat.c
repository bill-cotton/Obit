/* $Id:  $  */
/* Obit Task to duplicate channels in a UV data set                   */
/*--------------------------------------------------------------------*/
/*;  Copyright (C) 2015                                               */
...
/* internal prototypes */
/* Get inputs */
ObitInfoList* BloatIn (int argc, char **argv, ObitErr *err);
/* Set outputs */
void BloatOut (ObitInfoList* outList, ObitErr *err);
...
/* process */
void doBloat (ObitInfoList* myInput, ObitUV* inData, 
		 ObitErr* err);
/* Write history */
void BloatHistory (ObitInfoList* myInput, ObitUV* inData, 
		   ObitUV* outData, ObitErr* err);

/* Program globals */
gchar *pgmName = "Bloat";       /* Program name */
gchar *infile  = "Bloat.in" ;   /* File with program inputs */
gchar *outfile = "Bloat.out";   /* File to contain program outputs */
...
/*----------------------------------------------------------------------- */
{
    fprintf(stderr, "Usage: Bloat -input file -output ofile [args]\n");
    fprintf(stderr, "Duplicate channels in a UV data set\n");
    fprintf(stderr, "Arguments:\n");
    fprintf(stderr, "  -input input parameter file, def Bloat.in\n");
    fprintf(stderr, "  -output output result file, def Bloat.out\n");
...
ObitUV* setOutputUV (ObitInfoList *myInput, ObitUV* inData, ObitErr *err)
{
  ...
  if (!strncmp (Type, "AIPS", 4)) { /* AIPS input */
    ...
    /* Default out class is "Bloat" */
    if (!strncmp(Aclass, "      ", 6)) strncpy (Aclass, "Bloat", 7);
...
/*      err    Obit Error stack                                           */
/*----------------------------------------------------------------------- */
void doBloat (ObitInfoList* myInput, ObitUV* inData, ObitErr* err)
{
  ObitUV       *outData=NULL;
  ObitInfoType type;
  gint32       dim[MAXINFOELEMDIM] = {1,1,1,1,1};
  olong        doCalib=-1, doBand=-1, flagVer=-1;
  gchar        *dataParms[] = {  /* Parameters to calibrate/select data */
    "Stokes", "UVRange", "timeRange", "FreqID", "Sources", "souCode", "Qual", 
    "BIF", "EIF", "BChan", "EChan", "subA", "doCalWt", "dropSubA", 
    "doCalSelect", "doCalib", "gainUse", "doBand", "BPVer", "flagVer", 
    "doPol", "Mode", "corrType", "BLVer", "unHann", "nBloat",
    NULL  };
  ...
  /* Expand to output */
  outData = ObitUVUtilBloat (inData, FALSE, outData, err);
  if (err->error) Obit_traceback_msg (err, routine, inData->name);
  ...
  /* Do history */
  BloatHistory (myInput, inData, outData, err);
  if (err->error) Obit_traceback_msg (err, routine, inData->name);  
		   ...
		   ObitUV* outData, ObitErr* err)
{
  ...
  gchar        *hiEntries[] = {
    ...
    "BIF", "EIF", "BChan", "EChan",  "chInc", "chAvg", "UVRange",
    "UVRange",  "timeRange",  "Compress", "doCalWt", "dropSubA",
    "doCalSelect",  "doCalib",  "gainUse",  "doBand ",  "BPVer",  "flagVer", 
    "doPol", "BLVer", "nBloat",
    "dropSubA",  "doWtCal",  "corrType", "unHann",
    NULL};
  gchar *routine = "BloatHistory";
