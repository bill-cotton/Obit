/* $Id$  */
/* Obit task - AvgCh average planes of an image                  */
/*--------------------------------------------------------------------*/
/*;  Copyright (C) 2026                                               */
...
/* internal prototypes */
/* Get inputs */
ObitInfoList* AvgChIn (int argc, char **argv, ObitErr *err);
/* Set outputs */
void AvgChOut (ObitInfoList* outList, ObitErr *err);
...
/* Average */
void AvgChAver (ObitInfoList* myInput, ObitImage* inImage, 
		   ObitImage* outImage, ObitErr* err);
/* Write history */
void AvgChHistory (ObitInfoList* myInput, ObitImage* inImage, 
		      ObitImage* outImage, ObitErr* err);

/* Program globals */
gchar *pgmName = "AvgCh";       /* Program name */
gchar *infile  = "AvgCh.inp";   /* File with program inputs */
gchar *outfile = "AvgCh.out";   /* File to contain program outputs */
...
/*----------------------------------------------------------------------- */
/*   Obit program - Average planes of an image                            */
/*----------------------------------------------------------------------- */
{
  oint ierr = 0;
  ...
  /* Average */
  AvgChAver (myInput, inImage, outImage, err);
  if (err->error) {ierr = 1;  ObitErrLog(err);  if (ierr!=0) goto exit;}

  /* Do history */
  AvgChHistory ( myInput, inImage, outImage, err);
  if (err->error) {ierr = 1;  ObitErrLog(err);  if (ierr!=0) goto exit;}
...
/*----------------------------------------------------------------------- */
{
    fprintf(stderr, "Usage: AvgCh -input file -output ofile [args]\n");
    fprintf(stderr, "Extract subregion of an image\n");
    fprintf(stderr, "Arguments:\n");
    fprintf(stderr, "  -input input parameter file, def AvgCh.inp\n");
    fprintf(stderr, "  -output output result file, def AvgCh.out\n");
    ...
    fprintf(stderr, "  -outClass output AIPS file class [def 'AvgCh']\n");
...
ObitInfoList* defaultInputs(ObitErr *err)
{
  ...
  dim[0] = 1;
  itemp = 1;
  ObitInfoListPut (out, "avgCh", OBIT_oint, dim, &itemp, err);
  if (err->error) Obit_traceback_val (err, routine, "DefInput", out);
...
ObitImage* getOutputImage (ObitInfoList *myInput, ObitErr *err)
{
  ...
  if (!strncmp (Type, "AIPS", 4)) { /* AIPS input */
    ...
    if  (ObitInfoListGetP(myInput, "outClass", &type, dim, (gpointer)&strTemp)) {
      strncpy (Aclass, strTemp, 7);
    } else { /* Didn't find - use "AvgCh"*/
      strncpy (Aclass, "AvgCh ", 7);
    }
   ...
   /* define object */
    ...
    if (ObitInfoListGetP(myInput, "outFile", &type, dim, (gpointer)&strTemp)) {
      ...
      g_snprintf (outFile, 129, "AvgCh%s", strTemp2);
    }
    /* If blank use AvgCh+inFile */
...
/*      err    Obit Error stack                                           */
/*----------------------------------------------------------------------- */
void AvgChAver (ObitInfoList* myInput, ObitImage* inImage, ObitImage* outImage, 
		ObitErr* err)
{
  gboolean     done=FALSE, plnDone=FALSE;
  ObitIOCode   iretCode;
  ObitInfoType type;
  gint32       dim[MAXINFOELEMDIM] = {1,1,1,1,1};
  olong        blc[IM_MAXDIM] = {1,1,1,1,1,1,1};
  ...
  ObitFArray   **PlanePix=NULL;
  ofloat       tmp, norm=1.0;
  gchar        *routine = "AvgChAver";
  ...
  /* Control parameters */
  ObitInfoListGetTest(myInput, "chAvg", &type, dim, &chAvg); /* chAvg */
  chAvg = MAX(1, chAvg);
  norm = 1.0/chAvg;
  ...
  /* Make working arrays */
  PlanePix = g_malloc0(chAvg*sizeof(ObitFArray*));
  for (i=0; i<chAvg; i++) PlanePix[i] = ObitFArrayCreate("pln", 2, outImage->myDesc->inaxes);

  /* Loop over planes until hitting EOF */
  inext = 0; /* PlanePix array index */
  done = plnDone = FALSE;
  while (iretCode!= OBIT_IO_EOF) {
    ...
    iretCode = ObitImageRead (inImage, PlanePix[inext]->array, err); inext++;
    if (iretCode == OBIT_IO_EOF) done = TRUE;
    if (err->error) Obit_traceback_msg (err, routine, inImage->name);

    /* Time to average/dump? EOF, read chAvg or finished plane */
    if (done || (inext>=chAvg) || (plnDone)) {
      /* Average to PlanePix[0] */
      for (i=1; i<inext; i++) ObitFArrayAdd(PlanePix[0], PlanePix[i], PlanePix[0]);
      if (inext>1) {
	norm = 1./inext;
	ObitFArraySMul(PlanePix[0], norm);
      }
      /* Write plane */
  ...
  /* cleanup */
  if (PlanePix) {
    for (i=0; i<chAvg; i++) PlanePix[i] = ObitFArrayUnref(PlanePix[i]);
    g_free(PlanePix);
  }
  /* Copy tables */
  ObitDataCopyTables ((ObitData*)inImage, (ObitData*)outImage, exclist, NULL, err);
  if (err->error) Obit_traceback_msg (err, routine, outImage->name);
