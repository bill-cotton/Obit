...
void UVSubHistory (ObitInfoList* myInput, ObitUV* inData, ObitUV* outData, 
		   ObitErr* err);
/* Get beam images*/
void getBeam (ObitInfoList *myInput, ObitImage **Ipol, ObitImage **Vpol, 
	      ObitImage **Qpol, ObitImage **Upol, ObitErr *err);

/* Program globals */
gchar *pgmName = "UVSubBC";       /* Program name */
gchar *infile  = "UVSubBC.in" ;   /* File with program inputs */
gchar *outfile = "UVSubBC.out";   /* File to contain program outputs */
...
/*----------------------------------------------------------------------- */
/*   Obit Task to subtract CLEAN components from uvdata with beam corr.   */
/*----------------------------------------------------------------------- */
{
  oint         ierr = 0;
...
/*----------------------------------------------------------------------- */
{
    fprintf(stderr, "Usage: UVSubBC -input file -output ofile [args]\n");
    fprintf(stderr, "UVSubBC: Correct off-axis instrumental polarization\n");
    fprintf(stderr, "Arguments:\n");
    fprintf(stderr, "  -input input parameter file, def UVSubBC.in\n");
    fprintf(stderr, "  -output output result file, def UVSubBC.out\n");
				      ...
				      ObitErr *err)
{
  ...
  ObitImage    **image=NULL, *IBeam=NULL, *VBeam=NULL, *QBeam=NULL, *UBeam=NULL;
  ObitInfoType type;
  ...
  /* Get Beam */
  getBeam (myInput, &IBeam, &VBeam, &QBeam, &UBeam, err);
  if (err->error) Obit_traceback_val (err, routine, "myInput", skyModel);

  /* Create sky model */
  skyModel = ObitSkyModelVMBeamCreate ("Sky Model", mosaic, uvdata, 
				       IBeam, VBeam, QBeam, UBeam, err);
  if (err->error) Obit_traceback_val (err, routine, "myInput", skyModel);
