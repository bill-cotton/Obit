...
/* internal prototypes */
/* Get inputs */
ObitInfoList* RLDlyIn (int argc, char **argv, ObitErr *err);
/* Set outputs */
void RLDlyOut (ObitInfoList* outList, ObitErr *err);
...
/* Do solutions */
void RLDelayCal(ObitInfoList* myInput, ObitUV* avgData, ObitUV* inData, 
		ObitErr* err);
...
/* Write history */
void RLDlyHistory (ObitInfoList* myInput, ObitUV* inData, ObitErr* err);


/* Program globals */
gchar *pgmName = "RLDly";      /* Program name */
gchar *infile  = "RLDly.in" ;  /* File with program inputs */
gchar *outfile = "RLDly.out";  /* File to contain program outputs */
...
/*----------------------------------------------------------------------- */
{
    fprintf(stderr, "Usage: RLDly -input file -output ofile [args]\n");
    fprintf(stderr, "RLDly Obit task determine bandpass for UV data\n");
    fprintf(stderr, "Arguments:\n");
    fprintf(stderr, "  -input input parameter file, def RLDly.in\n");
    fprintf(stderr, "  -output uv data onto which to attach FG table, def RLDly.out\n");
 ...
 * \param sourId  Source ID (0-rel) in sList if uvdata is a single source file
		 ...
		 ObitErr* err)
{
  ...
  /* Fit - write SN table */
  SNTable = ObitUVRLDelayCal (avgData, inData, err);
  if (err->error) Obit_traceback_msg (err, routine, inData->name);
