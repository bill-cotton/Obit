...
ObitUV* ObitUVLin2Cir (ObitUV *inUV, ObitUV *outUV, ObitErr *err)
{
  ...
  while ((iretCode==OBIT_IO_OK) && (oretCode==OBIT_IO_OK)) {
	...
	gi = cosPA * sinPA + sinPA * cosPA; 
      ...
      /* Correct RL */
      tr = outUV->buffer[indx+2*incs];  
      ti = outUV->buffer[indx+2*incs+1];
      outUV->buffer[indx+2*incs]   = tr * gr - ti * gi;
      outUV->buffer[indx+2*incs+1] = ti * gr + tr * gi;
      
      /* Correct LR */
      tr = outUV->buffer[indx+3*incs];
      ti = outUV->buffer[indx+3*incs+1];
      outUV->buffer[indx+3*incs]   = tr * gr + ti * gi;
      outUV->buffer[indx+3*incs+1] = ti * gr - tr * gi;
