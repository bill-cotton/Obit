/* $Id: Hann.c 221 2010-09-15 01:10:23Z bill.cotton $  */
...
/* internal prototypes */
/* Get inputs */
ObitInfoList* HannIn (int argc, char **argv, ObitErr *err);
/* Set outputs */
void HannOut (ObitInfoList* outList, ObitErr *err);
...
/* process */
void doHann (ObitInfoList* myInput, ObitUV* inData, 
		 ObitErr* err);
/* Write history */
void HannHistory (ObitInfoList* myInput, ObitUV* inData, 
		   ObitUV* outData, ObitErr* err);

/* Program globals */
gchar *pgmName = "Hann";       /* Program name */
gchar *infile  = "Hann.in" ;   /* File with program inputs */
gchar *outfile = "Hann.out";   /* File to contain program outputs */
...
/*----------------------------------------------------------------------- */
{
    fprintf(stderr, "Usage: Hann -input file -output ofile [args]\n");
    fprintf(stderr, "Hann Obit Hanning smooth uv data\n");
    fprintf(stderr, "Arguments:\n");
    fprintf(stderr, "  -input input parameter file, def Hann.in\n");
    fprintf(stderr, "  -output output result file, def Hann.out\n");
...
ObitUV* setOutputUV (ObitInfoList *myInput, ObitUV* inData, ObitErr *err)
{
  ...
  if (!strncmp (Type, "AIPS", 4)) { /* AIPS input */
    ...
    /* Default out class is "Hann" */
    if (!strncmp(Aclass, "      ", 6)) strncpy (Aclass, "Hann", 7);
...
/*      err    Obit Error stack                                           */
/*----------------------------------------------------------------------- */
void doHann (ObitInfoList* myInput, ObitUV* inData, ObitErr* err)
{
  ObitUV       *outData=NULL;
  ObitInfoType type;
  gint32       dim[MAXINFOELEMDIM] = {1,1,1,1,1};
  ...
  /* Smooth to output */
  outData = ObitUVUtilHann (inData, FALSE, outData, err);
  if (err->error) Obit_traceback_msg (err, routine, inData->name);
  ...
  /* Do history */
  HannHistory (myInput, inData, outData, err);
  if (err->error) Obit_traceback_msg (err, routine, inData->name);  
