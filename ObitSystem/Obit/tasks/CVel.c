/* $Id$  */
/* to do
1) pick default reference channel = closest to line+velocityLSR
2) scale uvw by change in frequency
 */
/* Doppler tracking corrections for UV data          .                */
/*--------------------------------------------------------------------*/
/*;  Copyright (C) 2012                                               */
...
/* internal prototypes */
/* Get inputs */
ObitInfoList* CVelIn (int argc, char **argv, ObitErr *err);
/* Set outputs */
void CVelOut (ObitInfoList* outList, ObitErr *err);
...
/* Digest inputs */
void digestInputs(ObitInfoList *myInput, ObitUV* inData, ObitErr *err);
/* Get input data */
...
/* Write history */
void CVelHistory (ObitInfoList* myInput, ObitUV* inData, ObitUV* outData, 
		   ObitErr* err);

/* Program globals */
gchar *pgmName = "CVel";       /* Program name */
gchar *infile  = "CVel.in" ;   /* File with program inputs */
gchar *outfile = "CVel.out";   /* File to contain program outputs */
...
/*----------------------------------------------------------------------- */
{
  ...
  gchar        *dataParms[] = {  /* Parameters to calibrate/select data */
    ...
    "JDref", "RestFreq", "VelLSR", "refChan",
     NULL};
  gchar        *updateParm[] = {"refChan"};
  ObitErr      *err= NULL;

   /* Startup - parse command line, read inputs */
  err = newObitErr();

  myInput = CVelIn (argc, argv, err);
  ...
  /* Digest inputs */
  digestInputs(myInput, inData, err);
  if (err->error) ierr = 1; ObitErrLog(err); if (ierr!=0) goto exit;
  ...
  /* Correct for Doppler  */
  outData = ObitDopplerCVel(inData, FALSE, outData, err);
  if (err->error) ierr = 1; ObitErrLog(err); if (ierr!=0) goto exit;
  ...
  /* Get save updated parameters to myInput for history */
  ObitInfoListCopyList (outData->info, myInput, updateParm);
  
  /* History */
  CVelHistory (myInput, inData, outData, err);
  if (err->error) ierr = 1; ObitErrLog(err); if (ierr!=0) goto exit;
...
/*----------------------------------------------------------------------- */
{
 ...
 /* command line arguments */
  ...
  for (ax=1; ax<argc; ax++) {
    ...
    } else if (strcmp(arg, "-RestFreq") == 0) { /* rest frequency */
      dim[0] = 1;
      dtemp = strtod(argv[++ax], NULL);
      ObitInfoListPut (list, "RestFreq", OBIT_double, dim, &dtemp, err);
...
/*----------------------------------------------------------------------- */
{
    fprintf(stderr, "Usage: CVel -input file -output ofile [args]\n");
    fprintf(stderr, "CVel Mkkes Doppler corrections for UV data\n");
    fprintf(stderr, "Arguments:\n");
    fprintf(stderr, "  -input input parameter file, def CVel.in\n");
    fprintf(stderr, "  -output output result file, def CVel.out\n");
    ...
    fprintf(stderr, "  -RestFreq Rest frequency in Hz\n");
...
void digestInputs(ObitInfoList *myInput, ObitUV* inData, ObitErr *err)
{
  ...
  /* Convert reference date to JD - default data of uv data */
  ObitInfoListGetP (myInput, "refDate", &type, dim, (gpointer)&strTemp);
  if (strncmp (strTemp, "0000-00-00", 10)) ObitUVDescDate2JD (strTemp, &JDref);
  else JDref = inData->myDesc->JDObs;
  dim[0] = dim[1] = dim[2] = dim[3] = dim[4] = 1;
  ObitInfoListAlwaysPut (myInput, "JDref", OBIT_double, dim, &JDref);

  /* Check rest frequency */
  RestFreq = -1.0;
  ObitInfoListGetTest(myInput, "RestFreq", &type, dim, &RestFreq);
  Obit_return_if_fail((RestFreq>0.0), err,
                        "Rest Frequency MUST be provided");
...
ObitUV* setOutputData (ObitInfoList *myInput, ObitUV* inData, ObitErr *err)
{
  ...
  if (!strncmp (Type, "AIPS", 4)) { /* AIPS input */
    ...
    /* Default out class is "CVel" */
    if (!strncmp(Aclass, "      ", 6)) strncpy (Aclass, "CVel  ", 7);
		   ...
		   ObitErr* err)
{
  ...
  gchar        *hiEntries[] = {
    ...
    "doBand", "BPVer", "Smooth", "refDate", "RestFreq", "VelLSR", "refChan",
    "outFile",  "outDisk",  "outName", "outClass", "outSeq",
    NULL};
  gchar *routine = "CVelHistory";
