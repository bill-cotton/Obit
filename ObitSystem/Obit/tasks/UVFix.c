/* $Id$  */
/* Obit Task to copy uv data recomputing u,v,w       .                */
/*--------------------------------------------------------------------*/
/*;  Copyright (C) 2013                                               */
...
/* internal prototypes */
/* Get inputs */
ObitInfoList* UVFixIn (int argc, char **argv, ObitErr *err);
/* Set outputs */
void UVFixOut (ObitInfoList* outList, ObitErr *err);
...
/* Update AN table */
void UpdateAN (ObitInfoList *myInput, ObitUV* inData, ObitErr *err);
/* Write history */
void UVFixHistory (ObitInfoList* myInput, ObitUV* inData, ObitUV* outData, 
		   ObitErr* err);

/* Program globals */
gchar *pgmName = "UVFix";       /* Program name */
gchar *infile  = "UVFix.in" ;   /* File with program inputs */
gchar *outfile = "UVFix.out";   /* File to contain program outputs */
...
/*----------------------------------------------------------------------- */
{
  ...
  /* Get upDate AN table if Earth orientation parameters given */
  UpdateAN (myInput, inData, err);
  if (err->error) ierr = 1; ObitErrLog(err); if (ierr!=0) goto exit;
...
/*----------------------------------------------------------------------- */
{
    fprintf(stderr, "Usage: UVFix -input file -output ofile [args]\n");
    fprintf(stderr, "UVFix Obit task to compute u,v,w terms\n");
    fprintf(stderr, "Arguments:\n");
    fprintf(stderr, "  -input input parameter file, def UVFix.in\n");
    fprintf(stderr, "  -output output result file, def UVFix.out\n");
...
ObitUV* setOutputData (ObitInfoList *myInput, ObitUV* inData, ObitErr *err)
{
  ...
  if (!strncmp (Type, "AIPS", 4)) { /* AIPS input */
    ...
    /* Default out class is "UVFix" */
    if (!strncmp(Aclass, "      ", 6)) strncpy (Aclass, "UVFix", 7);
...
/*----------------------------------------------------------------------- */
/*  Possible update output AN table                                       */
/*   Input:                                                               */
/*      myInput   Input parameters on InfoList                            */
/*      inData   ObitUV to update                                         */
/*   Output:                                                              */
/*      err    Obit Error stack                                           */
/*----------------------------------------------------------------------- */
void UpdateAN (ObitInfoList* myInput, ObitUV* inData,  ObitErr* err)
{
  ofloat ut1utc=0.0, datautc=0.0, pole[2]={0.,0.};
  ObitInfoType type;
  gint32       dim[MAXINFOELEMDIM] = {1,1,1,1,1};
  ObitTableAN *ANTab=NULL;
  olong  iver, numIF=0, numOrb=0, numPCal=0;
  gchar *routine = "UpdateAN";
  ...
  /* Get EOP */
  ObitInfoListGetTest(myInput, "UT1UTC",  &type, dim, &ut1utc);
  ObitInfoListGetTest(myInput, "DataUTC", &type, dim, &datautc);
  ObitInfoListGetTest(myInput, "Pole",    &type, dim, pole);

  /* Anything to do? */
  if ((ut1utc==0.0) && (datautc==0.0) && (pole[0]==0.0) && (pole[1]==0.0))
    return;

  /* Get AN table */
  iver = 1;
  ANTab = newObitTableANValue ("AN table", (ObitData*)inData, 
			       &iver, OBIT_IO_ReadWrite, numIF, numOrb, numPCal, err);
  if (ANTab==NULL) Obit_log_error(err, OBIT_Error, "ERROR with AN table");
  if (err->error) Obit_traceback_msg (err, routine, inData->name);
  
   /* Open table */
  ObitTableANOpen (ANTab, OBIT_IO_ReadWrite, err);
  if (err->error) Obit_traceback_msg (err, routine, ANTab->name);

  /* Update Table */
  if ((pole[0]!=0.0) || (pole[1]!=0.0)) {
    ANTab->PolarX = pole[0];
    ANTab->PolarY = pole[1];
  }
  if (ut1utc!=0.0)  ANTab->ut1Utc  = ut1utc;
  if (datautc!=0.0) ANTab->dataUtc = datautc;

  ANTab->myStatus = OBIT_Modified;  /* Modified - force update */
