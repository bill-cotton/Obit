 		     ...
 		     ObitErr *err)
{
  ...
  if (!strncmp (Type, "AIPS", 4)) { /* AIPS input */
    ...
    if (ObitInfoListGetP(myInput, "out2Class", &type, dim, (gpointer)&strTemp)) {
      ...
      strncpy (Aclass, "BeamCo", 7);
    }
    /* Default for blank */
    if (!strncmp (Aclass, "    ", 4)) strncpy (Aclass, "BeamCo", 7);
    Aclass[6] = 0;
		 ...
		 ObitErr* err)
{
  ...
  olong        niter, numAntType, *IChanSel;
  gboolean     doFlat, autoWindow, Tr=TRUE, do3D, doCmplx=FALSE, doVPol;
  ...
  /* Want imag. images in beam model? */
  ...
  for (ipoln=bpoln; ipoln<=epoln; ipoln++) {
	...
	|| (IStokes[0]=='R') || (IStokes[0]=='L')) {
      ...
      /* Create SkyModel */
      skyModel = 
	(ObitSkyModel*)ObitSkyModelVMBeamMFCreate("BeamCor Model", 
						  imager->mosaic, outData, numAntType,
						  RXBeam, LYBeam, RLBeam, LRBeam,
  ...
  RXBeamIm  = ObitImageUnref(RXBeamIm);
  LYBeamIm  = ObitImageUnref(LYBeamIm);
  RLBeamIm  = ObitImageUnref(RLBeamIm);
  LRBeamIm  = ObitImageUnref(LRBeamIm);
		    ...
		    ObitErr* err)
{
  ...
  gchar        hicard[81];
  gchar        *hiEntries[] = {
    "DataType", "inFile",  "inDisk", "inName", "inClass", "inSeq","doCmplx", 
    ...
    "UVITaper", "MFTaper", "RobustIF", "TaperIF",
    "doCalSelect",  "doCalib",  "gainUse",  "doBand ",  "BPVer",  "flagVer", 
