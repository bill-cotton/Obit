...
/* internal prototypes */
/* Get inputs */
ObitInfoList* VL2FSIn (int argc, char **argv, ObitErr *err);
...
/* Get output data */
ObitImage* getOutputData (ObitInfoList *myInput, ObitErr *err);
/* Write history */
void VL2FSHistory (ObitInfoList* myInput, ObitImage* inData, ObitErr* err);

/* Program globals */
gchar *pgmName = "VL2FS";       /* Program name */
gchar *infile  = "VL2FS.in" ;   /* File with program inputs */
gchar *outfile = "VL2FS.out";   /* File to contain program outputs */
...
/*----------------------------------------------------------------------- */
{
  ...
  ObitImage    *inData = NULL, *outData = NULL;
  ObitTableVL  *inTab=NULL;
  ObitTableFS  *outTab=NULL, *outTab2=NULL;
  ObitErr      *err= NULL;
  ObitInfoType type;
  gboolean     doVL2FS=TRUE, doSpec=TRUE, doFit=TRUE, doPrint=TRUE;
  gchar        *idataParms[] = {  /* Control parameter */
    "minFlux",  "minSNR", NULL};
  gchar        *odataParms[] = {  /* Control parameter */
    "RMSsize",  "minFlux",  "minSNR", NULL};
  gchar        OutPrint[256];
  gint32       dim[MAXINFOELEMDIM] = {1,1,1,1,1};
  FILE  *prtFile;        /* Output print file */

  /* Startup - parse command line */
  err = newObitErr();
  myInput = VL2FSIn (argc, argv, err);
  ...
  ObitInfoListGetTest(myInput, "doSpec",   &type, dim, &doSpec);
  ObitInfoListGetTest(myInput, "doFit",    &type, dim, &doFit);
  ObitInfoListGetTest(myInput, "doPrint",  &type, dim, &doPrint);
  strcpy (OutPrint, "          ");
  ObitInfoListGetTest(myInput, "OutPrint", &type, dim, OutPrint);
  ...
  if (doVL2FS) {
    inTab = newObitTableVLValue (inData->name, (ObitData*)inData, 
				 &VLver,  OBIT_IO_ReadWrite, err);
    /* Get input parameters from myInput, copy to inTab */
    ObitInfoListCopyList (myInput, inTab->info, idataParms);
    Obit_log_error(err, OBIT_InfoErr, "Convert VL %d to FS %d", VLver, FSver);
    ObitErrLog(err);
    outTab = ObitTableFSUtilVL2FS (inTab, (ObitData*)outData, FSver, err);
  } else { 
    ObitImageOpen (outData, OBIT_IO_ReadWrite, err);
    outTab = newObitTableFSValue (outData->name, (ObitData*)outData, 
  ...
  ObitInfoListCopyList (myInput, outData->info, odataParms);

  /* Loop through output image planes extracting info */
  if (doSpec) {
    Obit_log_error(err, OBIT_InfoErr, "Get spectra from output image");
    ObitErrLog(err);
  ...
  /* Print table */
  if (doPrint) {
    if (strncmp (OutPrint, "        ", 8)) {
      ObitTrimTrail(OutPrint);  /* Trim any trailing blanks */
      prtFile = fopen (OutPrint, "a");
    } else prtFile=stdout;
    /* Get input parameters from myInput, copy to outTab2 */
    ObitInfoListCopyList (myInput, outTab2->info, odataParms);
  ObitTableFSPrint (outTab2, outData, prtFile, err);
 }
 ...
 exit:
  ...
  outTab    = ObitTableFSUnref(outTab);
  outTab2   = ObitTableFSUnref(outTab2);
...
/*----------------------------------------------------------------------- */
{
    fprintf(stderr, "Usage: VL2FS -input file -output ofile [args]\n");
    fprintf(stderr, "Convert VL table to FS \n");
    fprintf(stderr, "Arguments:\n");
    fprintf(stderr, "  -input input parameter file, def VL2FS.in\n");
    fprintf(stderr, "  -output uv data onto which to attach FG table, def VL2FS.out\n");
...
/*      err    Obit Error stack                                           */
/*   Return                                                               */
/*       ObitImage with output data                                        */
/*----------------------------------------------------------------------- */
ObitImage* getOutputData (ObitInfoList *myInput, ObitErr *err)
{
  ObitImage    *outData = NULL;
  ObitInfoType type;
  gint32       dim[MAXINFOELEMDIM] = {1,1,1,1,1};
  olong         blc[IM_MAXDIM] = {1,1,1,1,1,1,1};
  olong         trc[IM_MAXDIM] = {0,0,0,0,0,0,0};
  ...
  /* Create basic output Image data Object */
  outData = newObitImage("output Image data");
  ...
  if (!strncmp (Type, "AIPS", 4)) { /* AIPS input */
    ...
    /* define object */
    ObitImageSetAIPS (outData, OBIT_IO_byPlane, disk, cno, AIPSuser, blc, trc, err);
    if (err->error) Obit_traceback_val (err, routine, "myInput", outData);
    ...
    /* define object */
    ObitImageSetFITS (outData, OBIT_IO_byPlane, disk, outFile, blc, trc, err);
    if (err->error) Obit_traceback_val (err, routine, "myInput", outData);
  ...
  /* Full instantiation - needed to initialize tableList */
  ObitImageFullInstantiate (outData, TRUE, err);
  if (err->error) Obit_traceback_val (err, routine, "myInput", outData);
...
void VL2FSHistory (ObitInfoList* myInput, ObitImage* inData, ObitErr* err)
{
  ...
  gchar        *hiEntries[] = {
    ...
    "inVL", "minFlux", "minSNR", "RMSsize", "OutPrint",
    "doVL2FS", "doSpec", "doFit", "doPrint", 
    NULL};
  gchar *routine = "VL2FSHistory";
