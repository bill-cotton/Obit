""" Utility to convert MeerKAT HDF format data to AIPS (or FITS)

This module requires katfile and katpoint and their dependencies
From version by Tom Mauch
"""
...
def KAT2AIPS (katdata, outUV, disk, fitsdisk, err, \
    ...
    Parameters
    ----------
    katdata : input katfile object from katdal.open(filename)
    outUV : Obit AIPS UV object, should be a MeerKAT template for the
        appropriate number of IFs and poln. Use MakeTemplate to 
        generate a FITS version of the template then read that into 
        aips and pass as outUV
    disk  : int
        AIPS Disk number
    fitsdisk: int
        FITS Disk number
    ...
    stop_w : bool
        Fringe stop data? (Values only for KAT-7)
    """
    ################################################################
...
def MakeTemplate(inuv,outuv,katdata):
    ...
    inuv    = input template fits filename
    outuv   = output fits file name
    katdata = output of katdal.open(filename)
    """
