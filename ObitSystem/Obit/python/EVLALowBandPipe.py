...
The EVLA Lowband (P)Continuum Pipeline.  The pipeline can be invoked from the command line 
as, ::

    ObitTalk EVLALowBandPipe AipsSetupScript PipelineParamScript
...
def pipeline( aipsSetup, parmFile):
    ...
    * *parmFile* = pipeline input parameters file
    ...
    if parms["doSYGain"] and not check:
        plotFile = "./"+fileRoot+"SYGainCal.ps"
        retCode = EVLASYCal(uv, err,  \
                                SYVer=parms["SYVer"],  SYOut=parms["SYOut"], \
                                calInt=parms["SYCalInt"], smoTime=parms["SYSmoTime"], \
                                smoFunc=parms["SYSmoFunc"], SWUse=parms["SYSWUse"], \
