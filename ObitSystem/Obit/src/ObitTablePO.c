...
/**
 * \file ObitTablePO.c
 * ObitTablePO class function definitions.
 *
 * This class is derived from the #ObitTable class.
 */

/** name of the class defined in this file */
static gchar *myClassName = "ObitTablePO";
...
static gchar *myRowClassName = "ObitTablePORow";
 ...
 */
static ObitTablePORowClassInfo myRowClassInfo = {FALSE};

/*------------------  Table  ------------------------*/
/**
 * ClassInfo structure ObitTablePOClassInfo.
 * This structure is used by class objects to access class functions.
 */
static ObitTablePOClassInfo myClassInfo = {FALSE};

/*---------------Private function prototypes----------------*/
/** Private: Initialize newly instantiated Row object. */
void  ObitTablePORowInit  (gpointer in);

/** Private: Deallocate Row members. */
void  ObitTablePORowClear (gpointer in);

/** Private: Initialize newly instantiated object. */
void  ObitTablePOInit  (gpointer in);

/** Private: Deallocate members. */
void  ObitTablePOClear (gpointer in);

/** Private: update table specific info */
static void ObitTablePOUpdate (ObitTablePO *in, ObitErr *err);

/** Private: copy table keywords to descriptor info list */
static void ObitTablePODumpKey (ObitTablePO *in, ObitErr *err);

/** Private: Set Class function pointers */
static void ObitTablePOClassInfoDefFn (gpointer inClass);

/** Private: Set Row Class function pointers */
static void ObitTablePORowClassInfoDefFn (gpointer inClass);
/*----------------------Public functions---------------------------*/
 ...
 */
ObitTablePORow* newObitTablePORow (ObitTablePO *table)
{
  ObitTablePORow* out;
  odouble   *dRow;
  oint      *iRow;
  ...
  if (!myRowClassInfo.initialized) ObitTablePORowClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTablePORow));

  /* initialize values */
  out->name = g_strdup("TablePO Row");

  /* set ClassInfo */
  out->ClassInfo = (gpointer)&myRowClassInfo;

  /* initialize other stuff */
  ObitTablePORowInit((gpointer)out);
 ...
 * \return the new object.
 */
ObitTablePO* newObitTablePO (gchar* name)
{
  ObitTablePO* out;

  /* Class initialization if needed */
  if (!myClassInfo.initialized) ObitTablePOClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTablePO));
 ...
 */
gconstpointer ObitTablePOGetClass (void)
{
  /* Class initialization if needed */
  if (!myClassInfo.initialized) ObitTablePOClassInit();
 ...
 */
ObitTablePO* newObitTablePOValue (gchar* name, ObitData *file, olong *ver,
 	                    ObitIOAccess access,
  		    
		     ObitErr *err)
{
  ObitTablePO* out=NULL;
  ...
  ObitIOCode retCode;
  gchar *tabType = "AIPS PO";
  gchar *routine = "newObitTablePOValue";
 ...
 /* error checks */
  ...
  if (*ver==0) { 
    highVer = ObitTableListGetHigh (list, "AIPS PO");
  ...
  if (*ver>0) { /* has to be specified */
    ...
    if (testTab!=NULL) { /* it exists, use it if is an ObitTablePO */
      if (ObitTablePOIsA(testTab)) { /* it is an ObitTablePO */
	out = ObitTableRef(testTab);
      } else { /* needs conversion */
 	out = ObitTablePOConvert(testTab);
  ...
  /* Set values */
  out->revision = 1;
  strncpy (out->RefDate, "YYYYMMDD", MAXKEYCHARTABLEPO );
  ...
  for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
  colNo++;
  desc->FieldName[colNo] = g_strdup("SOURCE RA");
  desc->FieldUnit[colNo] = g_strdup("DEGREES");
  desc->type[colNo] = OBIT_double;
  for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
  colNo++;
  desc->FieldName[colNo] = g_strdup("SOURCE DEC");
  desc->FieldUnit[colNo] = g_strdup("DEGREES");
  desc->type[colNo] = OBIT_double;
  for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
  colNo++;
  desc->FieldName[colNo] = g_strdup("SOURCE DISTANCE");
  desc->FieldUnit[colNo] = g_strdup("AU");
  desc->type[colNo] = OBIT_double;
  for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
  colNo++;
 ...
 return out;
} /* end newObitTablePOValue */

/**
 * Convert an ObitTable to an ObitTablePO.
 ...
 */
ObitTablePO* ObitTablePOConvert (ObitTable* in)
{
  ObitTablePO *out;
 ...
 * \return pointer to the new object.
 */
ObitTablePO* ObitTablePOCopy (ObitTablePO *in, ObitTablePO *out, ObitErr *err)
{
  gchar *routine = "ObitTablePOCopy";
 ...
 /* error checks */
  ...
  out = (ObitTablePO*)ObitTableCopy ((ObitTable*)in, (ObitTable*)out, err);
  if (err->error) /* add traceback,return */
    Obit_traceback_val (err, routine,in->name, out);

  /* Copy this class  info */
  out->revision = in->revision;
  strncpy (out->RefDate, in->RefDate, MAXKEYCHARTABLEPO );
  /* Update class specific info */
  ObitTablePOUpdate (out, err);
 ...
 */
ObitIOCode ObitTablePOOpen (ObitTablePO *in, ObitIOAccess access, 
			  ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gint32 dim[MAXINFOELEMDIM] = {1,1,1,1,1};
  olong nRowPIO;
  gchar *routine = "ObitTablePOOpen";
 ...
 * \param iPORow   Row number, -1 -> next
 * \param row      Table Row structure to receive data
 * \param err ObitErr for reporting errors.
 * \return return code, OBIT_IO_OK=> OK
 */
ObitIOCode 
ObitTablePOReadRow  (ObitTablePO *in, olong iPORow, ObitTablePORow *row,
		     ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  odouble   *dRow;
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTablePOReadRow";
  ...
  if (in->myStatus == OBIT_Inactive) {
    Obit_log_error(err, OBIT_Error,
		   "AIPS PO Table is inactive for  %s ", in->name);
    return retCode;
 }

  /* read row iPORow */
  retCode = ObitTableRead ((ObitTable*)in, iPORow, NULL,  err);
  if (err->error) 
    Obit_traceback_val (err, routine, in->name, retCode);
  ...
  row->RA = dRow[in->RAOff];
  row->Dec = dRow[in->DecOff];
  row->Dist = dRow[in->DistOff];
  row->status = iRow[in->myDesc->statusOff];

  return retCode;
} /*  end ObitTablePOReadRow */
 ...
 */
void 
ObitTablePOSetRow  (ObitTablePO *in, ObitTablePORow *row,
		     ObitErr *err)
{
  odouble   *dRow;
  oint      *iRow;
  ...
  if (in->myStatus == OBIT_Inactive) {
    Obit_log_error(err, OBIT_Error,
		   "PO Table is inactive for  %s ", in->name);
    return;
 }
 ...
 * correct table buffer locations using ObitTablePOSetRow  
 * \param in       Table to read
 * \param iPORow   Row number, -1 -> next
 ...
 */
ObitIOCode 
ObitTablePOWriteRow  (ObitTablePO *in, olong iPORow, ObitTablePORow *row,
		      ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gshort    *siRow;
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTablePOWriteRow";
  ...
  dRow[in->RAOff] = row->RA;
  dRow[in->DecOff] = row->Dec;
  dRow[in->DistOff] = row->Dist;
  ...
  /* Write row iPORow */
  retCode = ObitTableWrite ((ObitTable*)in, iPORow, NULL,  err);
  if (err->error) 
    Obit_traceback_val (err, routine,in->name, retCode);

  return retCode;
} /*  end ObitTablePOWriteRow */
 ...
 */
ObitIOCode ObitTablePOClose (ObitTablePO *in, ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gchar *routine = "ObitTablePOClose";
 ...
 */
void ObitTablePORowInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTablePORow *in = inn;
 ...
 *           Actually it should be an ObitTablePORow* cast to an Obit*.
 */
void ObitTablePORowClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTablePORow *in = inn;
...
} /* end ObitTablePORowClear */

/**
 * Initialize global ClassInfo Structure.
 */
void ObitTablePORowClassInit (void)
{
  ...
  ObitTablePORowClassInfoDefFn ((gpointer)&myRowClassInfo);
 
  myRowClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTablePORowClassInit */
...
static void ObitTablePORowClassInfoDefFn (gpointer inClass)
{
  ObitTablePORowClassInfo *theClass = (ObitTablePORowClassInfo*)inClass;
  ...
  theClass->ObitClassInit = (ObitClassInitFP)ObitTablePORowClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTablePORowClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTablePORowGetClass;
  theClass->newObit         = NULL;
  theClass->newObitTableRow = (newObitTableRowFP)newObitTablePORow;
  theClass->ObitCopy        = NULL;
  theClass->ObitClone       = NULL;
  theClass->ObitClear       = (ObitClearFP)ObitTablePORowClear;
  theClass->ObitInit        = (ObitInitFP)ObitTablePORowInit;

} /* end ObitTablePORowClassDefFn */
 ...
 * \param inn Pointer to the object to initialize.
 */
void ObitTablePOInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTablePO *in = inn;
 ...
 * For some reason this wasn't build into the GType class.
 * \param  inn Pointer to the object to deallocate.
 *           Actually it should be an ObitTablePO* cast to an Obit*.
 */
void ObitTablePOClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTablePO *in = inn;
...
} /* end ObitTablePOClear */

/**
 * Initialize global ClassInfo Structure.
 */
void ObitTablePOClassInit (void)
{
  ...
  /* Set function pointers */
  ObitTablePOClassInfoDefFn ((gpointer)&myClassInfo);
 
  myClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTablePOClassInit */
...
static void ObitTablePOClassInfoDefFn (gpointer inClass)
{
  ObitTablePOClassInfo *theClass = (ObitTablePOClassInfo*)inClass;
  ...
  /* function pointers defined or overloaded this class */
  theClass->ObitClassInit = (ObitClassInitFP)ObitTablePOClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTablePOClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTablePOGetClass;
  theClass->newObit       = (newObitFP)newObitTablePO;
  theClass->ObitCopy      = (ObitCopyFP)ObitTablePOCopy;
  theClass->ObitClone     = (ObitCloneFP)ObitTableClone;
  theClass->ObitClear     = (ObitClearFP)ObitTablePOClear;
  theClass->ObitInit      = (ObitInitFP)ObitTablePOInit;
  theClass->ObitTableConvert = (ObitTableConvertFP)ObitTablePOConvert;
  theClass->ObitTableOpen    = (ObitTableOpenFP)ObitTablePOOpen;
  theClass->ObitTableClose   = (ObitTableCloseFP)ObitTablePOClose;
  ...
  theClass->ObitTableReadRow = 
    (ObitTableReadRowFP)ObitTablePOReadRow;
  theClass->ObitTableWriteRow = 
    (ObitTableWriteRowFP)ObitTablePOWriteRow;

} /* end ObitTablePOClassDefFn */
...
static void ObitTablePOUpdate (ObitTablePO *in, ObitErr *err)
{
 ...
 /* error checks */
  ...
  ObitInfoListGetTest(in->myDesc->info, "TABREV", &type, dim, 
   ...
   /* RefDate */
  strncpy (in->RefDate, "YYYYMMDD", MAXKEYCHARTABLEPO); 
  ...
  if (desc->FieldName) {
    for (i=0; i<desc->nfield; i++) {
	 ...
	 in->SourIDOff = desc->offset[i];
      ...
      if (!strncmp (desc->FieldName[i], "SOURCE RA", 9)) {
	 in->RAOff = desc->offset[i];
 	 in->RACol = i;
      }
      if (!strncmp (desc->FieldName[i], "SOURCE DEC", 10)) {
	 in->DecOff = desc->offset[i];
 	 in->DecCol = i;
      }
      if (!strncmp (desc->FieldName[i], "SOURCE DISTANCE", 15)) {
	 in->DistOff = desc->offset[i];
 	 in->DistCol = i;
      }
     }
  }
  ...
  Obit_return_if_fail((in->TimeOff > -1), err,
       "ObitTablePOUpdate: Could not find column Time");
  Obit_return_if_fail((in->SourIDOff > -1), err,
       "ObitTablePOUpdate: Could not find column SourID");
  Obit_return_if_fail((in->RAOff > -1), err,
       "ObitTablePOUpdate: Could not find column RA");
  Obit_return_if_fail((in->DecOff > -1), err,
       "ObitTablePOUpdate: Could not find column Dec");
} /* end ObitTablePOUpdate */
...
static void ObitTablePODumpKey (ObitTablePO *in, ObitErr *err)
{
 ...
 /* error checks */
  ...
  /* RefDate */
  type  = OBIT_string;
  dim[0] = MAXKEYCHARTABLEPO;
  ObitInfoListAlwaysPut(info, "RDATE", type, dim, 
		  (gpointer)&in->RefDate);
   
} /* end ObitTablePODumpKey */
