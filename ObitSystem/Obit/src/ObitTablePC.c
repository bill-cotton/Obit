...
/**
 * \file ObitTablePC.c
 * ObitTablePC class function definitions.
 *
 * This class is derived from the #ObitTable class.
 */

/** name of the class defined in this file */
static gchar *myClassName = "ObitTablePC";
...
static gchar *myRowClassName = "ObitTablePCRow";
 ...
 */
static ObitTablePCRowClassInfo myRowClassInfo = {FALSE};

/*------------------  Table  ------------------------*/
/**
 * ClassInfo structure ObitTablePCClassInfo.
 * This structure is used by class objects to access class functions.
 */
static ObitTablePCClassInfo myClassInfo = {FALSE};

/*---------------Private function prototypes----------------*/
/** Private: Initialize newly instantiated Row object. */
void  ObitTablePCRowInit  (gpointer in);

/** Private: Deallocate Row members. */
void  ObitTablePCRowClear (gpointer in);

/** Private: Initialize newly instantiated object. */
void  ObitTablePCInit  (gpointer in);

/** Private: Deallocate members. */
void  ObitTablePCClear (gpointer in);

/** Private: update table specific info */
static void ObitTablePCUpdate (ObitTablePC *in, ObitErr *err);

/** Private: copy table keywords to descriptor info list */
static void ObitTablePCDumpKey (ObitTablePC *in, ObitErr *err);

/** Private: Set Class function pointers */
static void ObitTablePCClassInfoDefFn (gpointer inClass);

/** Private: Set Row Class function pointers */
static void ObitTablePCRowClassInfoDefFn (gpointer inClass);
/*----------------------Public functions---------------------------*/
...
ObitTablePCRow* newObitTablePCRow (ObitTablePC *table)
{
  ...
  if (!myRowClassInfo.initialized) ObitTablePCRowClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTablePCRow));

  /* initialize values */
  out->name = g_strdup("TablePC Row");

  /* set ClassInfo */
  out->ClassInfo = (gpointer)&myRowClassInfo;

  /* initialize other stuff */
  ObitTablePCRowInit((gpointer)out);
...
ObitTablePC* newObitTablePC (gchar* name)
{
  ...
  /* Class initialization if needed */
  if (!myClassInfo.initialized) ObitTablePCClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTablePC));
 ...
 * \param numTones The number of Tones.
 * \param err Error stack, returns if not empty.
 * \return the new object, NULL on failure.
 */
ObitTablePC* newObitTablePCValue (gchar* name, ObitData *file, olong *ver,
 	                    ObitIOAccess access,
  		     oint numPol, oint numBand, oint numTones,
		     ObitErr *err)
{
  ObitTablePC* out=NULL;
  ...
  ObitIOCode retCode;
  gchar *tabType = "AIPS PC";
  gchar *routine = "newObitTablePCValue";
 ...
 /* error checks */
  ...
  if (*ver==0) { 
    highVer = ObitTableListGetHigh (list, "AIPS PC");
  ...
  if (*ver>0) { /* has to be specified */
    ...
    if (testTab!=NULL) { /* it exists, use it if is an ObitTablePC */
      if (ObitTablePCIsA(testTab)) { /* it is an ObitTablePC */
	out = ObitTableRef(testTab);
      } else { /* needs conversion */
 	out = ObitTablePCConvert(testTab);
  ...
  ncol = 7 + out->numPol*5 + out->numBand*0 + out->numTones*0 ;
  ...
  colNo++;
  if (numPol>=1) {
    desc->FieldName[colNo] = g_strdup("STATE 1");
  ...
  if (numPol>=1) {
    desc->FieldName[colNo] = g_strdup("PC_FREQ 1");
  ...
  if (numPol>=1) {
    desc->FieldName[colNo] = g_strdup("PC_REAL 1");
    desc->FieldUnit[colNo] = g_strdup("");
    desc->type[colNo] = OBIT_float;
    for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
    desc->dim[colNo][0] = numTones;
  ...
  if (numPol>=1) {
    desc->FieldName[colNo] = g_strdup("PC_IMAG 1");
    desc->FieldUnit[colNo] = g_strdup("");
    desc->type[colNo] = OBIT_float;
    for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
    desc->dim[colNo][0] = numTones;
  ...
  if (numPol>=1) {
    desc->FieldName[colNo] = g_strdup("PC_RATE 1");
  ...
  if (numPol>=2) {
    desc->FieldName[colNo] = g_strdup("STATE 2");
    desc->FieldUnit[colNo] = g_strdup("PERCENT");
    desc->type[colNo] = OBIT_float;
    for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
    desc->dim[colNo][0] = 4;
  ...
  if (numPol>=2) {
    desc->FieldName[colNo] = g_strdup("PC_FREQ 2");
    desc->FieldUnit[colNo] = g_strdup("HZ");
    desc->type[colNo] = OBIT_double;
    for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
    desc->dim[colNo][0] = numTones;
  ...
  if (numPol>=2) {
    desc->FieldName[colNo] = g_strdup("PC_REAL 2");
    desc->FieldUnit[colNo] = g_strdup("");
    desc->type[colNo] = OBIT_float;
    for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
    desc->dim[colNo][0] = numTones;
  ...
  if (numPol>=2) {
    desc->FieldName[colNo] = g_strdup("PC_IMAG 2");
    desc->FieldUnit[colNo] = g_strdup("");
    desc->type[colNo] = OBIT_float;
    for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
    desc->dim[colNo][0] = numTones;
  ...
  if (numPol>=2) {
    desc->FieldName[colNo] = g_strdup("PC_RATE 2");
    desc->FieldUnit[colNo] = g_strdup("SEC/SEC");
    desc->type[colNo] = OBIT_float;
    for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
    desc->dim[colNo][0] = numTones;
 ...
 return out;
} /* end newObitTablePCValue */

/**
 * Convert an ObitTable to an ObitTablePC.
 ...
 * \return pointer to the new object.
 */
ObitTablePC* ObitTablePCCopy (ObitTablePC *in, ObitTablePC *out, ObitErr *err)
{
  gchar *routine = "ObitTablePCCopy";
 ...
 /* error checks */
  ...
  out = (ObitTablePC*)ObitTableCopy ((ObitTable*)in, (ObitTable*)out, err);
  if (err->error) /* add traceback,return */
 ...
 * \param iPCRow   Row number, -1 -> next
		     ...
		     ObitErr *err)
{
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTablePCReadRow";
  ...
  if (in->myStatus == OBIT_Inactive) {
    Obit_log_error(err, OBIT_Error,
		   "AIPS PC Table is inactive for  %s ", in->name);
    return retCode;
 }

  /* read row iPCRow */
  retCode = ObitTableRead ((ObitTable*)in, iPCRow, NULL,  err);
  if (err->error) 
    Obit_traceback_val (err, routine, in->name, retCode);
		     ...
		     ObitErr *err)
{
  ...
  if (in->myStatus == OBIT_Inactive) {
    Obit_log_error(err, OBIT_Error,
		   "PC Table is inactive for  %s ", in->name);
    return;
 }
 ...
 * correct table buffer locations using ObitTablePCSetRow  
 * \param in       Table to read
 * \param iPCRow   Row number, -1 -> next
		      ...
		      ObitErr *err)
{
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTablePCWriteRow";
  ...
  /* Write row iPCRow */
  retCode = ObitTableWrite ((ObitTable*)in, iPCRow, NULL,  err);
  if (err->error) 
    Obit_traceback_val (err, routine,in->name, retCode);

  return retCode;
} /*  end ObitTablePCWriteRow */
...
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gchar *routine = "ObitTablePCClose";
 ...
 */
void ObitTablePCRowInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTablePCRow *in = inn;
 ...
 *           Actually it should be an ObitTablePCRow* cast to an Obit*.
 */
void ObitTablePCRowClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTablePCRow *in = inn;
...
void ObitTablePCRowClassInit (void)
{
  ...
  ObitTablePCRowClassInfoDefFn ((gpointer)&myRowClassInfo);
 
  myRowClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTablePCRowClassInit */
...
static void ObitTablePCRowClassInfoDefFn (gpointer inClass)
{
  ObitTablePCRowClassInfo *theClass = (ObitTablePCRowClassInfo*)inClass;
  ...
  theClass->ObitClassInit = (ObitClassInitFP)ObitTablePCRowClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTablePCRowClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTablePCRowGetClass;
  theClass->newObit         = NULL;
  theClass->newObitTableRow = (newObitTableRowFP)newObitTablePCRow;
  theClass->ObitCopy        = NULL;
  theClass->ObitClone       = NULL;
  theClass->ObitClear       = (ObitClearFP)ObitTablePCRowClear;
  theClass->ObitInit        = (ObitInitFP)ObitTablePCRowInit;

} /* end ObitTablePCRowClassDefFn */
 ...
 * \param inn Pointer to the object to initialize.
 */
void ObitTablePCInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTablePC *in = inn;
 ...
 * For some reason this wasn't build into the GType class.
 * \param  inn Pointer to the object to deallocate.
 *           Actually it should be an ObitTablePC* cast to an Obit*.
 */
void ObitTablePCClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTablePC *in = inn;
...
void ObitTablePCClassInit (void)
{
  ...
  /* Set function pointers */
  ObitTablePCClassInfoDefFn ((gpointer)&myClassInfo);
 
  myClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTablePCClassInit */
...
static void ObitTablePCClassInfoDefFn (gpointer inClass)
{
  ObitTablePCClassInfo *theClass = (ObitTablePCClassInfo*)inClass;
  ...
  /* function pointers defined or overloaded this class */
  theClass->ObitClassInit = (ObitClassInitFP)ObitTablePCClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTablePCClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTablePCGetClass;
  theClass->newObit       = (newObitFP)newObitTablePC;
  theClass->ObitCopy      = (ObitCopyFP)ObitTablePCCopy;
  theClass->ObitClone     = (ObitCloneFP)ObitTableClone;
  theClass->ObitClear     = (ObitClearFP)ObitTablePCClear;
  theClass->ObitInit      = (ObitInitFP)ObitTablePCInit;
  theClass->ObitTableConvert = (ObitTableConvertFP)ObitTablePCConvert;
  theClass->ObitTableOpen    = (ObitTableOpenFP)ObitTablePCOpen;
  theClass->ObitTableClose   = (ObitTableCloseFP)ObitTablePCClose;
  ...
  theClass->ObitTableReadRow = 
    (ObitTableReadRowFP)ObitTablePCReadRow;
  theClass->ObitTableWriteRow = 
    (ObitTableWriteRowFP)ObitTablePCWriteRow;

} /* end ObitTablePCClassDefFn */
...
static void ObitTablePCUpdate (ObitTablePC *in, ObitErr *err)
{
 ...
 /* error checks */
  ...
  if (!ObitInfoListGet(in->myDesc->info, "NO_BAND", &type, dim, 
   ...
   /* numTones */
  if (!ObitInfoListGet(in->myDesc->info, "NO_TONES", &type, dim, 
		       (gpointer)&in->numTones, err)) return;
  ...
  if (desc->FieldName) {
    for (i=0; i<desc->nfield; i++) {
	 ...
	 in->CableCalOff = desc->offset[i];
      ...
      if (!strncmp (desc->FieldName[i], "STATE 1", 7)) {
	     in->State1Off = desc->offset[i];
 	     in->State1Col = i;
      }
      if (!strncmp (desc->FieldName[i], "PC_FREQ 1", 9)) {
	     in->PCFreq1Off = desc->offset[i];
 	     in->PCFreq1Col = i;
      }
      if (!strncmp (desc->FieldName[i], "PC_REAL 1", 9)) {
	     in->PCReal1Off = desc->offset[i];
 	     in->PCReal1Col = i;
      }
      if (!strncmp (desc->FieldName[i], "PC_IMAG 1", 9)) {
	     in->PCImag1Off = desc->offset[i];
 	     in->PCImag1Col = i;
      }
      if (!strncmp (desc->FieldName[i], "PC_RATE 1", 9)) {
	     in->PCRate1Off = desc->offset[i];
 	     in->PCRate1Col = i;
      }
      if (!strncmp (desc->FieldName[i], "STATE 2", 7)) {
	     in->State2Off = desc->offset[i];
 	     in->State2Col = i;
      }
      if (!strncmp (desc->FieldName[i], "PC_FREQ 2", 9)) {
	     in->PCFreq2Off = desc->offset[i];
 	     in->PCFreq2Col = i;
      }
      if (!strncmp (desc->FieldName[i], "PC_REAL 2", 9)) {
	     in->PCReal2Off = desc->offset[i];
 	     in->PCReal2Col = i;
      }
      if (!strncmp (desc->FieldName[i], "PC_IMAG 2", 9)) {
	     in->PCImag2Off = desc->offset[i];
 	     in->PCImag2Col = i;
      }
      if (!strncmp (desc->FieldName[i], "PC_RATE 2", 9)) {
  ...
  Obit_return_if_fail((in->TimeOff > -1), err,
       "ObitTablePCUpdate: Could not find column Time");
  Obit_return_if_fail((in->TimeIOff > -1), err,
       "ObitTablePCUpdate: Could not find column TimeI");
  Obit_return_if_fail((in->SourIDOff > -1), err,
       "ObitTablePCUpdate: Could not find column SourID");
  Obit_return_if_fail((in->antennaNoOff > -1), err,
       "ObitTablePCUpdate: Could not find column antennaNo");
  Obit_return_if_fail((in->ArrayOff > -1), err,
       "ObitTablePCUpdate: Could not find column Array");
  Obit_return_if_fail((in->FreqIDOff > -1), err,
       "ObitTablePCUpdate: Could not find column FreqID");
  Obit_return_if_fail((in->CableCalOff > -1), err,
       "ObitTablePCUpdate: Could not find column CableCal");
  if (in->numPol>=1) {
    Obit_return_if_fail((in->State1Off > -1), err,
       "ObitTablePCUpdate: Could not find column State1");
    Obit_return_if_fail((in->PCFreq1Off > -1), err,
       "ObitTablePCUpdate: Could not find column PCFreq1");
    Obit_return_if_fail((in->PCReal1Off > -1), err,
       "ObitTablePCUpdate: Could not find column PCReal1");
    Obit_return_if_fail((in->PCImag1Off > -1), err,
       "ObitTablePCUpdate: Could not find column PCImag1");
  } /* end of if index in range */
  if (in->numPol>=2) {
    Obit_return_if_fail((in->State2Off > -1), err,
       "ObitTablePCUpdate: Could not find column State2");
    Obit_return_if_fail((in->PCFreq2Off > -1), err,
       "ObitTablePCUpdate: Could not find column PCFreq2");
    Obit_return_if_fail((in->PCReal2Off > -1), err,
       "ObitTablePCUpdate: Could not find column PCReal2");
    Obit_return_if_fail((in->PCImag2Off > -1), err,
       "ObitTablePCUpdate: Could not find column PCImag2");
  } /* end of if index in range */
} /* end ObitTablePCUpdate */
...
static void ObitTablePCDumpKey (ObitTablePC *in, ObitErr *err)
{
 ...
 /* error checks */
  ...
  type  = OBIT_oint;
  dim[0] = 1;
  ObitInfoListAlwaysPut(info, "NO_TONES", type, dim, 
		  (gpointer)&in->numTones);
   
} /* end ObitTablePCDumpKey */
