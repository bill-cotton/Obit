		     ...
		     ObitErr *err)
{
 ...
 /* error checks */
  ...
  for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
  colNo++;
  desc->FieldName[colNo] = g_strdup("DELTAZ  ");
  desc->FieldUnit[colNo] = g_strdup("DEGREE");
  desc->type[colNo] = OBIT_float;
  for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
  colNo++;
...
ObitIOCode ObitTableCCClose (ObitTableCC *in, ObitErr *err)
{
  ...
  /* Check for bad nrow */
  if ((in->myDesc->nrow<-10) || (in->myDesc->nrow>50000000) ||
      (((ObitTableDesc*)(in->myIO->myDesc))->nrow<-10) ||
      (((ObitTableDesc*)(in->myIO->myDesc))->nrow>50000000)) {
    Obit_log_error(err, OBIT_Error,
		   "AIPS CC Table BAD nrow %s %d %d ", 
		   in->name, in->myDesc->nrow,
		   ((ObitTableDesc*)(in->myIO->myDesc))->nrow);
  } /* end check bad nrow */
  ...
  if (desc->FieldName) {
    for (i=0; i<desc->nfield; i++) {
	 ...
	 in->DeltaYOff = desc->offset[i];
      ...
      if (!strncmp (desc->FieldName[i], "DELTAZ  ", 8)) {
	 in->DeltaZOff = desc->offset[i];
 	 in->DeltaZCol = i;
      }
