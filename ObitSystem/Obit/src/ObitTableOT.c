...
/**
 * \file ObitTableOT.c
 * ObitTableOT class function definitions.
 *
 * This class is derived from the #ObitTable class.
 */

/** name of the class defined in this file */
static gchar *myClassName = "ObitTableOT";
...
static gchar *myRowClassName = "ObitTableOTRow";
 ...
 */
static ObitTableOTRowClassInfo myRowClassInfo = {FALSE};

/*------------------  Table  ------------------------*/
/**
 * ClassInfo structure ObitTableOTClassInfo.
 * This structure is used by class objects to access class functions.
 */
static ObitTableOTClassInfo myClassInfo = {FALSE};

/*---------------Private function prototypes----------------*/
/** Private: Initialize newly instantiated Row object. */
void  ObitTableOTRowInit  (gpointer in);

/** Private: Deallocate Row members. */
void  ObitTableOTRowClear (gpointer in);

/** Private: Initialize newly instantiated object. */
void  ObitTableOTInit  (gpointer in);

/** Private: Deallocate members. */
void  ObitTableOTClear (gpointer in);

/** Private: update table specific info */
static void ObitTableOTUpdate (ObitTableOT *in, ObitErr *err);

/** Private: copy table keywords to descriptor info list */
static void ObitTableOTDumpKey (ObitTableOT *in, ObitErr *err);

/** Private: Set Class function pointers */
static void ObitTableOTClassInfoDefFn (gpointer inClass);

/** Private: Set Row Class function pointers */
static void ObitTableOTRowClassInfoDefFn (gpointer inClass);
/*----------------------Public functions---------------------------*/
 ...
 */
ObitTableOTRow* newObitTableOTRow (ObitTableOT *table)
{
  ObitTableOTRow* out;
  odouble   *dRow;
  oint      *iRow;
  ...
  if (!myRowClassInfo.initialized) ObitTableOTRowClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTableOTRow));

  /* initialize values */
  out->name = g_strdup("TableOT Row");

  /* set ClassInfo */
  out->ClassInfo = (gpointer)&myRowClassInfo;

  /* initialize other stuff */
  ObitTableOTRowInit((gpointer)out);
 ...
 * \return the new object.
 */
ObitTableOT* newObitTableOT (gchar* name)
{
  ObitTableOT* out;

  /* Class initialization if needed */
  if (!myClassInfo.initialized) ObitTableOTClassInit();

  /* allocate/init structure */
  out = g_malloc0(sizeof(ObitTableOT));
 ...
 */
gconstpointer ObitTableOTGetClass (void)
{
  /* Class initialization if needed */
  if (!myClassInfo.initialized) ObitTableOTClassInit();
 ...
 */
ObitTableOT* newObitTableOTValue (gchar* name, ObitData *file, olong *ver,
 	                    ObitIOAccess access,
  		    
		     ObitErr *err)
{
  ObitTableOT* out=NULL;
  ...
  ObitIOCode retCode;
  gchar *tabType = "AIPS OT";
  gchar *routine = "newObitTableOTValue";
 ...
 /* error checks */
  ...
  if (*ver==0) { 
    highVer = ObitTableListGetHigh (list, "AIPS OT");
  ...
  if (*ver>0) { /* has to be specified */
    ...
    if (testTab!=NULL) { /* it exists, use it if is an ObitTableOT */
      if (ObitTableOTIsA(testTab)) { /* it is an ObitTableOT */
	out = ObitTableRef(testTab);
      } else { /* needs conversion */
 	out = ObitTableOTConvert(testTab);
  ...
  for (i=0; i<MAXINFOELEMDIM; i++) desc->dim[colNo][i] = 1;
  colNo++;
  desc->FieldName[colNo] = g_strdup("OVER TOP");
  desc->FieldUnit[colNo] = g_strdup("");
 ...
 return out;
} /* end newObitTableOTValue */

/**
 * Convert an ObitTable to an ObitTableOT.
 ...
 */
ObitTableOT* ObitTableOTConvert (ObitTable* in)
{
  ObitTableOT *out;
 ...
 * \return pointer to the new object.
 */
ObitTableOT* ObitTableOTCopy (ObitTableOT *in, ObitTableOT *out, ObitErr *err)
{
  gchar *routine = "ObitTableOTCopy";
 ...
 /* error checks */
  ...
  out = (ObitTableOT*)ObitTableCopy ((ObitTable*)in, (ObitTable*)out, err);
  if (err->error) /* add traceback,return */
 ...
 */
ObitIOCode ObitTableOTOpen (ObitTableOT *in, ObitIOAccess access, 
			  ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gint32 dim[MAXINFOELEMDIM] = {1,1,1,1,1};
  olong nRowPIO;
  gchar *routine = "ObitTableOTOpen";
 ...
 * \param iOTRow   Row number, -1 -> next
 * \param row      Table Row structure to receive data
 * \param err ObitErr for reporting errors.
 * \return return code, OBIT_IO_OK=> OK
 */
ObitIOCode 
ObitTableOTReadRow  (ObitTableOT *in, olong iOTRow, ObitTableOTRow *row,
		     ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  odouble   *dRow;
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTableOTReadRow";
  ...
  if (in->myStatus == OBIT_Inactive) {
    Obit_log_error(err, OBIT_Error,
		   "AIPS OT Table is inactive for  %s ", in->name);
    return retCode;
 }

  /* read row iOTRow */
  retCode = ObitTableRead ((ObitTable*)in, iOTRow, NULL,  err);
  if (err->error) 
    Obit_traceback_val (err, routine, in->name, retCode);
  ...
  row->Antenna = iRow[in->AntennaOff];
  row->OverTop = lRow[in->OverTopOff];
  row->status = iRow[in->myDesc->statusOff];

  return retCode;
} /*  end ObitTableOTReadRow */
 ...
 */
void 
ObitTableOTSetRow  (ObitTableOT *in, ObitTableOTRow *row,
		     ObitErr *err)
{
  odouble   *dRow;
  oint      *iRow;
  ...
  if (in->myStatus == OBIT_Inactive) {
    Obit_log_error(err, OBIT_Error,
		   "OT Table is inactive for  %s ", in->name);
    return;
 }
 ...
 * correct table buffer locations using ObitTableOTSetRow  
 * \param in       Table to read
 * \param iOTRow   Row number, -1 -> next
 ...
 */
ObitIOCode 
ObitTableOTWriteRow  (ObitTableOT *in, olong iOTRow, ObitTableOTRow *row,
		      ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gshort    *siRow;
  ...
  gboolean  *lRow;
  guint8    *bRow;
  gchar *routine = "ObitTableOTWriteRow";
  ...
  iRow[in->AntennaOff] = row->Antenna;
  lRow[in->OverTopOff] = row->OverTop;
  ...
  /* Write row iOTRow */
  retCode = ObitTableWrite ((ObitTable*)in, iOTRow, NULL,  err);
  if (err->error) 
    Obit_traceback_val (err, routine,in->name, retCode);

  return retCode;
} /*  end ObitTableOTWriteRow */
 ...
 */
ObitIOCode ObitTableOTClose (ObitTableOT *in, ObitErr *err)
{
  ObitIOCode retCode = OBIT_IO_SpecErr;
  gchar *routine = "ObitTableOTClose";
 ...
 */
void ObitTableOTRowInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableOTRow *in = inn;
 ...
 *           Actually it should be an ObitTableOTRow* cast to an Obit*.
 */
void ObitTableOTRowClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableOTRow *in = inn;
...
} /* end ObitTableOTRowClear */

/**
 * Initialize global ClassInfo Structure.
 */
void ObitTableOTRowClassInit (void)
{
  ...
  ObitTableOTRowClassInfoDefFn ((gpointer)&myRowClassInfo);
 
  myRowClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTableOTRowClassInit */
...
static void ObitTableOTRowClassInfoDefFn (gpointer inClass)
{
  ObitTableOTRowClassInfo *theClass = (ObitTableOTRowClassInfo*)inClass;
  ...
  theClass->ObitClassInit = (ObitClassInitFP)ObitTableOTRowClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTableOTRowClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTableOTRowGetClass;
  theClass->newObit         = NULL;
  theClass->newObitTableRow = (newObitTableRowFP)newObitTableOTRow;
  theClass->ObitCopy        = NULL;
  theClass->ObitClone       = NULL;
  theClass->ObitClear       = (ObitClearFP)ObitTableOTRowClear;
  theClass->ObitInit        = (ObitInitFP)ObitTableOTRowInit;

} /* end ObitTableOTRowClassDefFn */
 ...
 * \param inn Pointer to the object to initialize.
 */
void ObitTableOTInit  (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableOT *in = inn;
 ...
 * For some reason this wasn't build into the GType class.
 * \param  inn Pointer to the object to deallocate.
 *           Actually it should be an ObitTableOT* cast to an Obit*.
 */
void ObitTableOTClear (gpointer inn)
{
  ObitClassInfo *ParentClass;
  ObitTableOT *in = inn;
...
} /* end ObitTableOTClear */

/**
 * Initialize global ClassInfo Structure.
 */
void ObitTableOTClassInit (void)
{
  ...
  /* Set function pointers */
  ObitTableOTClassInfoDefFn ((gpointer)&myClassInfo);
 
  myClassInfo.initialized = TRUE; /* Now initialized */
 
} /* end ObitTableOTClassInit */
...
static void ObitTableOTClassInfoDefFn (gpointer inClass)
{
  ObitTableOTClassInfo *theClass = (ObitTableOTClassInfo*)inClass;
  ...
  /* function pointers defined or overloaded this class */
  theClass->ObitClassInit = (ObitClassInitFP)ObitTableOTClassInit;
  theClass->ObitClassInfoDefFn = (ObitClassInfoDefFnFP)ObitTableOTClassInfoDefFn;
  theClass->ObitGetClass  = (ObitGetClassFP)ObitTableOTGetClass;
  theClass->newObit       = (newObitFP)newObitTableOT;
  theClass->ObitCopy      = (ObitCopyFP)ObitTableOTCopy;
  theClass->ObitClone     = (ObitCloneFP)ObitTableClone;
  theClass->ObitClear     = (ObitClearFP)ObitTableOTClear;
  theClass->ObitInit      = (ObitInitFP)ObitTableOTInit;
  theClass->ObitTableConvert = (ObitTableConvertFP)ObitTableOTConvert;
  theClass->ObitTableOpen    = (ObitTableOpenFP)ObitTableOTOpen;
  theClass->ObitTableClose   = (ObitTableCloseFP)ObitTableOTClose;
  ...
  theClass->ObitTableReadRow = 
    (ObitTableReadRowFP)ObitTableOTReadRow;
  theClass->ObitTableWriteRow = 
    (ObitTableWriteRowFP)ObitTableOTWriteRow;

} /* end ObitTableOTClassDefFn */
...
static void ObitTableOTUpdate (ObitTableOT *in, ObitErr *err)
{
 ...
 /* error checks */
  ...
  if (desc->FieldName) {
    for (i=0; i<desc->nfield; i++) {
	 ...
	 in->SourIDOff = desc->offset[i];
      ...
      if (!strncmp (desc->FieldName[i], "ANTENNA    ", 11)) {
	 in->AntennaOff = desc->offset[i];
 	 in->AntennaCol = i;
      }
      if (!strncmp (desc->FieldName[i], "OVER TOP", 8)) {
	 in->OverTopOff = desc->offset[i];
 	 in->OverTopCol = i;
      }
     }
  }
  ...
  Obit_return_if_fail((in->TimeOff > -1), err,
       "ObitTableOTUpdate: Could not find column Time");
  Obit_return_if_fail((in->TimeIOff > -1), err,
       "ObitTableOTUpdate: Could not find column TimeI");
  Obit_return_if_fail((in->SourIDOff > -1), err,
       "ObitTableOTUpdate: Could not find column SourID");
  Obit_return_if_fail((in->AntennaOff > -1), err,
       "ObitTableOTUpdate: Could not find column Antenna");
} /* end ObitTableOTUpdate */
