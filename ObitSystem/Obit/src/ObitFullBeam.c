 ...
 * is the same as the frequencies in uvData 
 * \param name    An optional name for the object.
 ...
 * \param uvData  ObitUVData defining order of frequencies/IFs
 *                Should have the same number of freq/IFs.
 * \return the new object.
 */
				  ...
				  ObitUV *uvData, ObitErr *err)
{
  ...
  olong nUVFreq, nUVIF, nImgFreq, nImgIF, nChIF, naxis[5], nx, ny;
  ...
  /* Check compatability between image and uvData - allow averaging */
     nUVFreq  = uvData->myDesc->inaxes[uvData->myDesc->jlocf];
     nImgFreq = image->myDesc->inaxes[image->myDesc->jlocf];
     /* Obit_retval_if_fail (((nImgFreq<=1) || (nImgFreq==nUVFreq)), err, out,
	"%s: Incompatable image,uv no. channels %d %d",
	routine, nImgFreq, nUVFreq);*/
     ...
     /* Obit_retval_if_fail (((nImgIF<=1) || (nImgIF==nUVIF)), err, out,
	"%s: Unequal image,uv no. IFs %d %d",
	routine, nImgIF, nUVIF);*/

  /* How big?  */
  nChIF = nUVFreq * nImgIF;
  ...
  iplane = 0;
  if ((uvData->myDesc->jlocf<uvData->myDesc->jlocif) || (uvData->myDesc->jlocif<0) ) {
    /* Frequency first */
